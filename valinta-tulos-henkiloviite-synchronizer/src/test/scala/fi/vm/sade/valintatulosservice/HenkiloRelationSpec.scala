...
class HenkiloRelationSpec extends Specification {
  "relatedHenkilot" >> {
    ...
    "returns a seqs that contain master with all slaves" >> {
      ...
      related.find(_.contains("master1")) must beSome(
        (_: Seq[String]) must (contain("slave1", "slave2") and not(contain("slave3")))
      )
    }
  }
  "allPairs" >> {
  ...
  "henkiloRelations" >> {
    val relations = HenkiloviiteSynchronizer.henkiloRelations(
      List(
        Henkiloviite("master1", "slave1"),
        Henkiloviite("master1", "slave2"),
    ...
    relations must contain(
      HenkiloRelation("master1", "slave1"),
      HenkiloRelation("slave1", "master1")
    )
    relations must contain(
      HenkiloRelation("master1", "slave2"),
      HenkiloRelation("slave2", "master1")
    )
    relations must contain(
      HenkiloRelation("master1", "slave3"),
      HenkiloRelation("slave3", "master1")
    )

    relations must contain(
      HenkiloRelation("master2", "slave4"),
      HenkiloRelation("slave4", "master2")
    )
    relations must contain(
      HenkiloRelation("master2", "slave5"),
      HenkiloRelation("slave5", "master2")
    )

    relations must contain(
      HenkiloRelation("master3", "slave6"),
      HenkiloRelation("slave6", "master3")
    )
  }
