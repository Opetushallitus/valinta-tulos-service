...
class ScalatraBootstrap extends LifeCycle with Logging {
  ...
  override def init(context: ServletContext) {
    ...
    implicit val appConfig: VtsAppConfig = VtsAppConfig.fromOptionalString(
      Option(context.getAttribute("valintatulos.profile").asInstanceOf[String])
    )
    ...
    lazy val hakuService = HakuService(
      appConfig,
    ...
    lazy val valintarekisteriDb = new ValintarekisteriDb(
      appConfig.settings.valintaRekisteriDbConfig,
      appConfig.isInstanceOf[IT]
    )
    ...
    lazy val sijoitteluService =
      new SijoitteluService(valintarekisteriDb, authorizer, hakuService, audit)

    lazy val valintarekisteriValintatulosDao = new ValintarekisteriValintatulosDaoImpl(
      valintarekisteriDb
    )
    lazy val valintarekisteriRaportointiService = new ValintarekisteriRaportointiServiceImpl(
      valintarekisteriDb,
      valintarekisteriValintatulosDao
    )
    lazy val valintarekisteriSijoittelunTulosClient =
    ...
    lazy val ataruHakemusTarjontaEnricher =
      new AtaruHakemusEnricher(appConfig, hakuService, oppijanumerorekisteriService)
    lazy val hakemusRepository =
      new HakemusRepository(hakuAppRepository, ataruHakemusRepository, ataruHakemusTarjontaEnricher)
    ...
    lazy val streamingValintatulosService =
      new StreamingValintatulosService(valintatulosService, valintarekisteriDb, hakijaDTOClient)(
        appConfig
      )
    ...
    lazy val hyvaksymiskirjeService =
      new HyvaksymiskirjeService(valintarekisteriDb, hakuService, audit, authorizer)
    lazy val lukuvuosimaksuService = new LukuvuosimaksuService(valintarekisteriDb, audit)
    lazy val hakemustenTulosHakuLock: HakemustenTulosHakuLock = new HakemustenTulosHakuLock(
      appConfig.settings.hakuResultsLoadingLockQueueLimit,
      appConfig.settings.hakuResultsLoadingLockSeconds
    )

    val sijoitteluajoDeleteScheduler =
      new SijoitteluajoDeleteScheduler(valintarekisteriDb, appConfig)
    sijoitteluajoDeleteScheduler.startScheduler()

    mountBasicVts()

    context.mount(
      new HakukohdeRefreshServlet(valintarekisteriDb, hakukohdeRecordService),
      "/virkistys"
    )
    ...
    def mountBasicVts(): Unit = {
      ...
      context.mount(
        new VirkailijanVastaanottoServlet(valintatulosService, vastaanottoService),
        "/virkailija",
        "virkailija"
      )
      context.mount(
        new LukuvuosimaksuServletWithoutCAS(lukuvuosimaksuService),
        "/lukuvuosimaksu",
        "lukuvuosimaksu"
      )
      context.mount(
        handler = new MuutoshistoriaServlet(
          valinnantulosService,
          valintarekisteriDb,
          skipAuditForServiceCall = true
        ),
        urlPattern = "/muutoshistoria",
        name = "muutoshistoria"
      )
      context.mount(
        new PrivateValintatulosServlet(
          valintatulosService,
          streamingValintatulosService,
          vastaanottoService,
      ...
      context.mount(
        new EnsikertalaisuusServlet(
          valintarekisteriDb,
          appConfig.settings.valintaRekisteriEnsikertalaisuusMaxPersonOids
        ),
        "/ensikertalaisuus",
        "ensikertalaisuus"
      )
      context.mount(
        new HakijanVastaanottoServlet(vastaanottoService),
        "/vastaanotto",
        "vastaanotto"
      )
      context.mount(
        new ErillishakuServlet(
          valinnantulosService,
          hyvaksymiskirjeService,
          userDetailsService,
          appConfig
        ),
      ...
      context
        .addFilter(
          "cas",
          createCasFilter(casSessionService, appConfig.securityContext.requiredRoles)
        )
        .addMappingForUrlPatterns(util.EnumSet.allOf(classOf[DispatcherType]), true, "/cas/haku/*")
      context
        .addFilter("kelaCas", createCasFilter(casSessionService, Set.empty))
        .addMappingForUrlPatterns(util.EnumSet.allOf(classOf[DispatcherType]), true, "/cas/kela/*")
      context.mount(
        new PublicValintatulosServlet(
          audit,
          valintatulosService,
          streamingValintatulosService,
          vastaanottoService,
      ...
      context.mount(
        new KelaServlet(
          audit,
          new KelaService(HakijaResolver(appConfig), hakuService, valintarekisteriDb),
          valintarekisteriDb
        ),
        "/cas/kela",
        "cas/kela"
      ...
      context.mount(
        new KelaHealthCheckServlet(
          audit,
          valintarekisteriDb,
          appConfig,
          new VtsKelaAuthenticationClient(appConfig)
        ),
        "/health-check/kela",
        "health-check/kela"
      )

      val valintaesitysService = new ValintaesitysService(
        hakuService,
        authorizer,
        valintarekisteriDb,
        valintarekisteriDb,
      ...
      context.mount(
        new SijoitteluServlet(sijoitteluService, valintarekisteriDb),
        "/auth/sijoittelu",
        "auth/sijoittelu"
      )
      context.mount(
        new SijoittelunTulosServlet(
          valintatulosService,
          valintaesitysService,
          valinnantulosService,
          hyvaksymiskirjeService,
          lukuvuosimaksuService,
          hakuService,
          authorizer,
          sijoitteluService,
          valintarekisteriDb
        ),
        "/auth/sijoitteluntulos",
        "auth/sijoitteluntulos"
      )
      context.mount(
        new HyvaksymiskirjeServlet(hyvaksymiskirjeService, valintarekisteriDb),
        "/auth/hyvaksymiskirje",
        "auth/hyvaksymiskirje"
      )
      context.mount(
        new LukuvuosimaksuServletWithCAS(
          lukuvuosimaksuService,
          valintarekisteriDb,
          hakuService,
          authorizer
      ...
      context.mount(
        handler = new MuutoshistoriaServlet(valinnantulosService, valintarekisteriDb),
        urlPattern = "/auth/muutoshistoria",
        name = "auth/muutoshistoria"
      )
      context.mount(
        new ValintaesitysServlet(valintaesitysService, valintarekisteriDb),
        "/auth/valintaesitys",
        "auth/valintaesitys"
      )
      context.mount(
        new PuuttuvienTulostenMetsastajaServlet(
          audit,
          valintarekisteriDb,
          hakuAppRepository,
          appConfig.properties("host.virkailija")
        ),
        "/auth/puuttuvat",
        "auth/puuttuvat"
      )
      context.mount(
        new HyvaksynnanEhtoServlet(
          valintarekisteriDb,
          hakuService,
          hakemusRepository,
          authorizer,
        ...
        "/auth/hyvaksynnan-ehto",
        "auth/hyvaksynnan-ehto"
      )
      context.mount(
        new HyvaksynnanEhtoMuutoshistoriaServlet(
          valintarekisteriDb,
          hakuService,
          hakemusRepository,
          authorizer,
        ...
        "/auth/hyvaksynnan-ehto-muutoshistoria",
        "auth/hyvaksynnan-ehto-muutoshistoria"
      )

      lazy val mailPollerRepository: MailPollerRepository = valintarekisteriDb
      lazy val mailPoller: MailPoller = new MailPoller(
      ...
      lazy val mailDecorator: MailDecorator =
        new MailDecorator(hakuService, oppijanTunnistusService, appConfig.ohjausparametritService)
      context.mount(
        new PublicEmailStatusServlet(mailPoller, valintarekisteriDb, audit),
        "/auth/vastaanottoposti",
        "auth/vastaanottoposti"
      )

      val registry: EmailerRegistry = EmailerRegistry.fromString(
        Option(System.getProperty("vtemailer.profile"))
          .getOrElse(if (appConfig.isInstanceOf[IT]) "it" else "default")
      )(mailPoller, mailDecorator)
      val emailerService =
        new EmailerService(registry, valintarekisteriDb, appConfig.settings.emailerCronString)
      context.mount(
        new EmailerServlet(emailerService, valintarekisteriDb, audit),
        "/auth/emailer",
        "auth/emailer"
      )
    }
