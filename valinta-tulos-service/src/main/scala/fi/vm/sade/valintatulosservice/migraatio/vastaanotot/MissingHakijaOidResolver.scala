...
object HakijaResolver {
  def apply(appConfig: VtsAppConfig): HakijaResolver =
    appConfig match {
      case _: StubbedExternalDeps =>
        new HakijaResolver {
          override def findPersonByHetu(hetu: String, timeout: Duration): Option[Henkilo] =
            hetu match {
              case "face-beef" =>
...
class MissingHakijaOidResolver(appConfig: VtsAppConfig)
    extends JsonFormats
    with Logging
    with HakijaResolver {
  private val hakuClient = createCasClient(appConfig, "/haku-app")
  private val oppijanumerorekisteriClient =
    createCasClient(appConfig, "/oppijanumerorekisteri-service")

  case class HakemusHenkilo(
  ...
  private def findPersonOidByHetu(hetu: String): Option[String] =
    findPersonByHetu(hetu).map(_.oidHenkilo)
  ...
  override def findPersonByHetu(hetu: String, timeout: Duration = 60 seconds): Option[Henkilo] = {
    ...
    val requestUri = createUri(
      appConfig.ophUrlProperties.url("oppijanumerorekisteri-service.henkiloPerusByHetu", hetu)
    )
    oppijanumerorekisteriClient
      .fetch(Request(uri = requestUri)) {
  ...
  def findPersonOidByHakemusOid(hakemusOid: String): Option[String] = {
    ...
    val henkiloFromHakemus = retryingClient
      .fetch(Request(method = Method.GET, uri = url)) {
        case r if 200 == r.status.code => r.as[HakemusHenkilo]
        case r =>
          Task.fail(
            new RuntimeException(
              s"Got non-OK response from haku-app when fetching hakemus $hakemusOid: ${r.toString}"
            )
          )
      }
    ...
    henkiloFromHakemus match {
      ...
      case Some(henkilo) =>
        henkilo.hetu
          .map(findPersonOidByHetu)
          .getOrElse({ throw new RuntimeException(s"Hakemuksella $hakemusOid ei hakijaoidia!") })
      case None =>
        throw new RuntimeException(s"Hakemuksen $hakemusOid henkilotietoja ei saatu parsittua.")
    }
  }

  private def handleFailure(t: Throwable, message: String) =
    t match {
      case pf: ParseFailure =>
        logger.error(s"Got parse exception when $message : $pf, ${pf.sanitized}", t); None
