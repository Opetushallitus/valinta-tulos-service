...
trait ValintarekisteriRaportointiService {
  ...
  def hakemuksetVainHakukohteenTietojenKanssa(
    sijoitteluAjo: SijoitteluAjo,
    hakukohdeOid: HakukohdeOid
  ): List[KevytHakijaDTO]
}

class ValintarekisteriRaportointiServiceImpl(
...
) extends ValintarekisteriRaportointiService
    with Logging {

  private def tryOrThrow[T](t: => T): T =
    Try(t) match {
  ...
  override def hakemukset(
    ...
    timed("RaportointiService.hakemukset", 1000) {
      ...
      val hakijat: List[HakijaDTO] = if (hakukohdeOids.isDefined && 0 != hakukohdeOids.get.size) {
        ...
        hakukohdeOids.get.flatMap(
          SijoitteluajonHakijat.dto(repository, sijoitteluajoId, hakuOid, _, Some(valinnantulokset))
        )
      } else {
        SijoitteluajonHakijat.dto(repository, sijoitteluajoId, hakuOid)
      }
      timed("RaportointiService.hakemukset -> hakijoiden konvertointi", 1000) {
        konvertoiHakijat(
          hyvaksytyt.getOrElse(false),
          ilmanHyvaksyntaa.getOrElse(false),
          vastaanottaneet.getOrElse(false),
          hakukohdeOids.getOrElse(List()).map(_.toString).asJava,
          toInteger(count),
          toInteger(index),
          new java.util.ArrayList(hakijat.asJava)
        )
      }
    }

  private def toInteger(x: Option[Int]): java.lang.Integer =
    x match {
  ...
  override def getSijoitteluAjo(sijoitteluajoId: Long): Option[SijoitteluAjo] =
    repository
      ...
      .map(_.entity(repository.getSijoitteluajonHakukohdeOidit(sijoitteluajoId)))

  override def hakemuksetVainHakukohteenTietojenKanssa(
    sijoitteluAjo: SijoitteluAjo,
    hakukohdeOid: HakukohdeOid
  ): List[KevytHakijaDTO] =
    tryOrThrow(SijoitteluajonHakijat.kevytDtoVainHakukohde(repository, sijoitteluAjo, hakukohdeOid))

  override def kevytHakemukset(
    sijoitteluAjo: SijoitteluAjo,
    hakukohdeOid: HakukohdeOid
  ): List[KevytHakijaDTO] =
  ...
  private def konvertoiHakijat(
    hyvaksytyt: Boolean,
    ilmanHyvaksyntaa: Boolean,
    vastaanottaneet: Boolean,
    hakukohdeOids: java.util.List[String],
    count: Integer,
    index: Integer,
    hakijat: java.util.ArrayList[HakijaDTO]
  ): HakijaPaginationObject = {
    ...
    def filter(hakija: HakijaDTO): Boolean = {
      ...
      for (hakutoiveDTO <- hakija.getHakutoiveet) {
        ...
        if (hakutoiveDTO.getVastaanottotieto eq ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI)
          isVastaanottanut = true
        import scala.collection.JavaConversions._
        for (valintatapajono <- hakutoiveDTO.getHakutoiveenValintatapajonot) {
          if (
            (valintatapajono.getTila eq HakemuksenTila.HYVAKSYTTY) || (valintatapajono.getTila eq VARASIJALTA_HYVAKSYTTY)
          ) isHyvaksytty = true
        }
      }
    ...
    def applyPagination(
      result: java.util.List[HakijaDTO],
      count: Integer,
      index: Integer
    ): java.util.List[HakijaDTO] = {
