...
class VastaanottoService(
  ...
  def vastaanotaVirkailijana(
    vastaanotot: List[VastaanottoEventDto]
  ): Iterable[VastaanottoResult] = {
    vastaanotot.groupBy(v => (v.hakukohdeOid, v.hakuOid)).flatMap {
      case ((hakukohdeOid, hakuOid), vastaanottoEventDtos) =>
        tallennaVirkailijanHakukohteenVastaanotot(hakukohdeOid, hakuOid, vastaanottoEventDtos)
    }
  }
  ...
  def vastaanotaVirkailijanaInTransaction(vastaanotot: List[VastaanottoEventDto]): Try[Unit] = {
    ...
    logger.info(
      s"Tallennettavat vastaanotot (${tallennettavatVastaanotot.size} kpl): " + tallennettavatVastaanotot
    )
    (for {
      _ <-
        MonadHelper
          .sequence(tallennettavatVastaanotot.map(v => findHakutoive(v.hakemusOid, v.hakukohdeOid)))
          .right
      hakukohdeRecords <-
        hakukohdeRecordService
          .getHakukohdeRecords(tallennettavatVastaanotot.map(_.hakukohdeOid).distinct)
          .right
      _ <- {
        val hakukohdeRecordsByOid = hakukohdeRecords.map(h => h.oid -> h).toMap
        val postCondition = DBIO.sequence(
          tallennettavatVastaanotot
            .filter(v => v.action == VastaanotaEhdollisesti || v.action == VastaanotaSitovasti)
            .map(v =>
              hakukohdeRecordsByOid(v.hakukohdeOid) match {
                case YPSHakukohde(_, _, koulutuksenAlkamiskausi) =>
                  hakijaVastaanottoRepository.findYhdenPaikanSaannonPiirissaOlevatVastaanotot(
                    v.henkiloOid,
                    koulutuksenAlkamiskausi
                  )
                case _ =>
                  hakijaVastaanottoRepository
                    .findHenkilonVastaanottoHakukohteeseen(v.henkiloOid, v.hakukohdeOid)
              }
            )
        )
  ...
  private def generateTallennettavatVastaanototList(
    vastaanotot: List[VastaanottoEventDto]
  ): List[VirkailijanVastaanotto] = {
    val hakuOidit = vastaanotot.map(_.hakuOid).toSet
    logger.info(
      s"Ollaan tallentamassa ${vastaanotot.size} vastaanottoa, joista löytyi ${hakuOidit.size} eri hakuOidia ($hakuOidit)."
    )
    if (hakuOidit.size > 1) {
    ...
    (for {
      ((hakukohdeOid, hakuOid), vastaanottoEventDtos) <-
        vastaanotot.groupBy(v => (v.hakukohdeOid, v.hakuOid))
      haunValintatulokset = henkiloidenVastaanototHauissaByHakuOid(hakuOid)
      hakukohteenValintatulokset: Map[String, Option[Valintatulos]] =
        haunValintatulokset.mapValues(_.find(_.getHakukohdeOid == hakukohdeOid.toString))
      vastaanottoEventDto <- vastaanottoEventDtos if isPaivitys(
        vastaanottoEventDto,
        hakukohteenValintatulokset.get(vastaanottoEventDto.henkiloOid).flatten.map(_.getTila)
      )
    } yield {
  ...
  private def tallennaVirkailijanHakukohteenVastaanotot(
    hakukohdeOid: HakukohdeOid,
    hakuOid: HakuOid,
    uudetVastaanotot: List[VastaanottoEventDto]
  ): List[VastaanottoResult] = {
    ...
    uudetVastaanotot.map(vastaanottoDto => {
      ...
      (for {
        ...
        hakutoive <-
          hakijaVastaanottoRepository
            .runAsSerialized(
              ...
              s"Storing vastaanotto $vastaanottoDto",
              for {
                sijoittelunTulos <-
                  sijoittelutulosService
                    .tulosVirkailijana(hakuOid, hakemusOid, henkiloOid, ohjausparametrit)
                aiemmatVastaanotot <- aiemmatVastaanotot(hakukohteet, henkiloOid)
                ilmoittautumisenAikaleimat <-
                  valinnantulosRepository.getIlmoittautumisenAikaleimat(henkiloOid)
                hakemuksenTulos = valintatulosService.julkaistavaTulos(
                  ...
                  vastaanottoKaudella =
                    oid => { aiemmatVastaanotot.get(oid).map(t => t._1 -> t._2.isDefined) },
                  ilmoittautumisenAikaleimat,
                  hasHetu = hakemus.henkilotiedot.hasHetu
                )(hakemus)
                hakutoive <- tarkistaHakutoiveenVastaanotettavuusVirkailijana(
                  hakemuksenTulos,
                  hakukohdeOid,
                  vastaanottoDto,
                  aiemmatVastaanotot.get(hakukohdeOid).flatMap(_._2)
                ).fold(DBIO.failed, DBIO.successful)
                _ <- hakutoive.fold[DBIO[Unit]](DBIO.successful())(_ =>
                  hakijaVastaanottoRepository.storeAction(vastaanotto)
                )
              } yield hakutoive
            )
            .right
        ...
        case Left(e @ (_: IllegalArgumentException | _: IllegalStateException)) =>
          createVastaanottoResult(400, Some(e), vastaanotto)
        case Left(e) => createVastaanottoResult(500, Some(e), vastaanotto)
      }
    })
  }

  private def isPaivitys(
    virkailijanVastaanotto: VastaanottoEventDto,
    valintatuloksenTila: Option[ValintatuloksenTila]
  ): Boolean =
  ...
  private def existingTilaMustBeUpdated(
    currentState: ValintatuloksenTila,
    newStateFromVirkailijanVastaanotto: Vastaanottotila
  ): Boolean = {
    if (
      newStateFromVirkailijanVastaanotto == Vastaanottotila.ottanut_vastaan_toisen_paikan || Vastaanottotila
        .matches(newStateFromVirkailijanVastaanotto, currentState)
    ) {
      return false
    }
    if (
      newStateFromVirkailijanVastaanotto == Vastaanottotila.kesken && currentState == ValintatuloksenTila.OTTANUT_VASTAAN_TOISEN_PAIKAN
    ) {
  ...
  private def createVastaanottoResult(
    statusCode: Int,
    ...
    VastaanottoResult(
      ...
      Result(statusCode, exception.map(_.getMessage))
    )
  }

  @Deprecated
  def tarkistaVastaanotettavuus(
    vastaanotettavaHakemusOid: HakemusOid,
    hakukohdeOid: HakukohdeOid
  ): Unit = {
    ...
    for {
      ...
      hakukohde <-
        hakukohdes
          ...
          .toRight(new RuntimeException(s"Hakukohde $hakukohdeOid not in hakemus $hakemusOid"))
          .right
        ...
        hakijaVastaanottoRepository
          .runAsSerialized(
            ...
            for {
              sijoittelunTulos <-
                sijoittelutulosService
                  .tulosHakijana(haku.oid, hakemusOid, hakemus.henkiloOid, ohjausparametrit)
              aiemmatVastaanotot <- aiemmatVastaanotot(hakukohdes, hakemus.henkiloOid)
              ilmoittautumisenAikaleimat <-
                valinnantulosRepository.getIlmoittautumisenAikaleimat(hakemus.henkiloOid)
              hakemuksenTulos = valintatulosService.julkaistavaTulos(
                sijoittelunTulos,
                haku,
                ohjausparametrit,
              ...
              _ <- tarkistaHakutoiveenVastaanotettavuus(hakemuksenTulos, hakukohdeOid, action)
                .fold(DBIO.failed, DBIO.successful)
              _ <- hakijaVastaanottoRepository.storeAction(
                HakijanVastaanotto(hakemuksenTulos.hakijaOid, hakemusOid, hakukohdeOid, action)
              )
            } yield ()
          )
          .right
    ...
    for {
      hakemus <-
        valintatulosService
          ...
          .toRight(new IllegalArgumentException(s"Hakemusta $hakemusOid ei löydy"))
          .right
      _ <-
        hakemus
          .findHakutoive(hakukohdeOid)
          .toRight(
            new IllegalArgumentException(
              s"Hakutoivetta $hakukohdeOid ei löydy hakemukselta $hakemusOid"
            )
          )
          .right
    } yield ()
  ...
  private def aiemmatVastaanotot(
    hakukohteet: Seq[HakukohdeRecord],
    henkiloOid: String
  ): DBIO[Map[HakukohdeOid, (Kausi, Option[VastaanottoRecord])]] = {
    DBIO
      .sequence(
        hakukohteet
          ...
          .map(kausi =>
            hakijaVastaanottoRepository
              .findYhdenPaikanSaannonPiirissaOlevatVastaanotot(henkiloOid, kausi)
              .map(kausi -> _)
          )
      )
      .map(_.toMap)
  ...
  private def tarkistaHakijakohtainenDeadline(
    hakutoive: Hakutoiveentulos
  ): Either[Throwable, Unit] =
    hakutoive.vastaanottoDeadline match {
  ...
  private def tarkistaHakutoiveenVastaanotettavuus(
    hakutoive: Hakutoiveentulos,
    haluttuTila: VastaanottoAction
  ): Either[Throwable, Unit] = {
    val e = new IllegalArgumentException(
      s"Väärä vastaanotettavuustila kohteella ${hakutoive.hakukohdeOid}: ${hakutoive.vastaanotettavuustila.toString} (yritetty muutos: $haluttuTila)"
    )
    haluttuTila match {
      case Peru | VastaanotaSitovasti
          if !Vastaanotettavuustila.isVastaanotettavissa(hakutoive.vastaanotettavuustila) =>
        Left(e)
      case VastaanotaEhdollisesti
          if hakutoive.vastaanotettavuustila != Vastaanotettavuustila.vastaanotettavissa_ehdollisesti =>
        Left(e)
      case _ => Right(())
    }
  }
  ...
  private def tarkistaHakutoiveenVastaanotettavuus(
    ...
    hakukohdeOid: HakukohdeOid,
    haluttuTila: VastaanottoAction
  ): Either[Throwable, Hakutoiveentulos] = {
    val missingHakutoive =
      s"Hakutoivetta $hakukohdeOid ei löydy hakemukselta ${hakemuksenTulos.hakemusOid}"
    for {
      hakutoive <-
        hakemuksenTulos
          .findHakutoive(hakukohdeOid)
          .toRight(new IllegalArgumentException(missingHakutoive))
          .right
      _ <- tarkistaHakutoiveenVastaanotettavuus(hakutoive._1, haluttuTila).right
    } yield hakutoive._1
  }

  private def tarkistaHakutoiveenVastaanotettavuusVirkailijana(
    vastaanotto: VirkailijanVastaanotto,
    hakutoive: Hakutoiveentulos,
    maybeAiempiVastaanottoKaudella: Option[VastaanottoRecord]
  ): Either[Throwable, Unit] =
    vastaanotto.action match {
      ...
      case VastaanotaSitovasti if !Valintatila.hasBeenHyväksytty(hakutoive.valintatila) =>
        logger.warn(
          s"Could not save $VastaanotaSitovasti because state was ${hakutoive.valintatila} in $vastaanotto"
        )
        Left(
          new IllegalArgumentException(
            s"""Ei voi tallentaa vastaanottotietoa, koska hakijalle näytettävä tila on "${hakutoive.valintatila}""""
          )
        )
  ...
  private def tarkistaHakutoiveenVastaanotettavuusVirkailijana(
    ...
    maybeAiempiVastaanottoKaudella: Option[VastaanottoRecord]
  ): Either[Throwable, Option[(Hakutoiveentulos, Int)]] = {
    val missingHakutoive =
      s"Hakutoivetta $hakukohdeOid ei löydy hakemukselta ${hakemuksentulos.hakemusOid}"
    for {
      hakutoiveJaJarjestysnumero <-
        hakemuksentulos
          .findHakutoive(hakukohdeOid)
          .toRight(new IllegalArgumentException(missingHakutoive))
          .right
      r <-
        if (
          isPaivitys(
            ...
            Some(
              ValintatuloksenTila
                .valueOf(hakutoiveJaJarjestysnumero._1.virkailijanTilat.vastaanottotila.toString)
            )
          )
        ) {
          ...
          logger.info(
            s"Vastaanotto event $vastaanottoDto is not an update to hakutoive ${hakutoiveJaJarjestysnumero._1}"
          )
          Right(None).right
        }
    } yield r
