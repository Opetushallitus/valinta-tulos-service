...
class ValinnantulosService(
  ...
  def getMuutoshistoriaForHakemusWithoutAuditInfo(
  ...
  def getMuutoshistoriaForHakemus(
    hakemusOid: HakemusOid,
    valintatapajonoOid: ValintatapajonoOid,
    auditInfo: AuditInfo
  ): List[Muutos] = {
  ...
  def getValinnantuloksetForHakukohde(
    hakukohdeOid: HakukohdeOid,
    auditInfo: AuditInfo
  ): Option[(Instant, Set[Valinnantulos])] = {
    val r = valinnantulosRepository
      .getValinnantuloksetAndLastModifiedDateForHakukohde(hakukohdeOid)
      .map(t => {
  ...
  def getValinnantuloksetForValintatapajono(
    valintatapajonoOid: ValintatapajonoOid,
    auditInfo: AuditInfo
  ): Option[(Instant, Set[Valinnantulos])] = {
    val r = valinnantulosRepository
      .getValinnantuloksetAndLastModifiedDateForValintatapajono(valintatapajonoOid)
      .map(t => {
        (t._1, yhdenPaikanSaannos(t._2).fold(throw _, x => x))
      })
    audit.log(
  ...
  def getValinnantuloksetForHakemus(
    hakemusOid: HakemusOid,
    auditInfo: AuditInfo
  ): Option[(Instant, Set[ValinnantulosWithTilahistoria])] = {
    val r = valinnantulosRepository
      .getValinnantuloksetAndLastModifiedDateForHakemus(hakemusOid)
      .map(t => {
        (t._1, yhdenPaikanSaannos(t._2).fold(throw _, x => x))
      })
    r.foreach(t => {
      val oids = hakuService
        .getHakukohdes(t._2.map(_.hakukohdeOid).toSeq)
        .fold(throw _, x => x)
      ...
      (
        ...
        t._2.map(tulos =>
          ValinnantulosWithTilahistoria(
            tulos,
            valinnantulosRepository.getHakemuksenTilahistoriat(
              tulos.hakemusOid,
  ...
  def getValinnantuloksetForHakemukset(
    hakemusOids: Set[HakemusOid],
    auditInfo: AuditInfo
  ): Set[ValinnantulosWithTilahistoria] = {
    val tulokset: Seq[Valinnantulos] = valinnantulosRepository
      .getValinnantuloksetForHakemukses(hakemusOids)
    val hakukohteet =
      Timer.timed(s"${hakemusOids.size} hakemuksen tuloksiin liittyvien hakukohteiden haku") {
        hakuService.getHakukohdes(tulokset.map(_.hakukohdeOid).distinct).fold(throw _, x => x)
      }
    val tuloksetJaHakukohteet = tulokset.map(t =>
      (
        t,
        hakukohteet
          .find(hk => hk.oid == t.hakukohdeOid)
    ...
    logger.info(
      s"${tulokset.size} valinnantuloksesta ${yps_ja_ilman._1.size} kohdistuu yhden paikan sääntöä käyttäviin hakukohteisiin ja ${yps_ja_ilman._2.size} muihin."
    )
    val tuloksetIlmanHistoriatietoa = yhdenPaikanSaannos
      .getYpsTuloksetForManyHakemukses(yps_ja_ilman._1)
      .fold(throw _, x => x) ++ yps_ja_ilman._2.map(t => t._1)
    val tilaHistoriat = valinnantulosRepository
      .getHakemustenTilahistoriat(hakemusOids)
      .groupBy(r => (r.hakemusOid, r.valintatapajonoOid))
    tuloksetIlmanHistoriatietoa.map(tulos =>
      ValinnantulosWithTilahistoria(
        tulos,
        tilaHistoriat.getOrElse((tulos.hakemusOid, tulos.valintatapajonoOid), List.empty)
      )
    )
  }
  ...
  def storeValinnantuloksetAndIlmoittautumiset(
    valintatapajonoOid: ValintatapajonoOid,
    valinnantulokset: List[Valinnantulos],
    ifUnmodifiedSince: Option[Instant],
    auditInfo: AuditInfo
  ): List[ValinnantulosUpdateStatus] = {
    val hakukohdeOid =
      valinnantulokset.head.hakukohdeOid // FIXME käyttäjän syötettä, tarvittaisiin jono-hakukohde tieto valintaperusteista
    (for {
      ...
      _ <-
        authorizer
          .checkAccess(
            ...
            Set(Role.SIJOITTELU_READ_UPDATE, Role.SIJOITTELU_CRUD) ++ (if (erillishaku) {
                                                                         Set(
                                                                           Role.ATARU_KEVYT_VALINTA_CRUD
                                                                         )
                                                                       } else {
      ...
      val strategy = if (erillishaku) {
        ...
        new SijoittelunValinnantulosStrategy(
          ...
          appConfig,
          valinnantulosRepository,
          ifUnmodifiedSince.getOrElse(
            throw new IllegalArgumentException(
              appConfig.settings.headerIfUnmodifiedSince + " on pakollinen otsake valinnantulosten tallennukselle"
            )
          ),
          audit
        )
  ...
  private def validateAndSaveValinnantuloksetInTransaction(
    valintatapajonoOid: ValintatapajonoOid,
    hakukohde: Hakukohde,
    s: ValinnantulosStrategy,
    valinnantulokset: List[Valinnantulos]
  ): List[ValinnantulosUpdateStatus] = {

    def vanhatValinnantuloksetYhdenPaikanSaannolla(): DBIO[Set[Valinnantulos]] = {
      valinnantulosRepository
        .getValinnantuloksetForValintatapajonoDBIO(valintatapajonoOid)
        .flatMap(vanhatValinnantulokset => {
          yhdenPaikanSaannos.ottanutVastaanToisenPaikanDBIO(hakukohde, vanhatValinnantulokset)
        })
    }

    valinnantulosRepository.runBlockingTransactionally(
      vanhatValinnantuloksetYhdenPaikanSaannolla().flatMap(vanhatValinnantulokset => {
        DBIO.sequence(
          valinnantulokset.map(uusi =>
            validateAndSaveValinnantulos(
              uusi,
              vanhatValinnantulokset.find(_.hakemusOid == uusi.hakemusOid),
              s
            )
          )
        )
      ...
      case Left(t) => {
        logger.error(
          s"""Kaikkien valinnantulosten tallennus valintatapajonolle ${valintatapajonoOid} epäonnistui""",
          t
        )
        throw t
      }
      case Right(valinnantulosErrorStatuses) =>
        valinnantulosErrorStatuses.map(_.left.toOption) flatten
    }
  }

  private def validateAndSaveValinnantulos(
    uusi: Valinnantulos,
  ...
  private def saveValinnantulos(
    uusi: Valinnantulos,
    vanha: Option[Valinnantulos],
    s: ValinnantulosStrategy
  ): DBIO[Either[ValinnantulosUpdateStatus, Unit]] = {
    s.save(uusi, vanha)
      .asTry
          ...
          DBIO.successful(
            Left(
              ValinnantulosUpdateStatus(
                409,
                "Hakemus on muuttunut lukemisen jälkeen",
        ...
        case Failure(t) => {
          ...
          DBIO.successful(
            Left(
              ValinnantulosUpdateStatus(
                500,
                s"Valinnantuloksen tallennus epäonnistui",
                uusi.valintatapajonoOid,
                uusi.hakemusOid
              )
            )
