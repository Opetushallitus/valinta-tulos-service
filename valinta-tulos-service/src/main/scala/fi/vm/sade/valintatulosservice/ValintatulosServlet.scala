...
import fi.vm.sade.valintatulosservice.json.{
  JsonFormats,
  JsonStreamWriter,
  StreamingFailureException
}
...
abstract class ValintatulosServlet(
    ...
    List(
      Hakutoiveentulos.julkaistavaVersioSijoittelunTuloksesta(
        ...
        Haku(
          ...
          sisältyvätHaut = Set(),
          hakuAjat = List(
            Hakuaika("12345", Some(System.currentTimeMillis()), Some(System.currentTimeMillis()))
          ),
        ...
        Ohjausparametrit(
          ...
          Some(DateTime.now().plusDays(10)),
          Some(DateTime.now().plusDays(30)),
          Some(DateTime.now().plusDays(60)),
          None,
          None,
          None,
          true
  ...
  get("/:hakuOid", operation(getHakemuksetSwagger)) {
    ...
    serveStreamingResults({
      valintatulosService.hakemustenTulosByHaku(HakuOid(hakuOidString), false)
    })
  }
  ...
  get("/:hakuOid/hakukohde/:hakukohdeOid", operation(getHakukohteenHakemuksetSwagger)) {
    ...
    serveStreamingResults({
      valintatulosService
        .hakemustenTulosByHakukohde(HakuOid(hakuOidString), HakukohdeOid(hakukohdeOidString))
        .right
        .toOption
    })
  }

  lazy val getHakukohteenHakemuksetSwagger: OperationBuilder =
    (apiOperation[Unit]("getHakukohteenHakemukset")
  ...
  lazy val getHakukohteenVastaanotettavuusSwagger: OperationBuilder =
    (apiOperation[Unit]("getHakukohteenHakemukset")
      ...
      tags swaggerGroupTag)
  get(
    "/:hakuOid/hakemus/:hakemusOid/hakukohde/:hakukohdeOid/vastaanotettavuus",
    operation(getHakukohteenVastaanotettavuusSwagger)
  ) {
    val hakemusOidString = params("hakemusOid")
    val hakukohdeOidString = params("hakukohdeOid")
    auditLog(
      Map("hakemusOid" -> hakemusOidString, "hakukohdeOid" -> hakukohdeOidString),
      HakemuksenLuku
    )
    Try(
      vastaanottoService.tarkistaVastaanotettavuus(
        HakemusOid(hakemusOidString),
        HakukohdeOid(hakukohdeOidString)
      )
    )
      .map((_) => Ok())
  ...
  // Real body param type cannot be used because of unsupported scala enumerations: https://github.com/scalatra/scalatra/issues/343
    notes "Bodyssä tulee antaa tieto hakukohteen ilmoittautumistilan muutoksesta Ilmoittautuminen tyyppinä. Esim:\n" +
      ...
      ) + ".\nMahdolliset ilmoittautumistilat: " + IlmoittautumisTila
      .values()
      .toList
      .map(_.toString)

    parameter pathParam[String]("hakuOid").description("Haun oid")
    parameter pathParam[String]("hakemusOid").description(
      "Hakemuksen oid, jonka vastaanottotilaa ollaan muokkaamassa"
    )
    tags swaggerGroupTag)
  post("/:hakuOid/hakemus/:hakemusOid/ilmoittaudu", operation(postIlmoittautuminenSwagger)) {
    ...
    val auditParams: Map[String, String] =
      Map("hakuOid" -> params("hakuOid"), "hakemusOid" -> hakemusOidString)
    val addedParams: Map[String, String] =
      Map("ilmoittautumisTila" -> ilmoittautuminen.tila.toString)
  ...
  lazy val getHaunSijoitteluajonTuloksetSwagger: OperationBuilder =
    (apiOperation[Unit]("getHaunSijoitteluajonTuloksetSwagger")
      ...
      parameter pathParam[String]("sijoitteluajoId")
        .description("""Sijoitteluajon id tai "latest"""")
        .required
      parameter queryParam[Boolean]("hyvaksytyt")
        .description("Listaa jossakin kohteessa hyvaksytyt")
        .optional
      parameter queryParam[Boolean]("ilmanHyvaksyntaa")
        .description("Listaa henkilot jotka ovat taysin ilman hyvaksyntaa (missaan kohteessa)")
        .optional
      parameter queryParam[Boolean]("vastaanottaneet")
        .description("Listaa henkilot jotka ovat ottaneet paikan vastaan")
        .optional
      parameter queryParam[List[String]]("hakukohdeOid")
        .description(
          "Rajoita hakua niin etta naytetaan hakijat jotka ovat jollain toiveella hakeneet naihin kohteisiin"
        )
        .optional
      parameter queryParam[Int]("count")
        .description("Nayta n kappaletta tuloksia. Kayta sivutuksessa")
        .optional
      parameter queryParam[Int]("index")
        .description("Aloita nayttaminen kohdasta n. Kayta sivutuksessa.")
        .optional
      tags swaggerGroupTag)
  get(
    "/:hakuOid/sijoitteluajo/:sijoitteluajoId/hakemukset",
    operation(getHaunSijoitteluajonTuloksetSwagger)
  ) {
    ...
    val hakijaPaginationObject = valintatulosService.sijoittelunTulokset(
      hakuOid,
      sijoitteluajoId,
      hyvaksytyt,
      ilmanHyvaksyntaa,
  ...
  lazy val getHakukohteenKaikkiHakijatSwagger: OperationBuilder =
    (apiOperation[Unit]("getHakukohteenKaikkiHakijatSwagger")
  ...
  get("/:hakuOid/hakukohde/:hakukohdeOid/hakijat", operation(getHakukohteenKaikkiHakijatSwagger)) {
    ...
    auditLog(
      Map("hakuOid" -> hakuOidString, "hakukohdeOid" -> hakukohdeOidString),
      HakutietojenLuku
    )
    Ok(JsonFormats.javaObjectToJsonString(hakijaPaginationObject))
  }

  lazy val getHaunIlmanHyvaksyntaaSwagger: OperationBuilder =
    (apiOperation[Unit]("getHaunIlmanHyvaksyntaaSwagger")
  ...
  lazy val getHaunHyvaksytytSwagger: OperationBuilder =
    (apiOperation[Unit]("getHaunHyvaksytytSwagger")
  ...
  lazy val getHakukohteenHyvaksytytSwagger: OperationBuilder =
    (apiOperation[Unit]("getHakukohteenHyvaksytytSwagger")
  ...
  lazy val getHakemuksenSijoitteluajonTulosSwagger: OperationBuilder =
    (apiOperation[Unit]("getHakemuksenSijoitteluajonTulosSwagger")
      ...
      tags swaggerGroupTag)
  get(
    "/:hakuOid/sijoitteluajo/:sijoitteluajoId/hakemus/:hakemusOid",
    operation(getHakemuksenSijoitteluajonTulosSwagger)
  ) {
    val hakuOidString = params("hakuOid")
    ...
    auditLog(
      Map(
        "hakuOid" -> hakuOidString,
        "sijoitteluajoId" -> sijoitteluajoIdString,
        "hakemusOid" -> hakemusOidString
      ),
      HakemuksenLuku
    )
    valintatulosService.sijoittelunTulosHakemukselle(
      HakuOid(hakuOidString),
      sijoitteluajoIdString,
      HakemusOid(hakemusOidString)
    ) match {
  ...
  lazy val getStreamingHaunSijoitteluajonTuloksetSwagger: OperationBuilder =
    (apiOperation[Unit]("getStreamingHaunSijoitteluajonTuloksetSwagger")
      ...
      parameter pathParam[String]("sijoitteluajoId")
        .description("""Sijoitteluajon id tai "latest"""")
        .required
      parameter queryParam[Boolean]("vainMerkitsevaJono")
        .description("Jos true, palautetaan vain merkitsevän valintatapajonon tiedot")
        .optional
      tags swaggerGroupTag)
  get(
    "/streaming/:hakuOid/sijoitteluajo/:sijoitteluajoId/hakemukset",
    operation(getStreamingHaunSijoitteluajonTuloksetSwagger)
  ) {
    val hakuOid = HakuOid(params("hakuOid"))
    val sijoitteluajoId = params("sijoitteluajoId")
    ...
    writeSijoittelunTuloksetStreamingToResponse(
      ...
      hakuOid,
      w =>
        streamingValintatulosService.streamSijoittelunTuloksetOfWholeHaku(
          hakuOid,
          sijoitteluajoId,
          w,
          vainMerkitsevaJono
  ...
  lazy val postStreamingHaunSijoitteluajonHakukohteidenTuloksetSwagger: OperationBuilder =
    (apiOperation[Unit]("postStreamingHaunSijoitteluajonHakukohteidenTuloksetSwagger")
      ...
      parameter queryParam[Boolean]("vainMerkitsevaJono")
        .description("Jos true, palautetaan vain merkitsevän valintatapajonon tiedot")
        .optional
      parameter bodyParam[Seq[String]]("hakukohdeOidit")
        .description("Hakukohteet, joiden tulokset halutaan")
        .required
      tags swaggerGroupTag)
  post(
    "/streaming/:hakuOid/sijoitteluajo/:sijoitteluajoId/hakemukset",
    operation(postStreamingHaunSijoitteluajonHakukohteidenTuloksetSwagger)
  ) {
    val hakuOid = HakuOid(params("hakuOid"))
    val sijoitteluajoId = params("sijoitteluajoId")
    ...
    if (hakukohdeOids.isEmpty) {
      ...
      writeSijoittelunTuloksetStreamingToResponse(
        ...
        hakuOid,
        w =>
          streamingValintatulosService.streamSijoittelunTuloksetOfHakukohdes(
            hakuOid,
            sijoitteluajoId,
            hakukohdeOids.toSet.map(HakukohdeOid),
            w,
            vainMerkitsevaJono
          )
      )
  ...
  private def writeSijoittelunTuloksetStreamingToResponse(
    response: HttpServletResponse,
    hakuOid: HakuOid,
    writeWholeResultsToResponse: (HakijaDTO => Unit) => Unit
  ): Unit = {
    ...
    try {
      ...
      case t: Throwable =>
        throw new StreamingFailureException(t, s""", {"error": "${t.getMessage}"}] """)
    }
  ...
  def auditLogChanged(
    ...
    auditParamsAdded: Map[String, String],
