...
class ValintaesitysServlet(
  ...
  val valintaesitysSwagger: OperationBuilder =
    (apiOperation[List[Valintaesitys]]("valintaesityksien haku")
  ...
  val valintaesityksenHyvaksyntaSwagger: OperationBuilder =
    (apiOperation[Valintaesitys]("valintaesityksen hyväksyntä")
  ...
  post("/:valintatapajonoOid/hyvaksytty", operation(valintaesityksenHyvaksyntaSwagger)) {
    ...
    Ok(
      valintaesitysService.hyvaksyValintaesitys(
        parseValintatapajonoOid.fold(throw _, x => x),
        auditInfo
      )
    )
  }
