...
class SijoittelutulosService(
  ...
  private def tulos(
    ...
    for {
      ...
      hyvaksyttyJulkaistuDates <-
        valintarekisteriDb.findHyvaksyttyJulkaistuDatesForHenkilo(henkiloOid)
      ...
      hakutoiveetSijoittelussa <- latestSijoitteluajoId.fold(
        DBIO.successful(List.empty[HakutoiveRecord])
      )(valintarekisteriDb.getHakemuksenHakutoiveetSijoittelussa(hakemusOid, _))
      valintatapajonotSijoittelussa <- latestSijoitteluajoId.fold(
        DBIO.successful(List.empty[HakutoiveenValintatapajonoRecord])
      )(valintarekisteriDb.getHakemuksenHakutoiveidenValintatapajonotSijoittelussa(hakemusOid, _))
    } yield HakemuksenSijoitteluntulos(
      hakemusOid,
      Some(henkiloOid),
      valinnantulokset
        .map(_.hakukohdeOid)
        .union(hakutoiveetSijoittelussa.map(_.hakukohdeOid).toSet)
        .toList
        .map(oid =>
          (
  ...
  private def hakutoiveenSijoittelunTulos(
    ohjausparametrit: Ohjausparametrit,
    vastaanotettavuusVirkailijana: Boolean,
    ...
    val jonot = valinnantulokset
      .map(_.valintatapajonoOid)
      .union(valintatapajonot.map(_.valintatapajonoOid).toSet)
      .toList
      .map(oid =>
        (
          valinnantulokset.find(_.valintatapajonoOid == oid),
          valintatapajonot.find(_.valintatapajonoOid == oid)
        )
      )
    ...
    val (merkitsevaValinnantulos, merkitsevaValintatapajono) = jonot.minBy({
      case (valinnantulos, valintatapajono) =>
        (
          ...
          valinnantulos
            .filter(_.valinnantila == Varalla)
            .flatMap(_ => valintatapajono.flatMap(_.varasijanNumero)),
    ...
    val (hakijanTilat, vastaanottoDeadline) = tilatietoJaVastaanottoDeadline(
      valintatila,
      vastaanotto,
      ohjausparametrit,
      hakutoiveenHyvaksyttyJaJulkaistuDate,
    ...
    val (virkailijanTilat, _) = tilatietoJaVastaanottoDeadline(
      valintatila,
      vastaanotto,
      ohjausparametrit,
      hakutoiveenHyvaksyttyJaJulkaistuDate,
      vastaanotettavuusVirkailijana = true
    )

    val hyväksyttyJulkaistussaJonossa = Valintatila.isHyväksytty(
      valintatila
    ) && merkitsevaValinnantulos.flatMap(_.julkaistavissa).getOrElse(false)
    val julkaistavissa = hyväksyttyJulkaistussaJonossa || jonot.forall({
      case (valinnantulos, _) => valinnantulos.flatMap(_.julkaistavissa).getOrElse(false)
    })
    ...
    HakutoiveenSijoitteluntulos(
      ...
      merkitsevaValinnantulos
        .map(_.valintatapajonoOid)
        .orElse(merkitsevaValintatapajono.map(_.valintatapajonoOid))
        .get,
      ...
      viimeisinHakemuksenTilanMuutos = merkitsevaValinnantulos
        .flatMap(_.valinnantilanViimeisinMuutos)
        .map(odt => Date.from(odt.toInstant)),
      viimeisinValintatuloksenMuutos = merkitsevaValinnantulos
        .flatMap(_.vastaanotonViimeisinMuutos)
        .map(odt => Date.from(odt.toInstant)),
      ...
      ehdollisestiHyvaksyttavissa =
        merkitsevaValinnantulos.flatMap(_.ehdollisestiHyvaksyttavissa).getOrElse(false),
      ehdollisenHyvaksymisenEhtoKoodi =
        merkitsevaValinnantulos.flatMap(_.ehdollisenHyvaksymisenEhtoKoodi),
      ehdollisenHyvaksymisenEhtoFI =
        merkitsevaValinnantulos.flatMap(_.ehdollisenHyvaksymisenEhtoFI),
      ehdollisenHyvaksymisenEhtoSV =
        merkitsevaValinnantulos.flatMap(_.ehdollisenHyvaksymisenEhtoSV),
      ehdollisenHyvaksymisenEhtoEN =
        merkitsevaValinnantulos.flatMap(_.ehdollisenHyvaksymisenEhtoEN),
      tilankuvaukset,
      pisteet,
      jonokohtaisetTulostiedot = jonot.map({
        case (valinnantulos, valintatapajono) =>
  ...
  private def jonokohtainenTulostieto(
    merkitsevaValinnantulos: Option[Valinnantulos],
    ...
    JonokohtainenTulostieto(
      oid = valinnantulos
        .map(_.valintatapajonoOid)
        .orElse(valintatapajono.map(_.valintatapajonoOid))
        .get,
      ...
      ehdollisestiHyvaksyttavissa =
        valinnantulos.flatMap(_.ehdollisestiHyvaksyttavissa).getOrElse(false),
      ehdollisenHyvaksymisenEhto = Some(
      ...
      varasijasaannotKaytossa =
        valintatapajono.exists(_.sijoiteltuIlmanVarasijasaantojaNiidenOllessaVoimassa)
    )
  }
  ...
  def hakemustenTulos(
    ...
    vainHakukohde: Boolean = false
  ): List[HakemuksenSijoitteluntulos] = {
    def fetchVastaanottos(
      hakemusOid: HakemusOid,
      hakijaOidFromSijoittelunTulos: Option[String]
    ): Set[VastaanottoRecord] =
      (
        hakijaOidFromSijoittelunTulos.orElse(personOidResolver.findBy(hakemusOid)),
        haunVastaanotot
      ) match {
    ...
    (for (
      ...
      hakijaDtot <- hakukohdeOid match {
        case Some(hakukohde) =>
          if (vainHakukohde)
            Option(
              Timer.timed("hakukohteen hakemukset", 1000)(
                raportointiService.hakemuksetVainHakukohteenTietojenKanssa(sijoittelu, hakukohde)
              )
            )
          else
            Option(
              Timer.timed("hakukohteen hakemukset", 1000)(
                raportointiService.kevytHakemukset(sijoittelu, hakukohde)
              )
            )
        case None =>
          Option(Timer.timed("hakemukset", 1000)(raportointiService.kevytHakemukset(sijoittelu)))
      };
      hakijat <- {
        val hyvaksyttyJaJulkaistuDates =
          valintarekisteriDb.findHyvaksyttyJulkaistuDatesForHaku(hakuOid)
        Option(
          hakijaDtot.map(h =>
            hakemuksenKevytYhteenveto(
              h,
              ohjausparametrit,
              hyvaksyttyJaJulkaistuDates.getOrElse(h.getHakijaOid, Map()),
  ...
  def findOhjausparametritFromOhjausparametritService(hakuOid: HakuOid): Ohjausparametrit = {
    Timer.timed(
      "findAikatauluFromOhjausparametritService -> ohjausparametritService.ohjausparametrit",
      100
    ) {
  ...
  def findLatestSijoitteluAjoForHaku(hakuOid: HakuOid): Option[SijoitteluAjo] = {
    Timer.timed(
      "findLatestSijoitteluAjoForHaku -> latestSijoitteluAjoClient.fetchLatestSijoitteluAjoFromSijoitteluService",
      100
    ) {
  ...
  def findLatestSijoitteluAjoWithoutHakukohdesForHaku(hakuOid: HakuOid): Option[SijoitteluAjo] = {
    Timer.timed(
      "findLatestSijoitteluAjoWithoutHakukohdesForHaku -> sijoittelunTulosClient.fetchLatestSijoitteluAjo(hakuOid)",
      100
    ) {
  ...
  def findLatestSijoitteluAjo(
    hakuOid: HakuOid,
    hakukohdeOid: Option[HakukohdeOid]
  ): Option[SijoitteluAjo] = {
    Timer.timed(
      s"findLatestSijoitteluAjo -> latestSijoitteluAjoClient.fetchLatestSijoitteluAjoFromSijoitteluService($hakuOid, $hakukohdeOid)",
      100
    ) {
  ...
  def sijoittelunTuloksetWithoutVastaanottoTieto(
    hakuOid: HakuOid,
  ...
  def sijoittelunTulosForAjoWithoutVastaanottoTieto(
  ...
  private def fetchVastaanotto(henkiloOid: String, hakuOid: HakuOid): Set[VastaanottoRecord] = {
    Timer.timed("hakijaVastaanottoRepository.findHenkilonVastaanototHaussa", 100) {
      valintarekisteriDb.runBlocking(
        valintarekisteriDb.findHenkilonVastaanototHaussa(henkiloOid, hakuOid)
      )
    }
  }

  private def hakemuksenKevytYhteenveto(
    ...
    val hakutoiveidenYhteenvedot = hakija.getHakutoiveet.toList.map {
      hakutoive: KevytHakutoiveDTO =>
        val vastaanotto =
          vastaanottoRecord.find(v => v.hakukohdeOid.toString == hakutoive.getHakukohdeOid)
        val jono = JonoFinder.merkitseväJono(hakutoive).getOrElse {
          throw new IllegalStateException(
            s"Ei löydy merkitsevää jonoa hakemuksen ${hakija.getHakemusOid} hakutoiveelle ${hakutoive.getHakukohdeOid} . " +
              s"Kaikki jonot (${hakutoive.getHakutoiveenValintatapajonot.size} kpl): ${hakutoive.getHakutoiveenValintatapajonot
                .map(ToStringBuilder.reflectionToString)}"
          )
        }
        val valintatila = jononValintatila(jono, hakutoive)

        val hakutoiveenHyvaksyttyJaJulkaistuDate =
          hyvaksyttyJulkaistuDates.get(HakukohdeOid(hakutoive.getHakukohdeOid))
        ...
        val hyväksyttyJulkaistussaJonossa =
          Valintatila.isHyväksytty(valintatila) && jono.isJulkaistavissa
        val julkaistavissa = hyväksyttyJulkaistussaJonossa || kaikkiJonotJulkaistu(hakutoive)

        val pisteet: Option[BigDecimal] =
          Option(jono.getPisteet).map((p: java.math.BigDecimal) => new BigDecimal(p))
        ...
        HakutoiveenSijoitteluntulos(
          ...
          SijoitteluajonIlmoittautumistila(
            Option(jono.getIlmoittautumisTila).getOrElse(IlmoittautumisTila.EI_TEHTY)
          ),
    ...
    HakemuksenSijoitteluntulos(
      ...
      Option(StringUtils.trimToNull(hakija.getHakijaOid)),
      hakutoiveidenYhteenvedot
    )
  }
  ...
  private def tilatietoJaVastaanottoDeadline(
    valintatila: Valintatila,
    vastaanotto: Option[VastaanottoRecord],
    ohjausparametrit: Ohjausparametrit,
    hakutoiveenHyvaksyttyJaJulkaistuDate: Option[OffsetDateTime],
    vastaanotettavuusVirkailijana: Boolean
  ): (HakutoiveenSijoittelunTilaTieto, Option[DateTime]) = {
    val (vastaanottotila, vastaanottoDeadline) = laskeVastaanottotila(
      valintatila,
      vastaanotto,
      ohjausparametrit,
      hakutoiveenHyvaksyttyJaJulkaistuDate,
    ...
    val uusiValintatila =
      vastaanottotilanVaikutusValintatilaan(valintatila, vastaanottotila, merkitsevaJono = true)
    val vastaanotettavuustila: Vastaanotettavuustila.Value =
      laskeVastaanotettavuustila(valintatila, vastaanottotila)
    (
      HakutoiveenSijoittelunTilaTieto(
        uusiValintatila,
        vastaanottotila,
        vastaanotto.map(v =>
          if (v.ilmoittaja == "järjestelmä") { Sijoittelu }
          else { Henkilo(v.ilmoittaja) }
        ),
  ...
  private def laskeVastaanotettavuustila(
    valintatila: Valintatila,
    vastaanottotila: Vastaanottotila
  ): Vastaanotettavuustila.Value = {
    if (
      Valintatila.isHyväksytty(valintatila) && Set(
        Vastaanottotila.kesken,
        Vastaanottotila.ehdollisesti_vastaanottanut
      ).contains(vastaanottotila)
    ) {
  ...
  private def hakemuksenTilastaJononValintatilaksi(
    valinnantulos: Option[Valinnantulos],
    valintatapajono: Option[HakutoiveenValintatapajonoRecord]
  ): Valintatila = {
    ...
    if (
      Valintatila
        .isHyväksytty(valintatila) && valintatapajono.exists(_.hyvaksyttyHarkinnanvaraisesti)
    ) {
  ...
  private def jononValintatila(
    jono: KevytHakutoiveenValintatapajonoDTO,
    hakutoive: KevytHakutoiveDTO
  ) = {
  ...
  def vastaanottotilaVainViimeisimmanVastaanottoActioninPerusteella(
    vastaanotto: Option[VastaanottoRecord]
  ): Vastaanottotila =
    vastaanotto.map(_.action) match {
  ...
  private def laskeVastaanottotila(
    valintatila: Valintatila,
    vastaanotto: Option[VastaanottoRecord],
    ohjausparametrit: Ohjausparametrit,
    hakutoiveenHyvaksyttyJaJulkaistuDate: Option[OffsetDateTime],
    vastaanotettavuusVirkailijana: Boolean = false
  ): (Vastaanottotila, Option[DateTime]) = {
    ...
    vastaanottotilaVainViimeisimmanVastaanottoActioninPerusteella(vastaanotto) match {
      case Vastaanottotila.kesken
          if Valintatila.isHyväksytty(valintatila) || valintatila == Valintatila.perunut =>
  ...
  def haeVastaanotonAikarajaTiedot(
    ...
    findLatestSijoitteluAjo(hakuOid, Some(hakukohdeOid)) match {
      ...
      case Some(sijoitteluAjo) =>
        val ohjausparametrit = findOhjausparametritFromOhjausparametritService(hakuOid)
        val hyvaksyttyJaJulkaistuDates =
          valintarekisteriDb.findHyvaksyttyJulkaistuDatesForHakukohde(hakukohdeOid)
        def queriedHakijasForHakukohde() = {
          val allHakijasForHakukohde = Timer.timed(
            s"Fetch hakemukset just for hakukohde $hakukohdeOid of haku $hakuOid",
            1000
          ) {
            raportointiService.hakemuksetVainHakukohteenTietojenKanssa(sijoitteluAjo, hakukohdeOid)
          }
          allHakijasForHakukohde.filter(hakijaDto =>
            hakemusOids.contains(HakemusOid(hakijaDto.getHakemusOid))
          )
        }

        def calculateLateness(hakijaDto: KevytHakijaDTO): VastaanottoAikarajaMennyt = {
          val hakutoiveDtoOfThisHakukohde: Option[KevytHakutoiveDTO] =
            hakijaDto.getHakutoiveet.toList.find(_.getHakukohdeOid == hakukohdeOid.toString)
          val vastaanottoDeadline: Option[DateTime] = hakutoiveDtoOfThisHakukohde.flatMap {
            hakutoive: KevytHakutoiveDTO =>
              laskeVastaanottoDeadline(
                ohjausparametrit,
                hyvaksyttyJaJulkaistuDates.get(hakijaDto.getHakijaOid)
              )
          }
          val isLate: Boolean = vastaanottoDeadline.exists(new DateTime().isAfter)
          VastaanottoAikarajaMennyt(
            HakemusOid(hakijaDto.getHakemusOid),
            isLate,
            vastaanottoDeadline
          )
        }
  ...
  private def vastaanottotilanVaikutusValintatilaan(
    valintatila: Valintatila,
    vastaanottotila: Vastaanottotila,
    merkitsevaJono: Boolean
  ): Valintatila = {
    if (
      List(Vastaanottotila.ehdollisesti_vastaanottanut, Vastaanottotila.vastaanottanut).contains(
        vastaanottotila
      ) && merkitsevaJono
    ) {
