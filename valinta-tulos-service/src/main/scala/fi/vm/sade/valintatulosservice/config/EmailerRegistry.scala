...
object EmailerRegistry extends Logging {
  ...
  def fromString(
    profile: String
  )(mailPoller: MailPoller, mailDecorator: MailDecorator): EmailerRegistry = {
    ...
    profile match {
      ...
      case name =>
        throw new IllegalArgumentException("Unknown value for vtemailer.profile: " + name)
    }
  }
  ...
  /**
    ...
    */
  class Default(val mailPoller: MailPoller, val mailDecorator: MailDecorator)
      extends EmailerRegistry {
    def configFile: String =
      System.getProperty("user.home") + "/oph-configuration/valinta-tulos-service.properties"

    lazy val settings: EmailerConfig =
      ApplicationSettingsLoader.loadSettings(configFile)(EmailerConfigParser())
  }

  /**
    * IT (integration test) profiles.
    */
  class IT(val mailPoller: MailPoller, val mailDecorator: MailDecorator)
      extends EmailerRegistry
      with StubbedGroupEmail {
    logger.info("Using template variables from " + templateAttributesFile)
    lazy val settings: EmailerConfig = loadSettings

    def loadSettings: EmailerConfig =
      ConfigTemplateProcessor.createSettings("valinta-tulos-service", templateAttributesFile)(
        EmailerConfigParser()
      )

    def templateAttributesFile = "src/main/resources/oph-configuration/dev-vars.yml"

    def lastEmailSize(): Int =
      groupEmailService match {
