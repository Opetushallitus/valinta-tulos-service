...
import fi.vm.sade.valintatulosservice.{
  AuditInfo,
  ValinnantuloksenLisays,
  ValinnantuloksenMuokkaus,
  ValinnantuloksenPoisto
}
import slick.dbio.DBIO

import scala.concurrent.ExecutionContext.Implicits.global

class ErillishaunValinnantulosStrategy(
  auditInfo: AuditInfo,
  haku: Haku,
  ...
  audit: Audit
) extends ValinnantulosStrategy
    with Logging {
  private val session = auditInfo.session._2

  lazy val hakukohdeRecord: Either[Throwable, HakukohdeRecord] =
    hakukohdeRecordService.getHakukohdeRecord(hakukohdeOid)
  lazy val vastaanottoValidator = new ErillishaunVastaanottoValidator(
    haku,
    hakukohdeOid,
    ohjausparametrit,
    valinnantulosRepository
  ...
  def hasChange(uusi: Valinnantulos, vanha: Valinnantulos) =
    uusi.hasChanged(vanha) || uusi.poistettava.getOrElse(false)

  def validate(uusi: Valinnantulos, vanha: Option[Valinnantulos]) = {

    def validateValinnantila() =
      (uusi.valinnantila, uusi.vastaanottotila) match {
        case (Hylatty, ValintatuloksenTila.KESKEN) | (Varalla, ValintatuloksenTila.KESKEN) |
            ...
            (Perunut, ValintatuloksenTila.PERUNUT) | (Peruutettu, ValintatuloksenTila.PERUUTETTU) |
            (_, ValintatuloksenTila.KESKEN) =>
          Right()
        ...
        case (_, _) =>
          Left(
            ValinnantulosUpdateStatus(
              409,
              s"Hakemuksen tila ${uusi.valinnantila} ja vastaanotto ${uusi.vastaanottotila} ovat ristiriitaiset.",
    ...
    def validateEhdollisestiHyvaksyttavissa() = {
      def notModified() =
        uusi.ehdollisestiHyvaksyttavissa.getOrElse(false) == vanha.exists(
          _.ehdollisestiHyvaksyttavissa.getOrElse(false)
        )
      ...
      if (notModified() || uusi.ohitaVastaanotto.getOrElse(false)) {
        ...
        Left(
          ValinnantulosUpdateStatus(
            409,
            s"Toisen asteen haussa ei voida hyväksyä ehdollisesti",
            uusi.valintatapajonoOid,
            uusi.hakemusOid
          )
        )
      ...
      (vanha.flatMap(_.julkaistavissa), uusi.julkaistavissa) match {
        case (Some(true), None) | (Some(true), Some(false)) if uusi.ilmoittautumistila != EiTehty =>
          Left(
            ValinnantulosUpdateStatus(
              409,
              s"Valinnantulosta ei voida merkitä ei-julkaistavaksi, koska sen ilmoittautumistila on ${uusi.ilmoittautumistila}",
              uusi.valintatapajonoOid,
              uusi.hakemusOid
            )
          )
        case (Some(true), None) | (Some(true), Some(false))
            if uusi.vastaanottotila != ValintatuloksenTila.KESKEN =>
          Left(
            ValinnantulosUpdateStatus(
              409,
              s"Valinnantulosta ei voida merkitä ei-julkaistavaksi, koska sen vastaanottotila on ${uusi.vastaanottotila}",
              uusi.valintatapajonoOid,
              uusi.hakemusOid
            )
          )
    ...
    def validateIlmoittautuminen() =
      (uusi.ilmoittautumistila, uusi.vastaanottotila, uusi.julkaistavissa) match {
        ...
        case (EiTehty, _, _) | (_, ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI, Some(true)) =>
          Right()
        case (_, _, _) =>
          Left(
            ValinnantulosUpdateStatus(
              409,
              s"Ilmoittautumistila ${uusi.ilmoittautumistila} ei ole sallittu, kun vastaanotto on ${uusi.vastaanottotila} ja julkaistavissa tieto on ${uusi.julkaistavissa}",
              uusi.valintatapajonoOid,
              uusi.hakemusOid
            )
          )
    ...
    def validateTilat() = {
      def ilmoittautunut(ilmoittautuminen: SijoitteluajonIlmoittautumistila) =
        ilmoittautuminen != EiTehty
      def hyvaksytty(tila: Valinnantila) =
        List(Hyvaksytty, VarasijaltaHyvaksytty).contains(
          tila
      ...
      def vastaanottoEiMyohastynyt(vastaanotto: ValintatuloksenTila) =
        vastaanotto != ValintatuloksenTila.KESKEN && vastaanotto != ValintatuloksenTila.EI_VASTAANOTETTU_MAARA_AIKANA
      def hylattyTaiVaralla(tila: Valinnantila) = Hylatty == tila || Varalla == tila
      def vastaanottaneena(tila: ValintatuloksenTila) =
        List(
      ...
      def keskenTaiPerunut(tila: ValintatuloksenTila) =
        List(
      ...
      (uusi.valinnantila, uusi.vastaanottotila, uusi.ilmoittautumistila) match {
        case (t, _, _)
            if uusi.ohitaVastaanotto
              .getOrElse(false) && uusi.ohitaIlmoittautuminen.getOrElse(false) =>
          Right()
        case (t, v, _)
            if hyvaksytty(t) && keskenTaiPerunut(v) && uusi.ohitaIlmoittautuminen.getOrElse(
              false
            ) =>
          Right()
        case (t, v, i)
            if t == Peruuntunut && v == ValintatuloksenTila.OTTANUT_VASTAAN_TOISEN_PAIKAN && i == EiTehty =>
          Right()
        case (_, _, _)
            if uusi.ohitaVastaanotto
              .getOrElse(false) || uusi.ohitaIlmoittautuminen.getOrElse(false) =>
          Left(
            ValinnantulosUpdateStatus(
              409,
              s"Vastaanoton tai ilmoittautumisen tallennusta ei voida ohittaa",
              uusi.valintatapajonoOid,
              uusi.hakemusOid
            )
          )
        case (t, v, i) if ilmoittautunut(i) && !(hyvaksytty(t) && vastaanotto(v)) =>
          Left(
            ValinnantulosUpdateStatus(
              409,
        ...
        case (t, v, i) if hylattyTaiVaralla(t) && (vastaanottaneena(v) || ilmoittautunut(i)) =>
          Left(
            ValinnantulosUpdateStatus(
              409,
        ...
        case (t, v, i) if peruneena(t) && !keskenTaiPerunut(v) =>
          Left(
            ValinnantulosUpdateStatus(
              409,
        ...
        case (t, v, i)
            if (t == Perunut && v == ValintatuloksenTila.PERUNUT) || (t == Peruutettu && v == ValintatuloksenTila.PERUUTETTU) =>
          Right()
        case (t, v, i) if vastaanottoEiMyohastynyt(v) && !hyvaksytty(t) =>
          Left(
            ValinnantulosUpdateStatus(
              409,
    ...
    def validatePoisto() =
      (uusi.poistettava.getOrElse(false), vanha) match {
        case (true, None) =>
          Left(
            ValinnantulosUpdateStatus(
              404,
    ...
    def validateVastaanottoNotChanged() =
      vanha.map(_.vastaanottotila) match {
        ...
        case Some(x) =>
          Left(
            ValinnantulosUpdateStatus(
              409,
              s"Valinnantulosta ei voida päivittää, koska vastaanottoa ${uusi.vastaanottotila} on muutettu samanaikaisesti tilaan ${x}",
              uusi.valintatapajonoOid,
              uusi.hakemusOid
            )
          )
  ...
  def save(uusi: Valinnantulos, vanhaOpt: Option[Valinnantulos]): DBIO[Unit] = {
    ...
    def vastaanottoAction() =
      new VirkailijanVastaanotto(
        ...
        uusi.hakemusOid,
        hakukohdeOid,
        VirkailijanVastaanottoAction.getVirkailijanVastaanottoAction(
          Vastaanottotila.values
            .find(Vastaanottotila.matches(_, uusi.vastaanottotila))
            .getOrElse(
              throw new IllegalArgumentException(
                s"Odottamaton vastaanottotila ${uusi.vastaanottotila}"
              )
            )
        ),
        muokkaaja,
      ...
      List(
        Some(
          valinnantulosRepository.storeValinnantila(
            uusi.getValinnantilanTallennus(muokkaaja),
            ifUnmodifiedSince
          )
        ),
        Some(
        ...
        Some(
          valinnantulosRepository.storeValinnantuloksenOhjaus(
            uusi.getValinnantuloksenOhjaus(muokkaaja, selite),
            ifUnmodifiedSince
          )
        ),
        uusi.getEhdollisenHyvaksynnanEhto.map(
        ...
        Option(uusi.ilmoittautumistila != EiTehty).collect {
          case true =>
            valinnantulosRepository.storeIlmoittautuminen(
              uusi.henkiloOid,
              Ilmoittautuminen(uusi.hakukohdeOid, uusi.ilmoittautumistila, muokkaaja, selite),
              ifUnmodifiedSince
            )
        },
        ...
        Option(
          uusi.vastaanottotila != ValintatuloksenTila.KESKEN && uusi.vastaanottotila != ValintatuloksenTila.OTTANUT_VASTAAN_TOISEN_PAIKAN
        ).collect {
    ...
    def createUpdateOperations(vanha: Valinnantulos) = {
      List(
        Option(uusi.valinnantila != vanha.valinnantila).collect {
          case true =>
            valinnantulosRepository.storeValinnantila(
              uusi.getValinnantilanTallennus(muokkaaja),
              ifUnmodifiedSince
        ...
        Option(uusi.hasOhjausChanged(vanha)).collect {
          case true =>
            valinnantulosRepository.storeValinnantuloksenOhjaus(
              uusi.getValinnantuloksenOhjauksenMuutos(vanha, muokkaaja, selite),
              ifUnmodifiedSince
            )
        },
        Option(
        ...
        Option(
          uusi.ilmoittautumistila != vanha.ilmoittautumistila && !uusi.ohitaIlmoittautuminen
            .getOrElse(false)
        ).collect {
          case true =>
            valinnantulosRepository.storeIlmoittautuminen(
        ...
        Option(
          uusi.vastaanottotila != vanha.vastaanottotila &&
    ...
    def createDeleteOperations(vanha: Valinnantulos) = {
      List(
        ...
        Option(vanha.ilmoittautumistila != EiTehty).collect {
          case true =>
        ...
        Option(
          uusi.vastaanottotila == ValintatuloksenTila.KESKEN && vanha.vastaanottotila != ValintatuloksenTila.KESKEN
        ).collect {
          case true => valinnantulosRepository.storeAction(vastaanottoAction())
        },
        Some(
    ...
    val operations = (uusi.poistettava.getOrElse(false), vanhaOpt) match {
      ...
      case (true, None) =>
        logger.warn(
          s"Käyttäjä ${muokkaaja} yritti poistaa " +
            s"hakemuksen ${uusi.hakemusOid} valinnan tuloksen erillishaun valintatapajonossa ${uusi.valintatapajonoOid}" +
  ...
  def audit(uusi: Valinnantulos, vanhaOpt: Option[Valinnantulos]): Unit =
    (uusi.poistettava.getOrElse(false), vanhaOpt) match {
      case (false, Some(vanha)) =>
        audit.log(
          auditInfo.user,
          ValinnantuloksenMuokkaus,
          ...
          new Changes.Builder()
            ...
            .updated(
              ...
              vanha.ehdollisestiHyvaksyttavissa.getOrElse(false).toString,
              uusi.ehdollisestiHyvaksyttavissa.getOrElse(false).toString
            )
            .updated(
              "julkaistavissa",
              vanha.julkaistavissa.getOrElse(false).toString,
              uusi.julkaistavissa.getOrElse(false).toString
            )
            .updated(
              "hyvaksyttyVarasijalta",
              vanha.hyvaksyttyVarasijalta.getOrElse(false).toString,
              uusi.hyvaksyttyVarasijalta.getOrElse(false).toString
            )
            .updated(
              "hyvaksyPeruuntunut",
              vanha.hyvaksyPeruuntunut.getOrElse(false).toString,
              uusi.hyvaksyPeruuntunut.getOrElse(false).toString
            )
            .updated(
              "vastaanottotila",
              vanha.vastaanottotila.toString,
              uusi.vastaanottotila.toString
            )
            .updated(
              "ilmoittautumistila",
              vanha.ilmoittautumistila.toString,
              uusi.ilmoittautumistila.toString
            )
            .build()
        )
      case (false, None) =>
