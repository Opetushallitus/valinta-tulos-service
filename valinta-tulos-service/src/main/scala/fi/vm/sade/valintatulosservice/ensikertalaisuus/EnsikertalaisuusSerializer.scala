...
class EnsikertalaisuusSerializer
    extends CustomSerializer[Ensikertalaisuus]((formats: Formats) =>
      (
        {
        ...
        },
        {
          ...
          case eiEnsikertalainen: EiEnsikertalainen =>
            ("personOid" -> eiEnsikertalainen.personOid) ~ ("paattyi" -> decompose(
              eiEnsikertalainen.paattyi
            )(formats))
        }
      )
    )
