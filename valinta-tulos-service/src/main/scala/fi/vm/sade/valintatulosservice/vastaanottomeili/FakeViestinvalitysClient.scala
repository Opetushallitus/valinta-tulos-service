...
class FakeViestinvalitysClient extends ViestinvalitysClient with JsonFormats with Logging {
  ...
  override def luoLahetys(lahetys: Lahetys): LuoLahetysSuccessResponse = {
    logger.info(s"create lahetys: ${javaObjectToJsonString(lahetys)}")
  ...
  override def luoViesti(viesti: Viesti): LuoViestiSuccessResponse = {
    logger.info(s"send viesti: ${javaObjectToJsonString(viesti)}")
