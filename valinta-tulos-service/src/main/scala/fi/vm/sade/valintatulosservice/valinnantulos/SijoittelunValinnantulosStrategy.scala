  ...
  def validate(uusi: Valinnantulos, vanhaOpt: Option[Valinnantulos]): DBIO[Either[ValinnantulosUpdateStatus, Unit]] = {
    if (vanhaOpt.isEmpty) {
      ...
      def validateIlmoittautumistila() = (uusi.ilmoittautumistila, uusi.vastaanottotila) match {
        ...
        case (EiTehty, ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI) => Left(ValinnantulosUpdateStatus(409, s"Ilmoittautumista ei voida poistaa, koska vastaanotto on sitova", uusi.valintatapajonoOid, uusi.hakemusOid))
