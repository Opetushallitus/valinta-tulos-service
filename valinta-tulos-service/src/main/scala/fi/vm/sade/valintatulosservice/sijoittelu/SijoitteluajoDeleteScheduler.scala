  ...
  val task = new Runnable {
    override def run(): Unit = Try {
      ...
      if (!repository.acquireLockForSijoitteluajoCleaning(55).head) {
        logger.info("sijoitteluajojen poistolukko: Lukkoa ei saatu. Joku muu VTS-node on vastuussa vanhojen sijoitteluajojen poistosta.")
      } else {
        logger.info("sijoitteluajojen poistolukko: Saatiin lukko. Tämä VTS-node on vastuussa vanhojen sijoitteluajojen poistosta. Suoritetaan siivous.")
        ...
        if (repository.clearLockForSijoitteluajoCleaning(55).head) {
          logger.info("sijoitteluajojen poistolukko: Sijoitteluajojen poisto on päättynyt ja lukitus onnistuneesti poistettu.")
        } else {
