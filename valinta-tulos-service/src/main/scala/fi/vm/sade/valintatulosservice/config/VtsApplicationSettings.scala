...
case class VtsApplicationSettings(config: Config) extends ApplicationSettings(config) {
  ...
  val useValintarekisteriInsteadOfSijoitteluMongo = BooleanUtils.isTrue(withConfig(_.getBoolean("valinta-tulos-service.use.valintarekisteri.instead.of.sijoittelu.mongo")))
