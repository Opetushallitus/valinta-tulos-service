...
class ValintarekisteriSijoittelunTulosClientImpl(
  repository: HakijaRepository with SijoitteluRepository with ValinnantulosRepository
) extends ValintarekisteriSijoittelunTulosClient {

  private def run[R](operations: slick.dbio.DBIO[R]): R = repository.runBlocking(operations)

  override def fetchLatestSijoitteluAjo(
    hakuOid: HakuOid,
    hakukohdeOid: Option[HakukohdeOid] = None
  ): Option[SijoitteluAjo] = {
    val latestId = repository.runBlocking(repository.getLatestSijoitteluajoId(hakuOid))

    val sijoitteluajonHakukohdeOidit =
      latestId.map(id => repository.getSijoitteluajonHakukohdeOidit(id)).getOrElse(List())
    val valinnantulostenHakukohdeOidit = run(
      repository.getValinnantulostenHakukohdeOiditForHaku(hakuOid)
    )
  ...
  override def fetchLatestSijoitteluAjoWithoutHakukohdes(
    hakuOid: HakuOid
  ): Option[SijoitteluAjo] = {
  ...
  override def fetchHakemuksenTulos(
    sijoitteluajoId: Option[Long],
    hakuOid: HakuOid,
    hakemusOid: HakemusOid
  ): Option[HakijaDTO] = {
