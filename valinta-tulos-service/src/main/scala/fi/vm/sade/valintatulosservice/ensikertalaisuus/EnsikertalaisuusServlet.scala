...
class EnsikertalaisuusServlet(
  ensikertalaisuusRepository: EnsikertalaisuusRepository,
  maxHenkiloOids: Int
)(implicit val swagger: Swagger, appConfig: VtsAppConfig)
    extends VtsServletBase
    with EnsikertalaisuusSwagger {
  ...
  get("/:henkilo", operation(getEnsikertalaisuusSwagger)) {
    ensikertalaisuusRepository.findEnsikertalaisuus(
      henkiloOid(params("henkilo")),
      parseKausi(params("koulutuksenAlkamiskausi"))
    )
  }
  ...
  post("/", operation(postEnsikertalaisuusSwagger)) {
    ...
    if (henkilot.size > maxHenkiloOids)
      throw new IllegalArgumentException(
        s"Too many henkilo oids (limit is $maxHenkiloOids, got ${henkilot.size})"
      )
...
trait EnsikertalaisuusSwagger extends VtsSwaggerBase { this: SwaggerSupport =>
  ...
  override protected def applicationDescription: String =
    "Ensikertalaisuus-tiedon vastaanotto-osuus"
  ...
  registerModel(
    Model(
      ...
      None,
      List(
        "personOid" -> ModelProperty(DataType.String, required = true),
        "paattyi" -> ModelProperty(DataType.DateTime)
      ),
      Some("Ensikertalaisuus"),
      None
    )
  )

  val getEnsikertalaisuusSwagger: OperationBuilder =
    apiOperation[Ensikertalaisuus]("getEnsikertalaisuus")
      .summary("Hae ensikertalaisuus-tiedon vastaanotto-osuus")
      .notes(
        "Ei pidä käyttää sellaisenaan, vain ainoastaan suoritusrekisterin kautta. Rajapinta palauttaa joko pelkän " +
      ...
      .parameter(
        queryParam[String]("koulutuksenAlkamiskausi")
          .description(
            "Koulutuksen alkamiskausi, josta lähtien alkavat koulutukset kyselyssä otetaan huomioon (esim. 2016S"
          )
          .required
      )
  ...
  val getVastaanottoHistoriaSwagger: OperationBuilder =
    apiOperation[VastaanottoHistoria]("getVastaanottoHistoria")
  ...
  val postEnsikertalaisuusSwagger: OperationBuilder =
    apiOperation[Seq[Ensikertalaisuus]]("getEnsikertalaisuus")
      .summary("Hae ensikertalaisuus-tiedon vastaanotto-osuus")
      .notes(
        "Ei pidä käyttää sellaisenaan, vain ainoastaan suoritusrekisterin kautta. Rajapinta palauttaa setin ensikertalaisuus-objekteja."
      )
      .parameter(
        bodyParam[Seq[String]]("henkiloOids")
          .description("Henkilöiden oidit json-sekvenssinä, enintään 10000 oidia yhdessä pyynnössä")
          .required
      )
      .parameter(
        queryParam[String]("koulutuksenAlkamiskausi")
