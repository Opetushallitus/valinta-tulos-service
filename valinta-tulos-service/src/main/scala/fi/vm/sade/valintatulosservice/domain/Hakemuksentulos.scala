...
object Hakutoiveentulos {
  def julkaistavaVersioSijoittelunTuloksesta(
    ilmoittautumisenAikaleima: Option[Date],
    ...
    hasHetu: Boolean
  )(implicit appConfig: VtsAppConfig): Hakutoiveentulos = {
    val saaJulkaista: Boolean =
      !checkJulkaisuAikaParametri || ohjausparametrit.tulostenJulkistusAlkaa.forall(_.isBeforeNow())
