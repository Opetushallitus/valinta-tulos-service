...
class ValintatulosService(
  ...
  def haunKoulutuksenAlkamiskaudenVastaanototYhdenPaikanSaadoksenPiirissa(
    hakuOid: HakuOid
    ...
    (for {
      ...
      .collect({
        case YPSHakukohde(oid, _, koulutuksenAlkamiskausi) => (oid, koulutuksenAlkamiskausi)
      })
      .groupBy(_._2)
      .toList match {
      ...
      case (kausi, _) :: Nil =>
        virkailijaVastaanottoRepository
          .findkoulutuksenAlkamiskaudenVastaanottaneetYhdenPaikanSaadoksenPiirissa(kausi)
      case kaudet =>
        throw new IllegalStateException(
          s"Haussa $hakuOid on YPS piirissä olevia hakukohteita joilla on eri koulutuksen alkamiskausi ${kaudet
            .map(_._2.map(_._1).mkString(", "))
            .mkString("; ")}"
        )
    }) match {
      case Right(vs) => vs
  ...
  def hakemuksentulos(h: Hakemus): Option[Hakemuksentulos] = {
    for {
      ...
      ilmoittautumisenAikaleimat =
        timed(s"Ilmoittautumisten aikaleimojen haku hakijalle ${h.henkiloOid}", 1000) {
          Map(
            h.henkiloOid -> valinnantulosRepository.runBlocking(
              valinnantulosRepository.getIlmoittautumisenAikaleimat(h.henkiloOid)
            )
          )
        }
      hakukohdeRecords <-
        hakukohdeRecordService.getHakukohdeRecords(h.toiveet.map(_.oid)).right.toOption
      vastaanototKausilla <- Some({
        val vastaanototByKausi = hakukohdeRecords
          ...
          .distinct
          .map(kausi =>
            kausi -> hakijaVastaanottoRepository.runBlocking(
              hakijaVastaanottoRepository
                .findYhdenPaikanSaannonPiirissaOlevatVastaanotot(h.henkiloOid, kausi)
            )
          )
          .toMap
        hakukohdeRecords
          .collect({
            case YPSHakukohde(oid, _, koulutuksenAlkamiskausi) =>
              oid -> (koulutuksenAlkamiskausi, vastaanototByKausi(koulutuksenAlkamiskausi)
                .map(_.henkiloOid)
                .toSet)
          })
          .toMap
      ...
      tulos = valinnantulosRepository.runBlocking(
        sijoittelutulosService.tulosHakijana(haku.oid, h.oid, h.henkiloOid, ohjausparametrit)
      )
  ...
  def hakemustenTulosByHaku(
    hakuOid: HakuOid,
    checkJulkaisuAikaParametri: Boolean
  ): Option[Iterator[Hakemuksentulos]] = {
  ...
  def haeTilatHakijoille(
    ...
    (
      hakuService.getHaku(hakuOid),
      hakukohdeRecordService.getHakukohdeRecords(uniqueHakukohdeOids)
    ) match {
      case (Right(haku), Right(hakukohdes)) =>
        val vastaanototByKausi = timed("kaudenVastaanotot", 1000)({
          virkailijaVastaanottoRepository
            .findkoulutuksenAlkamiskaudenVastaanottaneetYhdenPaikanSaadoksenPiirissa(
              hakukohdes
                .collect({
                  case YPSHakukohde(_, _, koulutuksenAlkamiskausi) => koulutuksenAlkamiskausi
                })
                .toSet
            )
        })
        val vastaanototKausilla = hakukohdes
          .collect({
            case YPSHakukohde(oid, _, koulutuksenAlkamiskausi) =>
              oid -> (koulutuksenAlkamiskausi, vastaanototByKausi(koulutuksenAlkamiskausi)
                .map(_.henkiloOid))
          ...
          .toMap
        val ilmoittautumisenAikaleimat =
          timed(s"Ilmoittautumisten aikaleimojen haku haulle $hakuOid", 1000) {
            valinnantulosRepository
              .runBlocking(valinnantulosRepository.getIlmoittautumisenAikaleimat(hakuOid))
              .groupBy(_._1)
              .mapValues(i => i.map(t => (t._2, t._3)))
          }
        val ohjausparametrit =
          sijoittelutulosService.findOhjausparametritFromOhjausparametritService(hakuOid)
        val tulokset = valinnantulosRepository.runBlocking(
          DBIO.sequence(
            hakemusOids
              .map(oid =>
                sijoittelutulosService
                  .tulosHakijana(haku.oid, oid, hakijaOidByHakemusOid(oid), ohjausparametrit)
              )
              .toSeq
          )
        )
        ...
        hakemustenTulokset.flatMap { hakemuksenTulos =>
          hakemuksenTulos.hakutoiveet.find(_.valintatapajonoOid == valintatapajonoOid).map {
            hakutoiveenTulos =>
              val tilaHakijalle =
                ValintatuloksenTila.valueOf(hakutoiveenTulos.vastaanottotila.toString)
              TilaHakijalle(
  ...
  def hakemustenTulosByHaku(
    hakuOid: HakuOid,
    haunVastaanotot: Option[Map[String, Set[VastaanottoRecord]]],
    ...
    timed("Fetch hakemusten tulos for haku: " + hakuOid, 1000)(
      for {
        ...
        hakukohteet <-
          hakukohdeRecordService.getHakukohdeRecords(hakuOid, hakukohdeOids.toSet).right.toOption
        ...
        fetchTulokset(
          ...
          personOidFromHakemusResolver =>
            sijoittelutulosService.hakemustenTulos(
              hakuOid,
              None,
              personOidFromHakemusResolver,
              haunVastaanotot = haunVastaanotot
            ),
          Some(new PersonOidFromHakemusResolver {
            private lazy val hakijaOidByHakemusOid =
              timed("personOids from hakemus", 1000)(hakemusRepository.findPersonOids(hakuOid))
  ...
  def hakemustenTulosByHakukohde(
    hakuOid: HakuOid,
    hakukohdeOid: HakukohdeOid,
    hakukohteenVastaanotot: Option[Map[String, Set[VastaanottoRecord]]] = None,
    checkJulkaisuAikaParametri: Boolean = true,
    vainHakukohteenTiedot: Boolean = false
  ): Either[Throwable, Iterator[Hakemuksentulos]] = {
    val hakemukset: Seq[Hakemus] =
      hakemusRepository.findHakemuksetByHakukohde(hakuOid, hakukohdeOid).toVector //hakemus-mongo
    val uniqueHakukohdeOids: Seq[HakukohdeOid] = hakemukset.flatMap(_.toiveet.map(_.oid)).distinct
    timed("Fetch hakemusten tulos for haku: " + hakuOid + " and hakukohde: " + hakukohdeOid, 1000)(
      for {
        ...
        hakukohteet <-
          hakukohdeRecordService
            .getHakukohdeRecords(uniqueHakukohdeOids)
            .right //valintarekisteri/hakukohde
      } yield {
        ...
        val vastaanototKausilla = hakukohteet
          .collect({
            case YPSHakukohde(oid, _, koulutuksenAlkamiskausi) =>
              oid -> (koulutuksenAlkamiskausi, vastaanototByKausi(koulutuksenAlkamiskausi)
                .map(_.henkiloOid))
        ...
        fetchTulokset(
          ...
          personOidFromHakemusResolver =>
            sijoittelutulosService.hakemustenTulos(
              ...
              Some(hakukohdeOid),
              personOidFromHakemusResolver,
              hakukohteenVastaanotot,
              vainHakukohde = vainHakukohteenTiedot
            ),
          Some(new PersonOidFromHakemusResolver {
            private lazy val hakijaOidByHakemusOid = timed("personOids from hakemus", 1000)(
  ...
  def findValintaTuloksetForVirkailija(hakuOid: HakuOid): List[Valintatulos] = {
    val haunVastaanotot =
      virkailijaVastaanottoRepository.findHaunVastaanotot(hakuOid).groupBy(_.henkiloOid)
    val hakemustenTulokset = hakemustenTulosByHaku(hakuOid, Some(haunVastaanotot)).getOrElse(
      throw new IllegalArgumentException(s"Unknown hakuOid $hakuOid")
    )
    val valintatulokset = valintatulosDao.loadValintatulokset(hakuOid)

    setValintatuloksetTilat(
      valintatulokset,
      mapHakemustenTuloksetByHakemusOid(hakemustenTulokset),
      haunVastaanotot
    )
    valintatulokset
  }

  def findValintaTuloksetForVirkailija(
    hakuOid: HakuOid,
    hakukohdeOid: HakukohdeOid
  ): List[Valintatulos] = {
    val haunVastaanotot: Map[String, Set[VastaanottoRecord]] =
      virkailijaVastaanottoRepository.findHaunVastaanotot(hakuOid).groupBy(_.henkiloOid)
    val hakemustenTulokset: Iterator[Hakemuksentulos] =
      hakemustenTulosByHakukohde(hakuOid, hakukohdeOid, Some(haunVastaanotot)) match {
        case Right(x) => x
        case Left(e)  => throw e
      }
    val valintatulokset: List[Valintatulos] =
  ...
  def findValintaTuloksetForVirkailijaWithoutTilaHakijalle(
    hakuOid: HakuOid,
    hakukohdeOid: HakukohdeOid
  ): List[Valintatulos] = {
    val haunVastaanotot: Map[String, Set[VastaanottoRecord]] =
      timed(s"Fetch vastaanotto records for haku $hakuOid", 1000) {
        virkailijaVastaanottoRepository.findHaunVastaanotot(hakuOid).groupBy(_.henkiloOid)
      }
    lazy val hakemusOidsByHakijaOids = timed(
      s"Fetch hakija oids by hakemus oids for haku $hakuOid and hakukohde $hakukohdeOid",
      1000
    ) {
      hakemusRepository.findPersonOids(hakuOid, hakukohdeOid)
    }
    lazy val hakemusOidsByHakijaOidsForWholeHaku =
      timed(s"Fetch hakija oids by hakemus oids for haku $hakuOid $hakukohdeOid", 1000) {
        hakemusRepository.findPersonOids(hakuOid)
      }

    val valintatulokset: List[Valintatulos] =
      timed(s"Fetch plain valintatulokset for haku $hakuOid", 1000) {
        valintatulosDao.loadValintatuloksetForHakukohde(hakukohdeOid)
      }
    ...
    val kaudenVastaanotot =
      timed(s"Fetch YPS related kauden vastaanotot for hakukohde $hakukohdeOid", 1000) {
        hakukohde match {
    ...
    while (valintatulosIterator.hasNext) {
      ...
      val hakijaOid: String =
        if (StringUtils.isNotBlank(v.getHakijaOid)) v.getHakijaOid
        else
          hakemusOidsByHakijaOids.getOrElse(
            HakemusOid(v.getHakemusOid), {
              logger.warn(
                s"Could not find hakija oid for ${v.getHakemusOid} when finding for hakukohde $hakukohdeOid , " +
      ...
      val hakijanVastaanototHakukohteeseen: List[VastaanottoRecord] =
        henkilonVastaanotot.map(_.filter(_.hakukohdeOid == hakukohdeOid)).toList.flatten
      val tilaVirkailijalle: ValintatuloksenTila = paatteleVastaanottotilaVirkailijaaVarten(
        hakijaOid,
        hakuOid,
        hakukohde,
        hakijanVastaanototHakukohteeseen,
        kaudenVastaanotot
      )
      v.setTila(
        tilaVirkailijalle,
        ...
        "",
        ""
      ) // pass same old and new tila to avoid log entries
      v.setHakijaOid(hakijaOid, "")
    }
    valintatulokset
  }

  def haeVastaanotonAikarajaTiedot(
    hakuOid: HakuOid,
    hakukohdeOid: HakukohdeOid,
    hakemusOids: Set[HakemusOid]
  ): Set[VastaanottoAikarajaMennyt] = {
  ...
  def findValintaTuloksetForVirkailijaByHakemus(hakemusOid: HakemusOid): List[Valintatulos] = {
    val hakemuksenTulos = hakemuksentulos(hakemusOid)
      .getOrElse(
        throw new IllegalArgumentException(s"Not hakemuksen tulos for hakemus $hakemusOid")
      )
    val hakuOid = hakemuksenTulos.hakuOid
    val henkiloOid = hakemuksenTulos.hakijaOid
    val vastaanotot = virkailijaVastaanottoRepository.runBlocking(
      virkailijaVastaanottoRepository.findHenkilonVastaanototHaussa(henkiloOid, hakuOid)
    )
    val valintatulokset: List[Valintatulos] =
      valintatulosDao.loadValintatuloksetForHakemus(hakemusOid)

    setValintatuloksetTilat(
      valintatulokset,
      Map(hakemusOid -> hakemuksenTulos),
      Map(henkiloOid -> vastaanotot)
    )
    valintatulokset
  }

  def sijoittelunTulokset(
    hakuOid: HakuOid,
    sijoitteluajoId: String,
    hyvaksytyt: Option[Boolean],
    ilmanHyvaksyntaa: Option[Boolean],
    vastaanottaneet: Option[Boolean],
    hakukohdeOid: Option[List[HakukohdeOid]],
    count: Option[Int],
    index: Option[Int]
  ): HakijaPaginationObject =
    timed(s"Getting sijoittelun tulokset for haku $hakuOid") {
      ...
      val hakemustenTulokset = hakukohdeOid match {
        case Some(oid :: Nil) =>
          hakemustenTulosByHakukohde(hakuOid, oid, Some(haunVastaanototByHakijaOid)).right.get
        case _ =>
          hakemustenTulosByHaku(hakuOid, Some(haunVastaanototByHakijaOid)).getOrElse(Iterator.empty)
      }
      ...
      try {
        val hakijaPaginationObject =
          sijoittelutulosService.sijoittelunTuloksetWithoutVastaanottoTieto(
            hakuOid,
            sijoitteluajoId,
            hyvaksytyt,
            ilmanHyvaksyntaa,
        ...
        hakijaPaginationObject.getResults.asScala.foreach { hakijaDto =>
          ...
          hakijaDto.getHakutoiveet.asScala.foreach(hakutoiveDto => {
            val tulos = hakemuksenTulos
              .findHakutoive(HakukohdeOid(hakutoiveDto.getHakukohdeOid))
              .getOrElse(
                throw new IllegalStateException(
                  s"Ei löydy hakutoiveelle ${hakutoiveDto.getHakukohdeOid} tulosta hakemukselta ${hakijaDto.getHakemusOid}. " +
                    s"Hakutoive saattaa olla poistettu hakemukselta sijoittelun jälkeen."
                )
              )
              ._1
            hakutoiveDto.setVastaanottotieto(
              fi.vm.sade.sijoittelu.tulos.dto.ValintatuloksenTila
                .valueOf(tulos.vastaanottotila.toString)
            )
          })
        }
        hakijaPaginationObject
  ...
  private def assertNoMissingHakemusOidsInKeys(
    hakemusOids: Set[HakemusOid],
    keys: Set[HakemusOid]
  ): Unit = {
    val missingHakemusOids: Set[HakemusOid] = hakemusOids.diff(keys)
    if (missingHakemusOids.nonEmpty) {
      val missingOidsException =
        s"HakijaDTOs contained more hakemusOids than in hakukohteeseen hyväksytyt: $missingHakemusOids"
      throw new RuntimeException(missingOidsException)
    }
  }

  def sijoittelunTulosHakemukselle(
    hakuOid: HakuOid,
    sijoitteluajoId: String,
    hakemusOid: HakemusOid
  ): Option[HakijaDTO] = {
    val hakemuksenTulosOption = hakemuksentulos(hakemusOid)
    val hakijaOidFromHakemusOption =
      hakemusRepository.findHakemus(hakemusOid).right.map(_.henkiloOid)
    val id = sijoittelutulosService.findSijoitteluAjo(hakuOid, sijoitteluajoId)
    sijoittelutulosService
      .sijoittelunTulosForAjoWithoutVastaanottoTieto(id, hakuOid, hakemusOid)
      .map(hakijaDto => {
        hakijaOidFromHakemusOption.right
          .foreach(hakijaOidFromHakemus => hakijaDto.setHakijaOid(hakijaOidFromHakemus))
        hakemuksenTulosOption.foreach(hakemuksenTulos =>
  ...
  def copyVastaanottotieto(hakutoiveDto: HakutoiveDTO, hakutoiveenTulos: Hakutoiveentulos): Unit = {
    hakutoiveDto.setVastaanottotieto(
        ...
        .valueOf(hakutoiveenTulos.vastaanottotila.toString)
    )
  }

  def populateVastaanottotieto(
    hakijaDto: HakijaDTO,
    hakemuksenHakutoiveidenTuloksetVastaanottotiedonKanssa: List[Hakutoiveentulos]
  ): Unit = {
    hakijaDto.getHakutoiveet.asScala.foreach(palautettavaHakutoiveDto =>
      hakemuksenHakutoiveidenTuloksetVastaanottotiedonKanssa.find(
        _.hakukohdeOid.toString == palautettavaHakutoiveDto.getHakukohdeOid
      ) match {
        case Some(hakutoiveenOikeaTulos) =>
          copyVastaanottotieto(palautettavaHakutoiveDto, hakutoiveenOikeaTulos)
        case None => palautettavaHakutoiveDto.setVastaanottotieto(dto.ValintatuloksenTila.KESKEN)
      }
    )
  }

  private def mapHakemustenTuloksetByHakemusOid(
    hakemustenTulokset: Iterator[Hakemuksentulos]
  ): Map[HakemusOid, Hakemuksentulos] = {
    hakemustenTulokset.toList.groupBy(_.hakemusOid).mapValues(_.head)
  }

  private def setValintatuloksetTilat(
    ...
    valintatulokset.foreach(valintaTulos => {
      val hakemuksenTulosOption: Option[Hakemuksentulos] = hakemustenTulokset
        .get(HakemusOid(valintaTulos.getHakemusOid))
        .orElse(crashOrLog(s"No hakemuksen tulos found for hakemus ${valintaTulos.getHakemusOid}"))
      val hakutoiveenTulosOption: Option[Hakutoiveentulos] = hakemuksenTulosOption
        .flatMap { _.findHakutoive(HakukohdeOid(valintaTulos.getHakukohdeOid)).map(_._1) }
        .orElse(
          crashOrLog(
            s"No hakutoive found for hakukohde ${valintaTulos.getHakukohdeOid} in hakemus ${valintaTulos.getHakemusOid}"
          )
        )

      val tulosPari: Option[(Hakemuksentulos, Hakutoiveentulos)] = hakemuksenTulosOption.flatMap {
        hakemuksenTulos => hakutoiveenTulosOption.map((hakemuksenTulos, _))
      }
      ...
      tulosPari match {
        case Some((hakemuksenTulos, hakutoiveenTulos)) =>
          ...
          val tilaVirkailijalle = ValintatulosService.toVirkailijaTila(
            tilaHakijalle,
            haunVastaanotot.get(hakijaOid),
        ...
        case None =>
          crashOrLog(
            s"Problem when processing valintatulos for hakemus ${valintaTulos.getHakemusOid}"
          )
          valintaTulos.setTila(
            ValintatuloksenTila.KESKEN,
            ValintatuloksenTila.KESKEN,
            "Tilaa ei saatu luettua sijoittelun tuloksista",
            ""
          )
  ...
  private def assertThatHakijaOidsDoNotConflict(
    valintaTulos: Valintatulos,
    hakemuksenTulos: Hakemuksentulos
  ): Unit = {
    if (
      valintaTulos.getHakijaOid != null && !valintaTulos.getHakijaOid.equals(
        hakemuksenTulos.hakijaOid
      )
    ) {
      if (
        virkailijaVastaanottoRepository
          .runBlocking(virkailijaVastaanottoRepository.aliases(valintaTulos.getHakijaOid))
          .contains(hakemuksenTulos.hakijaOid)
      ) {
        logger.warn(
          s"Valintatulos $valintaTulos with hakijaoid that is alias of hakijaoid in $hakemuksenTulos"
        )
      } else {
        crashOrLog(
          s"Conflicting hakija oids: valintaTulos: ${valintaTulos.getHakijaOid} vs hakemuksenTulos: ${hakemuksenTulos.hakijaOid} in $valintaTulos , $hakemuksenTulos"
        )
      }
    }
  }
  ...
  def fetchTulokset(
    ...
    ilmoittautumisenAikaleimat: Map[String, Iterable[(HakukohdeOid, Instant)]]
  ): Iterator[Hakemuksentulos] = {
    ...
    val sijoitteluTulokset = timed("Fetch sijoittelun tulos", 1000) {
      getSijoittelunTulos(
        personOidFromHakemusResolver.getOrElse(new PersonOidFromHakemusResolver {
          private lazy val hakijaOidByHakemusOid =
            getHakemukset().map(h => (h.oid, h.henkiloOid)).toMap
          override def findBy(hakemusOid: HakemusOid): Option[String] =
            hakijaOidByHakemusOid.get(hakemusOid)
        })
    ...
    hakemukset.map(hakemus => {
      val sijoitteluTulos = sijoitteluTulokset.getOrElse(
        hakemus.oid,
        HakemuksenSijoitteluntulos(hakemus.oid, None, Nil)
      )
  ...
  def julkaistavaTulos(
    sijoitteluTulos: HakemuksenSijoitteluntulos,
    haku: Haku,
    ...
    val tulokset = h.toiveet.map { toive =>
      val hakutoiveenSijoittelunTulos: HakutoiveenSijoitteluntulos = sijoitteluTulos.hakutoiveet
        .find { t =>
          t.hakukohdeOid == toive.oid
        }
        .getOrElse(HakutoiveenSijoitteluntulos.kesken(toive.oid, toive.tarjoajaOid))
      val ilmoittautumisenAikaleima =
        ilmoittautumisenAikaleimat.find(_._1 == toive.oid).map(t => Date.from(t._2))
    ...
    Hakemuksentulos(
      haku.oid,
      h.oid,
      sijoitteluTulos.hakijaOid.getOrElse(h.henkiloOid),
      ohjausparametrit.vastaanottoaikataulu,
      lopullisetTulokset
    )
  }

  private def piilotaEhdollisenHyväksymisenEhdotJonoiltaKunEiEhdollisestiHyväksytty(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
  ): List[Hakutoiveentulos] = {
    tulokset.map { tulos =>
      tulos.copy(
        jonokohtaisetTulostiedot = tulos.jonokohtaisetTulostiedot.map { jonokohtainenTulostieto =>
          if (
            jonokohtainenTulostieto.julkaistavissa && Valintatila
              .isHyväksytty(jonokohtainenTulostieto.valintatila)
          ) {
  ...
  private def merkitseValintatapajonotPeruuntuneeksiKunEiVastaanottanutMääräaikaanMennessä(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
    ...
    tulokset.map { tulos =>
      if (
        tulos.vastaanottotila == Vastaanottotila.ei_vastaanotettu_määräaikana && Valintatila
          .isHyväksytty(tulos.valintatila)
      ) {
        tulos.copy(
          jonokohtaisetTulostiedot = tulos.jonokohtaisetTulostiedot.map { jonokohtainenTulostieto =>
  ...
  private def piilotaVarasijanumeroJonoiltaJosValintatilaEiVaralla(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
  ...
  private def piilotaKuvauksetEiJulkaistuiltaValintatapajonoilta(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
  ...
  private def asetaKelaURL(
    ...
    val näytetäänKelaURL = if (
      hakukierrosEiOlePäättynyt && näytetäänSiirryKelaanURL && haku.sallittuKohdejoukkoKelaLinkille
    ) Some(appConfig.settings.kelaURL)
    else None
  ...
  private def asetaVastaanotettavuusValintarekisterinPerusteella(
    vastaanottoKaudella: HakukohdeOid => Option[(Kausi, Boolean)]
  )(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    ...
    def merkitseJonokohtainenTulostietoPerutuksiJosVoiTullaHyväksytyksi(
      jonokohtainenTulostieto: JonokohtainenTulostieto
    ) = {
      if (
        Valintatila.voiTullaHyväksytyksi(
        ...
        jonokohtainenTulostieto.copy(
          ...
          alinHyvaksyttyPistemaara = None,
          tilanKuvaukset =
            tilankuvaukset(PERUUNTUNUT_VASTAANOTTANUT_TOISEN_PAIKAN_YHDEN_SAANNON_PAIKAN_PIIRISSA)
        )
      } else {
        jonokohtainenTulostieto
      }
    ...
    def hyvaksyttyTaiVaralla(tulos: Hakutoiveentulos): Boolean =
      isHyväksytty(tulos.valintatila) || tulos.valintatila == Valintatila.varalla
    val hakutoiveetGroupedByKausi: Map[Option[(Kausi, Boolean)], List[Hakutoiveentulos]] =
      tulokset.groupBy(tulos => vastaanottoKaudella(tulos.hakukohdeOid))
    ...
    hakutoiveetGroupedByKausi
      .flatMap {
        case (Some((_, vastaanotto)), kaudenTulokset) =>
          val ehdollinenVastaanottoTallaHakemuksella = kaudenTulokset.exists(x =>
            Vastaanottotila.ehdollisesti_vastaanottanut == x.vastaanottotila
          )
          val sitovaVastaanottoTallaHakemuksella =
            kaudenTulokset.exists(x => vastaanottanut == x.vastaanottotila)
          if (ehdollinenVastaanottoTallaHakemuksella) {
            logger.debug(
              "asetaVastaanotettavuusValintarekisterinPerusteella ehdollinenVastaanottoTallaHakemuksella"
            )
            kaudenTulokset
          } else if (sitovaVastaanottoTallaHakemuksella) {
            kaudenTulokset.map(tulos =>
              if (
                Vastaanottotila.kesken == tulos.virkailijanTilat.vastaanottotila && hyvaksyttyTaiVaralla(
                  tulos
                )
              ) {
                logger.debug(
                  "asetaVastaanotettavuusValintarekisterinPerusteella sitovaVastaanottoTallaHakemuksella peruuntunutOttanutVastaanToisenPaikan"
                )
                peruuntunutOttanutVastaanToisenPaikan(tulos)
              } else {
                logger.debug(
                  "asetaVastaanotettavuusValintarekisterinPerusteella sitovaVastaanottoTallaHakemuksella"
                )
                tulos
              }
            )
            ...
            kaudenTulokset.map(tulos =>
              if (
                ...
                logger.debug(
                  "asetaVastaanotettavuusValintarekisterinPerusteella sitovaVastaanottoTallaHakemuksella peruuntunutOttanutVastaanToisenPaikan vastaanotto"
                )
                peruuntunutOttanutVastaanToisenPaikan(tulos)
              } else {
                tulos
      ...
      .toList
      .sorted(hakutoiveetToOriginalOrder)
  }

  private def paatteleVastaanottotilaVirkailijaaVarten(
    hakijaOid: String,
    hakuOid: HakuOid,
    hakukohde: HakukohdeRecord,
    hakijanVastaanototHakukohteeseen: List[VastaanottoRecord],
    kaudenVastaanototYpsnPiirissa: Set[VastaanottoRecord]
  ): ValintatuloksenTila = {
    def resolveValintatuloksenTilaVirkailijalleFrom(
      several: Iterable[VastaanottoRecord]
    ): ValintatuloksenTila = {
      if (several.groupBy(_.action).size == 1) {
        several.head.action.valintatuloksenTila
      } else {
        throw new IllegalStateException(
          s"Don't know how to choose relevant vastaanotto record from $several"
        )
      }
    }

    val tilaSuoraanHakukohteenVastaanottojenPerusteella: ValintatuloksenTila =
      hakijanVastaanototHakukohteeseen match {
    ...
    if (!hakukohde.yhdenPaikanSaantoVoimassa) {
      ...
      tilaSuoraanHakukohteenVastaanottojenPerusteella == ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI ||
      ...
      paatteleVastaanottotilaYhdenPaikanSaadoksenMukaanVirkailijaaVarten(
        hakijaOid,
        hakuOid,
        kaudenVastaanototYpsnPiirissa
      )
    }
  }

  private def paatteleVastaanottotilaYhdenPaikanSaadoksenMukaanVirkailijaaVarten(
    henkiloOid: String,
    hakuOid: HakuOid,
    kaudenVastaanototYpsnPiirissa: Set[VastaanottoRecord]
  ): ValintatuloksenTila = {
    val henkilonAiemmatVastaanototSamalleKaudelleYpsnPiirissa =
      kaudenVastaanototYpsnPiirissa.filter(_.henkiloOid == henkiloOid)
    def vastaanottoEriHaussa: Boolean =
      henkilonAiemmatVastaanototSamalleKaudelleYpsnPiirissa.exists(_.hakuOid != hakuOid)
  ...
  private def sovellaSijoitteluaKayttanvaKorkeakouluhaunSaantoja(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
    ...
    if (haku.korkeakoulu && haku.sijoitteluJaPriorisointi) {
      ...
      val firstHyvaksyttyUnderFirstVarallaAndNoPeruttuInBetween = if (firstVaralla >= 0) {
        indexedTulokset.indexWhere {
          case (tulos, index) =>
            index > firstVaralla && Valintatila.isHyväksytty(
              tulos.valintatila
            ) && (firstPeruttuAfterFirstVaralla == -1 || firstPeruttuAfterFirstVaralla > index)
        }
      } else {
        -1
      }
      ...
      def hyvaksyttyJaVastaanottamatta(tulos: Hakutoiveentulos, index: Int): Hakutoiveentulos = {
        if (firstVastaanotettu >= 0 && index != firstVastaanotettu) {
          ...
          logger.debug(
            "sovellaSijoitteluaKayttanvaKorkeakouluhaunSaantoja valintatila > peruuntunut, ei vastaanotettavissa {}",
            index
          )
          tulos.copy(
            valintatila = Valintatila.peruuntunut,
          ...
          if (ehdollinenVastaanottoMahdollista(ohjausparametrit)) {
            logger.debug(
              "sovellaSijoitteluaKayttanvaKorkeakouluhaunSaantoja vastaanotettavuustila > vastaanotettavissa_ehdollisesti {}",
              index
            )
            // Ehdollinen vastaanotto mahdollista
            tulos.copy(vastaanotettavuustila =
              Vastaanotettavuustila.vastaanotettavissa_ehdollisesti
            )
          } else {
            logger.debug(
              "sovellaSijoitteluaKayttanvaKorkeakouluhaunSaantoja toOdottaaYlempienHakutoiveidenTuloksia {} {}",
              index,
              Valintatila.isHyväksytty(tulos.valintatila)
            )
      ...
      tulokset.zipWithIndex.map {
        case (tulos, index)
            if isHyväksytty(tulos.valintatila) && tulos.vastaanottotila == Vastaanottotila.kesken =>
          hyvaksyttyJaVastaanottamatta(tulos, index)
        case (tulos, index)
            if firstVastaanotettu >= 0 && index != firstVastaanotettu && List(
              Valintatila.varalla,
              Valintatila.kesken
            ).contains(tulos.valintatila) =>
            ...
            jonokohtaisetTulostiedot = tulos.jonokohtaisetTulostiedot.map {
              jonokohtainenTulostieto =>
                jonokohtainenTulostieto.copy(
                  valintatila =
                    if (
                      List(Valintatila.varalla, Valintatila.kesken)
                        .contains(jonokohtainenTulostieto.valintatila)
                    ) {
  ...
  private def peruunnutaValmistaAlemmatKeskeneräisetJosKäytetäänSijoittelua(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
    ...
    if (haku.sijoitteluJaPriorisointi) {
      ...
      tulokset.zipWithIndex.map {
        case (tulos, index)
          ...
          logger.debug(
            "peruunnutaValmistaAlemmatKeskeneräisetJosKäytetäänSijoittelua valintatila > peruuntunut {}",
            index
          )
      ...
      logger.debug(
        "peruunnutaValmistaAlemmatKeskeneräisetJosKäytetäänSijoittelua ei käytä sijoittelua"
      )
      tulokset
    }
  }

  private def näytäAlemmatPeruuntuneetHakukohteetKeskeneräisinäJosYlemmätKeskeneräisiä(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
    ...
    tulokset.zipWithIndex.map {
      case (tulos, index)
          if firstKeskeneräinen >= 0 && index > firstKeskeneräinen && tulos.valintatila == Valintatila.peruuntunut =>
        logger.debug(
          "näytäAlemmatPeruuntuneetHakukohteetKeskeneräisinäJosYlemmätKeskeneräisiä toKesken {}",
          index
        )
        tulos.toKesken
      case (tulos, _) =>
  ...
  private def näytäAlemmatPeruuntuneetJonotKeskeneräisinäJosYlemmätKeskeneräisiä(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
    ...
    tulokset.map { hakutoiveenTulos =>
      hakutoiveenTulos.copy(jonokohtaisetTulostiedot =
        näytäToiveenAlemmatPeruuntuneetJonotKeskeneräisinäJosYlemmätKeskeneräisiä(hakutoiveenTulos)
      )
    }
  }

  private def näytäToiveenAlemmatPeruuntuneetJonotKeskeneräisinäJosYlemmätKeskeneräisiä(
    hakutoiveenTulos: Hakutoiveentulos
  ): List[JonokohtainenTulostieto] = {
    val firstKeskeneräinen =
      hakutoiveenTulos.jonokohtaisetTulostiedot.indexWhere(_.valintatila == Valintatila.kesken)
    hakutoiveenTulos.jonokohtaisetTulostiedot.zipWithIndex.map {
      case (jononTulos, index)
          if firstKeskeneräinen >= 0 && index > firstKeskeneräinen && jononTulos.valintatila == Valintatila.peruuntunut =>
        logger.debug(
          "näytäToiveenAlemmatPeruuntuneetJonotKeskeneräisinäJosYlemmätKeskeneräisiä toKesken {}",
          index
        )
        jononTulos.toKesken
      case (jononTulos, _) =>
  ...
  /**
    ...
    */
  private def onJulkaisematontaAlempiaPeruuntuneitaTaiKeskeneräisiä(
    ylimmänJulkaisemattomanIndeksi: Int,
    tulos: Hakutoiveentulos,
    hakutoiveenIndeksi: Int
  ): Boolean = {
  ...
  private def näytäJulkaisematontaAlemmatPeruuntuneetKeskeneräisinä(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
    ...
    tulokset.zipWithIndex.map {
      case (tulos, index)
          if onJulkaisematontaAlempiaPeruuntuneitaTaiKeskeneräisiä(
            firstJulkaisematon,
            tulos,
            index
          ) =>
        logger.debug("näytäJulkaisematontaAlemmatPeruuntuneetKeskeneräisinä toKesken {}", index)
        tulos.toKesken.copy(jonokohtaisetTulostiedot = tulos.jonokohtaisetTulostiedot.map { t =>
  ...
  private def näytäHyväksyttyäJulkaisematontaAlemmistaKorkeinHyvaksyttyOdottamassaYlempiä(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
    ...
    tuloksetWithIndex.map {
      case (tulos, index) =>
        ...
        if (
          higherJulkaisematon && List(Valintatila.peruuntunut, Valintatila.kesken).contains(
            tulos.valintatila
          )
        ) {
          val jonoJostaOliHyvaksyttyJulkaistu: Option[ValintatapajonoOid] = valinnantulosRepository
            .getViimeisinValinnantilaMuutosHyvaksyttyJaJulkaistuJonoOidHistoriasta(
              hakemusOid,
              tulos.hakukohdeOid
            )
          val wasHyvaksyttyJulkaistu = jonoJostaOliHyvaksyttyJulkaistu.nonEmpty
          val existsHigherHyvaksyttyJulkaistu = tuloksetWithIndex.exists(twi =>
            twi._2 < index && twi._1.valintatila.equals(
              Valintatila.hyväksytty
            ) && twi._1.julkaistavissa
          )
          if (wasHyvaksyttyJulkaistu && !existsHigherHyvaksyttyJulkaistu && !firstChanged) {
            logger.info(
              s"Merkitään aiemmin hyväksyttynä ollut ${tulos.valintatila} hyväksytyksi koska ylemmän hakutoiveen tuloksia ei ole vielä julkaistu. Index {}, tulos {}",
              index,
              tulos
            )
            firstChanged = true
            tulos.copy(
              ...
              jonokohtaisetTulostiedot = tulos.jonokohtaisetTulostiedot.map {
                jonokohtainenTulostieto =>
                  if (
                    jonoJostaOliHyvaksyttyJulkaistu.get == jonokohtainenTulostieto.oid && jonokohtainenTulostieto.valintatila == Valintatila.peruuntunut
                  ) {
                    jonokohtainenTulostieto
                      .copy(valintatila = Valintatila.hyväksytty, tilanKuvaukset = None)
                  } else {
                    jonokohtainenTulostieto
                  }
              }
            ...
            logger.debug(
              "näytäHyväksyttyäJulkaisematontaAlemmistaKorkeinHyvaksyttyOdottamassaYlempiä {}",
              tulos.valintatila
            )
            tulos
          }
  ...
  private def piilotaKuvauksetKeskeneräisiltä(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
  ...
  private def näytäVarasijaltaHyväksytytHyväksyttyinäJosVarasijasäännötEiVoimassa(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
    ...
    tulokset.map {
      case tulos
          if tulos.valintatila == Valintatila.varasijalta_hyväksytty && !ehdollinenVastaanottoMahdollista(
            ohjausparametrit
          ) =>
        logger.debug(
          "näytäVarasijaltaHyväksytytHyväksyttyinäJosVarasijasäännötEiVoimassa valintatila > hyväksytty"
        )
  ...
  private def peruuntumisenSyyksiHyväksyttyToisessaJonossa(
    hakemusOid: HakemusOid,
    hakukohdeOid: HakukohdeOid,
    jonokohtainenTulostieto: JonokohtainenTulostieto
  ): JonokohtainenTulostieto = {
    if (
      jonokohtainenTulostieto.valintatila == Valintatila.peruuntunut && !jonokohtainenTulostieto.tilanKuvaukset
        .map(_.asJava)
        .contains(TilanKuvaukset.peruuntunutHyvaksyttyToisessaJonossa)
    ) {
      val uusiSyy: Some[Map[String, String]] = Some(
        TilanKuvaukset.peruuntunutHyvaksyttyToisessaJonossa.asScala.toMap
      )
      logger.info(
        s"Vaihdetaan peruuntumisen syyn ilmoittavat tilankuvaukset peruuntuneelta tulokselta " +
  ...
  private def muutaJonojenPeruuntumistenSyytHakukohteissaJoissaOnHyväksyttyTulos(
    hakemusOid: HakemusOid,
    tulokset: List[Hakutoiveentulos],
    haku: Haku,
    ohjausparametrit: Ohjausparametrit
    ...
    tulokset.map {
      case tulos if tulos.valintatila == Valintatila.hyväksytty =>
        tulos.copy(jonokohtaisetTulostiedot =
          tulos.jonokohtaisetTulostiedot.map(
            peruuntumisenSyyksiHyväksyttyToisessaJonossa(hakemusOid, tulos.hakukohdeOid, _)
          )
        )
      case tulos =>
        tulos
  ...
  case class Välitulos(
    ...
    def map(
      f: (HakemusOid, List[Hakutoiveentulos], Haku, Ohjausparametrit) => List[Hakutoiveentulos]
    ): Välitulos = {
...
object ValintatulosService {
  def toVirkailijaTila(
    valintatuloksenTilaForHakija: ValintatuloksenTila,
    hakijanVastaanototHaussa: Option[Set[VastaanottoRecord]],
    hakukohdeOid: HakukohdeOid
  ): ValintatuloksenTila = {

    def merkittyMyohastyneeksi(v: VastaanottoRecord): Boolean =
      v.hakukohdeOid == hakukohdeOid && v.action == MerkitseMyohastyneeksi
    if (
      valintatuloksenTilaForHakija == ValintatuloksenTila.EI_VASTAANOTETTU_MAARA_AIKANA && !hakijanVastaanototHaussa
        .exists(_.exists(merkittyMyohastyneeksi))
    ) {
      ValintatuloksenTila.KESKEN
    } else {
