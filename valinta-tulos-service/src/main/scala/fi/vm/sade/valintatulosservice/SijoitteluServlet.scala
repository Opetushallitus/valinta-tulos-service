...
class SijoitteluServlet(
  ...
  lazy val getSijoitteluajoSwagger: OperationBuilder =
    (apiOperation[Unit]("getSijoitteluajoSwagger")
      summary "Hakee koko sijoitteluajon tiedot."
      parameter pathParam[String]("hakuOid").description(
        "Haun yksilöllinen tunniste"
      ) //TODO tarpeeton?
  ...
  lazy val getSijoitteluajonPerustiedotSwagger: OperationBuilder =
    (apiOperation[Unit]("getSijoitteluajoSwagger")
  ...
  get("/:hakuOid/sijoitteluajo/:sijoitteluajoId/perustiedot", operation(getSijoitteluajoSwagger)) {
    ...
    Ok(
      JsonFormats.javaObjectToJsonString(
        sijoitteluService.getSijoitteluajonPerustiedot(hakuOid, sijoitteluajoId, auditInfo)
      )
    )
  }

  lazy val getHakemusBySijoitteluajoSwagger: OperationBuilder =
    (apiOperation[Unit]("getHakemusBySijoitteluajoSwagger")
  ...
  get(
    ...
    operation(getHakemusBySijoitteluajoSwagger)
  ) {
    implicit val authenticated = authenticate
    authorize(
    ...
    ) //TODO: organization hierarchy check?!?!
    val hakuOid = HakuOid(params("hakuOid"))
    val sijoitteluajoId = params("sijoitteluajoId")
    ...
    try {
      Ok(
        JsonFormats.javaObjectToJsonString(
          sijoitteluService.getHakemusBySijoitteluajo(
            hakuOid,
            sijoitteluajoId,
            hakemusOid,
            auditInfo
  ...
  lazy val sijoitteluajoExistsForHakuJonoSwagger: OperationBuilder =
    (apiOperation[Unit]("sijoitteluajoExistsForHakuJonoSwagger")
