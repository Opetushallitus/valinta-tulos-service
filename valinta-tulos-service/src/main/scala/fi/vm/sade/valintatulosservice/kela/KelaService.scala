...
import fi.vm.sade.valintatulosservice.valintarekisteri.db.{VirkailijaVastaanottoRepository, VastaanottoRecord}
...
class KelaService(hakijaResolver: HakijaResolver, hakuService: HakuService, organisaatioService: OrganisaatioService, vastaanottoRepository: VirkailijaVastaanottoRepository) {
  ...
  def fetchVastaanototForPersonWithHetu(hetu: String, alkaen: Option[Date]): Option[Henkilo] = {
    ...
    henkilo match {
      case Some(henkilo) =>
        val vastaanotot = vastaanottoRepository.findHenkilonVastaanotot(henkilo.oidHenkilo, alkaen)
