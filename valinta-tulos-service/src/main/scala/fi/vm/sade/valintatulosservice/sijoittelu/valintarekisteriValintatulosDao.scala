...
class ValintarekisteriValintatulosDaoImpl(valinnantulosRepository: ValinnantulosRepository)
    extends ValintarekisteriValintatulosDao
    with Logging {

  private def run[R](operations: slick.dbio.DBIO[R]): R =
    valinnantulosRepository.runBlocking(operations)
  ...
  override def loadValintatuloksetForHakukohde(hakukohdeOid: HakukohdeOid): List[Valintatulos] =
    run(valinnantulosRepository.getValinnantuloksetForHakukohde(hakukohdeOid))
      .map(_.toValintatulos())
      .toList

  override def loadValintatuloksetForHakemus(hakemusOid: HakemusOid): List[Valintatulos] =
    run(valinnantulosRepository.getValinnantuloksetForHakemus(hakemusOid))
      .map(_.toValintatulos())
      .toList
}
