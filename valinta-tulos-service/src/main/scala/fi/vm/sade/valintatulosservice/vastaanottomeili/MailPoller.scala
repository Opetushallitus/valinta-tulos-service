...
class MailPoller(mailPollerRepository: MailPollerRepository,
  ...
  private def mailStatusFor(hakemus: Hakemus,
    ...
    hakemus match {
      case Hakemus(_, _, _, asiointikieli, _, Henkilotiedot(Some(kutsumanimi), Some(email), hasHetu, _, _, _, _), _) =>
      ...
      case Hakemus(_, _, _, _, _, Henkilotiedot(None, None, _, _, _, _, _), _) =>
        logger.error(s"Hakemus ${hakemus.oid} is missing hakemus.henkilotiedot.kutsumanimi and hakemus.henkilotiedot.email")
        None
      case Hakemus(_, _, _, _, _, Henkilotiedot(None, Some(email), _, _, _, _, _), _) =>
        logger.error(s"Hakemus ${hakemus.oid} is missing hakemus.henkilotiedot.kutsumanimi")
        None
      case Hakemus(_, _, _, _, _, Henkilotiedot(Some(kutsumanimi), None, _, _, _, _, _), _) =>
