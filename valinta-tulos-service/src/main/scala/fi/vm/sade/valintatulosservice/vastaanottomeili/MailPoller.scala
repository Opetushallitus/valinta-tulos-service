...
class MailPoller(
  ...
  private val emptyHakukohdeRecheckInterval: Duration =
    vtsApplicationSettings.mailPollerResultlessHakukohdeRecheckInterval
  private val hakemusRecheckIntervalHours: Int =
    vtsApplicationSettings.mailPollerHakemusRecheckInterval

  def pollForAllMailables(
    ...
    val hakukohdeOidsWithTheirHakuOids: ParSeq[(HakuOid, HakukohdeOid)] =
      timed(fetchHakusTaskLabel, 1000) {
        etsiHaut.par
      }

    pollForHakukohdes(
      hakukohdeOidsWithTheirHakuOids,
      ignoreEarlier = false,
      mailDecorator,
      mailablesWanted,
      timeLimit
  ...
  def pollForMailablesForHaku(
    ...
    val hakukohdeOidsWithTheirHakuOids: ParSeq[(HakuOid, HakukohdeOid)] =
      timed(fetchHakuOidsForHakukohdeOids, 1000) {
  ...
  def pollForMailablesForHakukohde(
    ...
    val hakukohdeOidsWithTheirHakuOids: ParSeq[(HakuOid, HakukohdeOid)] = List(
  ...
  def pollForMailablesForValintatapajono(
    hakukohdeOid: HakukohdeOid,
    ...
    pollForHakukohdes(
      ...
      mailablesWanted,
      timeLimit,
      valintatapajonoFilter = Some(jonoOid)
    )
  }
  ...
  def pollForMailablesForHakemus(
    ...
    val mailables: List[Ilmoitus] = {
      ...
      val hakutoiveCheckedCountsAndMailables: List[(Int, List[Ilmoitus])] = hakemus.toiveet.map {
        hakutoive =>
          getMailablesForHakemuses(hakemuksetByOid, hakutoive.oid, candidates, 1, mailDecorator)
      }
      val (checkedCandidatesCount: Int, mailables: List[Ilmoitus]) = (
        hakutoiveCheckedCountsAndMailables.map(_._1).sum,
        hakutoiveCheckedCountsAndMailables.flatMap(_._2)
      )
      logger.info(
        s"${mailables.size} mailables from $checkedCandidatesCount candidates for hakemus $hakemusOid"
      )
      mailables
    }
  ...
  def markAsSent(ilmoituses: List[Ilmoitus]): Unit = {
    val set: Set[(HakemusOid, HakukohdeOid)] = ilmoituses.flatMap { r =>
      r.hakukohteet.map { h => (r.hakemusOid, h.oid) }
    }.toSet
    mailPollerRepository.markAsSent(set)
  }

  private def pollForHakukohdes(
    hakukohdeOidsWithTheirHakuOids: ParSeq[(HakuOid, HakukohdeOid)],
    ignoreEarlier: Boolean,
    mailDecorator: MailDecorator,
    mailablesWanted: Int,
    timeLimit: Duration,
    valintatapajonoFilter: Option[ValintatapajonoOid] = None
  ): PollResult = {
    ...
    val fetchMailablesTaskLabel =
      s"Fetching mailables for ${hakukohdeOidsWithTheirHakuOids.size} hakukohdes " +
    ...
    timed(fetchMailablesTaskLabel, 1000) {
      processHakukohteesForMailables(
        mailDecorator,
        useLimit,
        timeLimit,
        hakukohdeOidsWithTheirHakuOids,
        ignoreEarlier = ignoreEarlier,
        PollResult(),
        valintatapajonoFilter
      )
    }
  }
      ...
      .filter { haku =>
        ohjausparameteritService.ohjausparametrit(haku.oid) match {
          case Right(ohjausparametrit) if ohjausparametrit.hakukierrosPaattyy.isEmpty =>
            ...
            false
          case Right(Ohjausparametrit(_, _, _, Some(hakukierrosPaattyy), _, _, _, _))
              if hakukierrosPaattyy.isBeforeNow =>
            logger.debug(
              s"Pudotetaan haku ${haku.oid} koska hakukierros päättynyt $hakukierrosPaattyy"
            )
            false
          case Right(Ohjausparametrit(_, _, _, _, Some(tulostenJulkistusAlkaa), _, _, _))
              if tulostenJulkistusAlkaa.isAfterNow =>
            logger.info(
              s"Pudotetaan haku ${haku.oid} koska tulosten julkistus alkaa $tulostenJulkistusAlkaa"
            )
            false
          case Left(e) =>
            logger.error(
              s"Pudotetaan haku ${haku.oid} koska ohjausparametrien haussa tapahtui virhe",
              e
            )
            false
          case _ =>
  ...
  private def etsiHakukohteet(hakuOids: List[HakuOid]): List[(HakuOid, HakukohdeOid)] = {
    ...
    if (filtered.size != found.size) {
      logger.info(
        s"Pudotettiin ${found.size - filtered.size} hakukohdetta koska ne on tarkistettu viimeisten $emptyHakukohdeRecheckInterval aikana"
      )
    } else {
      logger.info(
        s"Ei pudotettavia viimeisten $emptyHakukohdeRecheckInterval aikana tarkistettuja hakukohteita"
      )
    }
  ...
  private def filterHakukohdesRecentlyChecked(
    hakuHakukohdePairs: List[(HakuOid, HakukohdeOid)]
  ): List[(HakuOid, HakukohdeOid)] = {
    val vastikaanTarkistetutHakukohteet: Set[HakukohdeOid] =
      mailPollerRepository.findHakukohdeOidsCheckedRecently(emptyHakukohdeRecheckInterval)

    hakuHakukohdePairs
      .filter { oids =>
        if (vastikaanTarkistetutHakukohteet.contains(oids._2)) {
          logger.info(
            s"Pudotetaan hakukohde ${oids._2}, koska se on tarkistettu viimeisten $emptyHakukohdeRecheckInterval aikana"
          )
          false
        } else {
          true
  ...
  @tailrec
  private def processHakukohteesForMailables(
    mailDecorator: MailDecorator,
    ...
    ignoreEarlier: Boolean,
    acc: PollResult,
    valintatapajonoFilter: Option[ValintatapajonoOid]
  ): PollResult = {
    if (hakukohdeOids.isEmpty || acc.size >= totalMailablesWanted) {
      logger.info(
        s"Returning ${acc.size} mailables for totalMailablesWanted of $totalMailablesWanted"
      )
      acc.copy(isPollingComplete = true)
    } else if (acc.exceeds(timeLimit)) {
      logger.warn(
        s"Finding $totalMailablesWanted mailables has taken more than the time limit of $timeLimit . " +
      ...
      var hakukohteesToProcessOnThisIteration =
        Math.min(Math.max(mailablesNeeded / 10, 1), hakukohdeOids.size)
      ...
      val mailablesNeededPerHakukohde =
        (mailablesNeeded / toPoll.size) + (mailablesNeeded % toPoll.size) :: List.fill(
          toPoll.size - 1
        )(mailablesNeeded / toPoll.size)
      assert(mailablesNeededPerHakukohde.size == toPoll.size)
      assert(mailablesNeededPerHakukohde.sum == mailablesNeeded)
      logger.debug(
        s"Prosessoidaan $hakukohteesToProcessOnThisIteration hakukohdetta. Näiden jälkeen jäljellä vielä ${rest.size}. " +
      ...
      val (oidsWithCandidatesLeft, checkedCandidatesCount, mailables) = toPoll
        .zip(mailablesNeededPerHakukohde)
        .map {
          case ((hakuOid, hakukohdeOid), n) =>
            val (checkedCandidatesCount, mailables) = timed(
              s"Fetching mailables for hakukohde $hakukohdeOid in haku $hakuOid, ignoreEarlier = $ignoreEarlier.",
              1000
            ) {
              searchAndCreateMailablesForSingleHakukohde(
                mailDecorator,
                n,
                hakuOid,
                hakukohdeOid,
            ...
            (
              if (mailables.size == n) { List((hakuOid, hakukohdeOid)) }
              else { List.empty },
              checkedCandidatesCount,
              mailables
            )
        }
        .reduce[(List[(HakuOid, HakukohdeOid)], Int, List[Ilmoitus])] {
          case (t, tt) => (t._1 ++ tt._1, t._2 + tt._2, t._3 ++ tt._3)
        }
      ...
      processHakukohteesForMailables(
        ...
        ignoreEarlier,
        acc.copy(
          candidatesProcessed = acc.candidatesProcessed + checkedCandidatesCount,
          mailables = acc.mailables ++ mailables
        ),
  ...
  private def searchAndCreateMailablesForSingleHakukohde(
    mailDecorator: MailDecorator,
    limit: Int,
    hakuOid: HakuOid,
    hakukohdeOid: HakukohdeOid,
    ...
    val logMsg =
      s"Vastaanottopostien lähetyksen kandidaattien haku hakukohteessa $hakukohdeOid haussa $hakuOid"
    val candidates: Set[MailableCandidate] = timed(logMsg, 1000) {
      mailPollerRepository.candidates(
        hakukohdeOid,
        ignoreEarlier = ignoreEarlier,
        recheckIntervalHours = hakemusRecheckIntervalHours
      )
    }
    ...
    val (checkedCandidatesCount, mailables) = getMailablesForHakemuses(
      hakemuksetByOid,
      hakukohdeOid,
      candidates,
      limit,
    ...
    val logMessage =
      s"${mailables.size} mailables from $checkedCandidatesCount candidates for hakukohde $hakukohdeOid in haku $hakuOid"
  ...
  private def getMailablesForHakemuses(
    hakemuksetByOid: Map[HakemusOid, Hakemus],
    hakukohdeOid: HakukohdeOid,
    ...
    val (checkedCandidates, mailableStatii, mailables) =
      timed(
        s"Vastaanottopostien statuksien hakeminen ${candidates.size} kandidaatille hakukohteessa $hakukohdeOid",
        1000
      ) {
        candidates
          .groupBy(_._1)
          .foldLeft((Set.empty[HakemusOid], Set.empty[HakemusMailStatus], List.empty[Ilmoitus]))({
            case ((candidatesAcc, hakemusMailStatiiAcc, mailablesAcc), (hakemusOid, mailReasons))
                if mailablesAcc.size < limit =>
              (for {
                hakemus <- hakemuksetByOid.get(hakemusOid)
                hakemuksentulos <- fetchHakemuksentulos(hakemus)
                hakemusMailStatii <- mailStatusFor(
                  hakemus,
                  hakukohdeOid,
                  hakemuksentulos,
                  mailReasons.map(m => m._2 -> (m._3, m._4)).toMap,
                  valintatapajonoFilter
                )
    ...
    timed(
      s"Lähetettäväksi merkitseminen ${mailableStatii.size} vastaanottopostin statukselle hakukohteessa $hakukohdeOid",
      1000
    ) {
  ...
  private def mailStatusFor(
    ...
    hakemus match {
      case Hakemus(
            ...
            asiointikieli,
            _,
            Henkilotiedot(Some(kutsumanimi), Some(email), hasHetu)
          ) =>
        val hakukohteenToiveet = hakemuksenTulos.hakutoiveet.filter(_.hakukohdeOid == hakukohdeOid)
        val filteredHakutoiveet = hakukohteenToiveet.filter(toive =>
          valintatapajonoFilter.isEmpty || valintatapajonoFilter.get.equals(
            toive.valintatapajonoOid
          )
        )
        if (filteredHakutoiveet.size != hakukohteenToiveet.size) {
          logger.info(
            s"Suodatettiin pois ${hakukohteenToiveet.size - filteredHakutoiveet.size} hakutoivetta hakemuksen ${hakemus.oid} statuksesta valintatapajonosuodattimella $valintatapajonoFilter"
          )
        }
        val hakukohdeMailStatii =
          mailStatiiForHakutoiveet(hakemus.oid, filteredHakutoiveet, mailReasons)
        Some(
          HakemusMailStatus(
      ...
      case Hakemus(_, _, _, _, _, Henkilotiedot(None, None, _)) =>
        logger.error(
          s"Hakemus ${hakemus.oid} is missing hakemus.henkilotiedot.kutsumanimi and hakemus.henkilotiedot.email"
        )
        None
  ...
  private def mailStatiiForHakutoiveet(
    hakemusOid: HakemusOid,
    filteredHakutoiveet: List[Hakutoiveentulos],
    mailReasons: Map[HakukohdeOid, (Option[MailReason], Boolean)]
  ): List[HakukohdeMailStatus] = {
    ...
    if (reasonsToMail.isEmpty) {
      logger.info(
        s"Ei hakutoiveita joilla syytä sähköpostin lähetykselle hakemuksella ${hakemusOid}"
      )
    }
  ...
  private def getReasonToMail(
    ...
    hasPreviouslySent: Boolean
  ): Option[MailReason] = {
    if (!hasPreviouslySent && mailReason.nonEmpty) {
      logger.warn(
        s"Hakemuksella $hakemusOid hakutoiveella $hakukohdeOid oli aiempi syy viestin lähetykselle mutta ei" +
    ...
    if (
      Vastaanotettavuustila.isVastaanotettavissa(
        vastaanotettavuustila
      ) && (!hasPreviouslySent || !mailReason.contains(Vastaanottoilmoitus))
    ) {
      Some(Vastaanottoilmoitus)
    } else if (
      vastaanotonIlmoittaja.contains(
        Sijoittelu
      ) && vastaanottotila == Vastaanottotila.vastaanottanut && (!hasPreviouslySent || !mailReason
        .contains(SitovanVastaanotonIlmoitus))
    ) {
      Some(SitovanVastaanotonIlmoitus)
    } else if (
      vastaanotonIlmoittaja.contains(
        Sijoittelu
      ) && vastaanottotila == Vastaanottotila.ehdollisesti_vastaanottanut && (!hasPreviouslySent || !mailReason
        .contains(EhdollisenPeriytymisenIlmoitus))
    ) {
      Some(EhdollisenPeriytymisenIlmoitus)
    } else {
      logger.info(
        s"Hakemuksella $hakemusOid ei syytä sähköpostin lähetykseelle hakutoiveella $hakukohdeOid. " +
  ...
  private def markAsToBeSent(mailableStatii: Set[HakemusMailStatus]): Unit = {
    mailPollerRepository.markAsToBeSent(
      mailableStatii
        ...
        .collect {
          case (hakemusOid, hakukohdeOid, Some(reason)) => (hakemusOid, hakukohdeOid, reason)
        }
    )
  }
  ...
  def deleteIncompleteMailEntries(): Unit = {
    ...
    if (deletedSet.isEmpty) {
      ...
      logger.warn(
        s"Cleaned up ${deletedSet.size} incomplete mail entries: ${deletedSet.mkString(", ")}"
      )
    }
  }
