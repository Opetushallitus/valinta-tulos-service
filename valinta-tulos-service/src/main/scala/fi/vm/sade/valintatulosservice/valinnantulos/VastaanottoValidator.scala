...
trait VastaanottoValidator {
  ...
  def error(valinnantulos:Valinnantulos, msg:String) = Left(new ValinnantulosUpdateStatus(400, msg, valinnantulos.valintatapajonoOid, valinnantulos.hakemusOid))
  ...
  def onkoEhdollisestiVastaanotettavissa(valinnantulos: Valinnantulos): Boolean

  def validateVastaanotto(uusi: Valinnantulos, vanha: Valinnantulos): Either[ValinnantulosUpdateStatus, Unit] = validateVastaanotto(uusi, Some(vanha))

  def validateVastaanotto(uusi: Valinnantulos, vanha: Option[Valinnantulos]): Either[ValinnantulosUpdateStatus, Unit] = {
