...
class ValintaesitysService(
  ...
  def get(hakukohdeOid: HakukohdeOid, auditInfo: AuditInfo): Set[Valintaesitys] = {
    ...
    val valintaesitykset =
      valinnantulosRepository.runBlocking(valintaesitysRepository.get(hakukohdeOid))
    auditlogGet(valintaesitykset, auditInfo)
    valintaesitykset
  }

  def hyvaksyValintaesitys(
    valintatapajonoOid: ValintatapajonoOid,
    auditInfo: AuditInfo
  ): Valintaesitys = {
    val valintaesitys = valinnantulosRepository
  ...
  private def authorizeGet(hakukohdeOid: HakukohdeOid, session: Session): Unit = {
    (for {
      tarjoajaOids <-
        hakuService.getHakukohde(hakukohdeOid).right.map(_.organisaatioOiditAuktorisointiin).right
      _ <-
        authorizer
          .checkAccess(
            session,
  ...
  private def authorizeValintaesityksenHyvaksyminen(
    ...
    (for {
      tarjoajaOids <-
        hakuService
          .getHakukohde(valintaesitys.hakukohdeOid)
          .right
          .map(_.organisaatioOiditAuktorisointiin)
          .right
      _ <-
        authorizer
          .checkAccess(
  ...
  private def hyvaksyValintaesitys(
    valintatapajonoOid: ValintatapajonoOid,
    session: Session
  ): DBIO[Valintaesitys] = {
    for {
      ...
      _ <- valinnantulosRepository.setJulkaistavissa(
        valintatapajonoOid,
        session.personOid,
        "Valintaesityksen hyväksyntä"
      )
      _ <- valinnantulosRepository.setHyvaksyttyJaJulkaistavissa(
        valintatapajonoOid,
        session.personOid,
        "Valintaesityksen hyväksyntä"
      )
  ...
  private def auditlogValintaesityksenHyvaksyminen(
    valintaesitys: Valintaesitys,
    auditInfo: AuditInfo
  ): Unit = {
    audit.log(
