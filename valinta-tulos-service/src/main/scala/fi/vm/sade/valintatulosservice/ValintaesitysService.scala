...
class ValintaesitysService(hakuService: HakuService,
  ...
  private def authorizeGet(hakukohdeOid: HakukohdeOid, session: Session): Unit = {
    (for {
      ...
      _ <- authorizer.checkAccess(session, tarjoajaOids, Set (Role.SIJOITTELU_READ, Role.SIJOITTELU_READ_UPDATE, Role.SIJOITTELU_CRUD), hakukohdeOid).right
  ...
  private def authorizeValintaesityksenHyvaksyminen(valintaesitys: Valintaesitys, session: Session): DBIO[Valintaesitys] = {
    (for {
      ...
      _ <- authorizer.checkAccess(session, tarjoajaOids, Set (Role.SIJOITTELU_READ_UPDATE, Role.SIJOITTELU_CRUD), valintaesitys.hakukohdeOid).right
