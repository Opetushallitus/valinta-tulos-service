...
import fi.vm.sade.valintatulosservice.valintaperusteet.{
  ValintaPerusteetServiceImpl,
...
object VtsAppConfig extends Logging {
  ...
  private val embeddedMongoPortChooser = new PortFromSystemPropertyOrFindFree(
    "valintatulos.embeddedmongo.port"
  )
  ...
  def fromString(profile: String) = {
    ...
    profile match {
      ...
      case name =>
        throw new IllegalArgumentException("Unknown value for valintatulos.profile: " + name);
    }
  }
  ...
  /**
    ...
    */
  class LocalTestingWithTemplatedVars(
    val templateAttributesFile: String = System.getProperty("valintatulos.vars")
  ) extends VtsAppConfig
      with TemplatedProps
      with CasSecurity {
  ...
  class Dev
      extends VtsAppConfig
      with ExampleTemplatedProps
      with CasSecurity
      with StubbedExternalDeps {
    override val ophUrlProperties: OphUrlProperties = {
      val ps = new DevOphUrlProperties(propertiesFile)
      ps.addOverride(
        "ataru-service.applications",
        s"http://localhost:$vtsMockPort/valinta-tulos-service/util/ataru/applications"
      )
      ps.addOverride(
        "oppijanumerorekisteri-service.henkilotByOids",
        s"http://localhost:$vtsMockPort/valinta-tulos-service/util/oppijanumerorekisteri/henkilot"
      )
      ps
    }
  ...
  class IT_sysprops extends IT {
    override val ophUrlProperties: OphUrlProperties = new OphUrlProperties(
      propertiesFile,
      false,
      Some(
        System.getProperty(
          "valinta-tulos-service.it-profile.hostname",
  ...
  class IT extends ExampleTemplatedProps with StubbedExternalDeps with MockSecurity {
    override val ophUrlProperties: OphUrlProperties = {
      ...
      ps.addOverride(
        ...
        "http://localhost:" + vtsMockPort + "/valinta-tulos-service/util/kayttooikeus/userdetails/$1"
      )
      ps
    }
    ...
    override lazy val settings = loadSettings
      .withOverride(
        ("hakemus.mongodb.uri", "mongodb://localhost:" + embeddedMongoPortChooser.chosenPort)
      )
      .withOverride(
      ...
      .withOverride(
        (
          "cas.service.organisaatio-service",
          s"http://localhost:${organisaatioMockPort}/organisaatio-service"
        )
      )
      .withOverride(
        (
          "cas.service.valintaperusteet-service",
          s"http://localhost:${valintaPerusteetMockPort}/valintaperusteet-service"
        )
      )
      .withOverride(("cas.url", s"https://itest-virkailija.oph.ware.fi/cas"))
      .withOverride(
        (
          "valinta-tulos-service.cas.service",
          s"http://localhost:${vtsMockPort}/valinta-tulos-service"
        )
      )
  ...
  class IT_externalHakemus extends IT {
    override lazy val settings = loadSettings
      .withOverride(
        "hakemus.mongodb.uri",
        "mongodb://localhost:" + System.getProperty("hakemus.embeddedmongo.port", "28018")
      )
      .withOverride(
        ("valinta-tulos-service.valintarekisteri.ensikertalaisuus.max.henkilo.oids", "100")
      )
    ...
    override def importFixturesToHakemusDatabase {
      /* Don't import initial fixtures, as database is considered external */
    }
  }

  class IT_disabledIlmoittautuminen extends IT {
    override lazy val settings =
      loadSettings.withOverride("valinta-tulos-service.ilmoittautuminen.enabled", "")
  }
  ...
  trait MockSecurity extends VtsAppConfig {
    lazy val securityContext: SecurityContext = {
      new MockSecurityContext(
        ...
        Map(
          "testuser" -> KayttooikeusUserDetails(
            settings.securitySettings.requiredRoles.map(role => Role(role)).toSet,
            "mockoid"
          ),
          "sijoitteluUser" -> KayttooikeusUserDetails(
            List(
              "APP_VALINTATULOSSERVICE_CRUD",
              "APP_SIJOITTELU_CRUD",
              "APP_SIJOITTELU_CRUD_123.123.123.123"
            ).map(role => Role(role)).toSet,
