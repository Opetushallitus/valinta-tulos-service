...
case class SijoitteluFixtures(valintarekisteriDb: ValintarekisteriDb) extends json4sCustomFormats {
  ...
  implicit val formats = DefaultFormats ++ List(new NumberLongSerializer) ++ Oids.getSerializers()
  ...
  private def importJsonFixturesToPostgres(
    fixtureName: String,
    yhdenPaikanSaantoVoimassa: Boolean = false,
    kktutkintoonJohtava: Boolean = false
  ): Unit = {
    val text = scala.io.Source
      .fromInputStream(new ClassPathResource("fixtures/sijoittelu/" + fixtureName).getInputStream)
      .mkString
    val json = parse(text)
    SijoitteluWrapper.fromJson(json) match {
      case Some(wrapper) =>
        wrapper.hakukohteet.foreach(h =>
          storeHakukohde(
            h.getOid,
            wrapper.sijoitteluajo.getHakuOid,
            wrapper.sijoitteluajo.getSijoitteluajoId,
            h.isKaikkiJonotSijoiteltu,
  ...
  private def storeValintatulokset(
    ...
    valintatulokset.foreach(tulos => {
      valintarekisteriDb.runBlocking(
        valintarekisteriDb.storeValinnantuloksenOhjaus(
    ...
    hakukohteet.foreach(hakukohde =>
      hakukohde.getValintatapajonot.asScala.foreach(valintatapajono =>
        valintatapajono.getHakemukset.asScala
          .filter(h =>
            !valintatulokset.exists(t =>
              t.getValintatapajonoOid == valintatapajono.getOid && t.getHakemusOid == h.getHakemusOid
            )
          )
          .foreach(hakemus => {
            valintarekisteriDb.runBlocking(
  ...
  private def storeEhdollisenHyvaksynnanEhto(json: JValue) = {
    ...
    for (valintatulos <- valintatulokset) {
      if ((valintatulos \ "ehdollisestiHyvaksyttavissa").extractOpt[Boolean].contains(true)) {
        ...
        valintarekisteriDb.runBlocking(
          valintarekisteriDb.insertHyvaksynnanEhtoValintatapajonossa(
            hakemusOid,
            valintatapajonoOid,
            hakukohdeOid,
            ehto
  ...
  private def storeHakukohde(
    ...
    valintarekisteriDb.storeHakukohde((yhdenPaikanSaantoVoimassa, kktutkintoonJohtava) match {
      ...
      case (false, true) =>
        EiYPSHakukohde(HakukohdeOid(hakukohdeOid), HakuOid(hakuOid), Kevat(2016))
      case (false, false) =>
  ...
  private def getVastaanottoAction(vastaanotto: String) =
    vastaanotto match {
      case "KESKEN"                        => None
