...
class CasSessionService(
  ...
  private def validateServiceTicket(ticket: ServiceTicket): Either[Throwable, String] = {
    logger.info(
      "validateServiceTicket: using timeout value: " + securityContext.validateServiceTicketTimeout
    )
    val ServiceTicket(s) = ticket
    securityContext.casClient
      .validateServiceTicket(serviceIdentifier)(s)
      .handleWith {
        case NonFatal(t) =>
          Task.fail(new AuthenticationFailedException(s"Failed to validate service ticket $s", t))
      }
      .attemptRunFor(securityContext.validateServiceTicketTimeout)
      .toEither
  }

  private def storeSession(
    ticket: ServiceTicket,
    user: KayttooikeusUserDetails
  ): Either[Throwable, (UUID, Session)] = {

    val session = CasSession(ticket, user.oid, user.roles)
    logger.debug(
      "Storing to session:" + session.casTicket + " " + session.personOid + " " + session.roles
    )
  ...
  private def createSession(ticket: ServiceTicket): Either[Throwable, (UUID, Session)] = {
    validateServiceTicket(ticket).right
