...
class ValintatulosServiceLisahakuSpec extends ITSpecification with TimeWarp {
  ...
  "ValintaTulosService" should {
    ...
    "yhteishaun lisähaku korkeakouluihin" in {
      ...
      "ei tuloksia, ei julkaistavaa" in {
        ...
        checkHakutoiveState(
          getHakutoive("1.2.246.562.14.2013120515524070995659"),
          Valintatila.kesken,
        ...
        checkHakutoiveState(
          getHakutoive("1.2.246.562.14.2014022408541751568934"),
          Valintatila.kesken,
          Vastaanottotila.kesken,
          Vastaanotettavuustila.ei_vastaanotettavissa,
          false
      ...
      "molemmat vastaanotettavissa" in {
        useFixture(
          ...
          hakemusFixtures = hakemusFixtures
        )
        ...
        hakutoiveentulos1.jonokohtaisetTulostiedot.head.valintatila must beEqualTo(
          Valintatila.hyväksytty
        )
        ...
        hakutoiveentulos2.jonokohtaisetTulostiedot.head.valintatila must beEqualTo(
          Valintatila.hyväksytty
        )
      ...
      "ensimmäinen vastaanotettavissa, toinen ei hyväksytty" in {
        useFixture(
          "lisahaku-vastaanotettavissa-2-ensimmainen.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = hakemusFixtures
        )
        checkHakutoiveState(
      ...
      "toinen vastaanotettavissa, ensimmäinen ei hyväksytty" in {
        useFixture(
          "lisahaku-vastaanotettavissa-2-toinen.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = hakemusFixtures
        )
        checkHakutoiveState(
      ...
      "toinen vastaanotettu, ensimmäistä ei voi vastaanottaa" in {
        useFixture(
          "lisahaku-vastaanottanut.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = hakemusFixtures,
          yhdenPaikanSaantoVoimassa = true,
          kktutkintoonJohtava = true
        )
        val hakutoiveOttanutVastaanToisenPaikan =
          getHakutoive("1.2.246.562.14.2013120515524070995659")
        checkHakutoiveState(
          hakutoiveOttanutVastaanToisenPaikan,
        ...
        hakutoiveOttanutVastaanToisenPaikan.jonokohtaisetTulostiedot.forall(
          jonokohtainenTulostieto => jonokohtainenTulostieto.valintatila == Valintatila.peruuntunut
        ) must beTrue
        ...
        hakutoiveHyväksytty.jonokohtaisetTulostiedot.forall(jonokohtainenTulostieto =>
  ...
  def getHakutoive(hakukohdeOidSuffix: String) =
    hakemuksenTulos.hakutoiveet.find { _.hakukohdeOid.toString.endsWith(hakukohdeOidSuffix) }.get
  ...
  def checkHakutoiveState(
    ...
    (
      ...
      hakuToive.julkaistavissa
    ) must_== (expectedTila, vastaanottoTila, vastaanotettavuustila, julkaistavissa)
  }
}
