...
object EnsikertalaisuusBatchAPITester extends App with Logging {
  ...
  val valintarekisteriDb = new ValintarekisteriDb(
  ...
  valintarekisteriDb.storeHakukohde(
    YPSHakukohde(
      HakukohdeOid("1.2.246.561.20.00000000001"),
      HakuOid("1.2.246.561.29.00000000001"),
      Kevat(2015)
    )
  )
  ...
  for (_ <- 1.to(5)) {
    ...
    val (_, _, result) = new DefaultHttpRequest(
      Http(
        s"http://localhost:${SharedJetty.port}/valinta-tulos-service/ensikertalaisuus?koulutuksenAlkamiskausi=2014K"
      )
        .method("POST")
        .options(DefaultHttpClient.defaultOptions)
        .header("Content-Type", "application/json")
        .postData(Serialization.write(fetchOids))
    ).responseWithHeaders()
    println(s"request of size ${fetchOids.size} took ${System.currentTimeMillis() - start} ms")
    start = System.currentTimeMillis()
    println(
      s"parsing response of size ${Serialization.read[List[Ensikertalaisuus]](result).size} took ${System
  ...
  private def insertHenkilos(): (List[String], List[String]) = {
    ...
    Await.result(
      valintarekisteriDb.db.run(SimpleDBIO[(List[String], List[String])](jdbcActionContext => {
          ...
          val x = oids.foldLeft((List.empty[String], List.empty[String]))((t, oid) => {
            ...
            if (writeVastaanottoEvent) {
              ...
              insertStatement.setString(
                2,
                if (isSitova) "VastaanotaSitovasti"
                else if (isEhdollinen) "VastaanotaEhdollisesti"
                else "Peru"
              )
  ...
  private def insertHenkiloviitteet(henkilot: (List[String], List[String])): Unit = {
    ...
    def groupHenkilot(
      vastaanottaneet: List[String],
      eiVastaanottaneet: List[String],
      groups: List[List[String]]
    ): List[List[String]] =
      vastaanottaneet match {
        case Nil => groups
        case oid :: rest =>
          val n = Random.nextInt(4) + 1
          groupHenkilot(
            rest,
            eiVastaanottaneet.drop(n),
            Random.shuffle(oid +: eiVastaanottaneet.take(n)) +: groups
          )
      }
    Await.ready(
      valintarekisteriDb.db.run(SimpleDBIO[Unit](jdbcActionContext => {
        ...
        try {
          for {
            group <-
              groupHenkilot(vastaanottaneet.take(testDataSize / 50), eiVastaanottaneet, List())
