...
class VastaanottoServiceVirkailijanaSpec extends ITSpecification with TimeWarp with ThrownMessages {
  ...
  val ilmoittautumisaikaPaattyy2100: Ilmoittautumisaika =
    Ilmoittautumisaika(None, Some(new DateTime(2100, 1, 10, 23, 59, 59, 999)))
  ...
  "vastaanotaVirkailijana" in {
    ...
    "vastaanota ehdollisesti yksi hakija" in {
      ...
      val r = vastaanotaVirkailijana(
        ...
        personOid,
        hakemusOid,
        alemmanHakutoiveenHakukohdeOid,
        hakuOid,
    ...
    "varalla olevaa hakutoivetta ei voi ottaa vastaan" in {
      ...
      val ehdollinenVastaanottoResult = vastaanotaVirkailijana(
        valintatapajonoOid,
        personOid,
        hakemusOid,
        ylinHakukohdeOid,
      ...
      val sitovaVastaanottoResult = vastaanotaVirkailijana(
        valintatapajonoOid,
        personOid,
        hakemusOid,
        ylinHakukohdeOid,
    ...
    "kahdesta hyväksytystä hakutoiveesta alempaa ei voi ottaa ehdollisesti vastaan" in {
      ...
      val alimmanToiveenEhdollinenVastaanottoResult = vastaanotaVirkailijana(
        valintatapajonoOid,
        personOid,
        hakemusOid,
        alinHakukohdeOid,
      ...
      alimmanToiveenEhdollinenVastaanottoResult.result.message must_== Some(
        "Hakutoivetta ei voi ottaa ehdollisesti vastaan"
      )
      ...
      val keskimmaisenToiveenEhdollinenVastaanottoResult = vastaanotaVirkailijana(
        valintatapajonoOid,
        personOid,
        hakemusOid,
        keskimmainenHakukohdeOid,
    ...
    "virkailija voi vastaanottaa peruutetun hakutoiveen" in {
      ...
      val peruutusResponse = vastaanotaVirkailijana(
        valintatapajonoOid,
        personOid,
        hakemusOid,
        vastaanotettavissaHakuKohdeOid,
      ...
      val sitovaResponse = vastaanotaVirkailijana(
        valintatapajonoOid,
        personOid,
        hakemusOid,
        vastaanotettavissaHakuKohdeOid,
    ...
    "vastaanota yksi hakija joka ottanut vastaan toisen kk paikan -> error" in {
      ...
      r.result.message must_== Some(
        "Ei voi tallentaa vastaanottotietoa, koska hakijalle näytettävä tila on \"PERUUNTUNUT\""
      )
      r.result.status must_== 400
      hakemuksenTulos
        .hakutoiveet(0)
        .vastaanottotila must_== Vastaanottotila.ottanut_vastaan_toisen_paikan
    }
  ...
  "vastaanotaVirkailijanaInTransaction" in {
    "älä vastaanota sitovasti hakijaa, kun toinen vastaanotto ei onnistu" in {
      ...
      vastaanotaVirkailijanaTransaktiossa(
        List(
          VastaanottoEventDto(
            valintatapajonoOid,
            personOid,
    ...
    "siirrä ehdollinen vastaanotto ylemmälle hakutoiveelle" in {
      ...
      useFixture(
        "hyvaksytty_ylempi_varalla_hyvaksy_peruuntunut.json",
        Nil,
        hakuFixture = HakuFixtures.korkeakouluYhteishaku,
        yhdenPaikanSaantoVoimassa = true,
        kktutkintoonJohtava = true
      ...
      useFixture(
        ...
        yhdenPaikanSaantoVoimassa = true,
        kktutkintoonJohtava = true,
        clearFixturesInitially = false
      )

      val `hakemuksentulosHakijanVastaanotonJa"Sijoittelun"Jalkeen` = hakemuksenTulos
      `hakemuksentulosHakijanVastaanotonJa"Sijoittelun"Jalkeen`
        .hakutoiveet(0)
      ...
      `hakemuksentulosHakijanVastaanotonJa"Sijoittelun"Jalkeen`
        .hakutoiveet(0)
        .vastaanottotila must_== Vastaanottotila.kesken
      `hakemuksentulosHakijanVastaanotonJa"Sijoittelun"Jalkeen`
        .hakutoiveet(1)
        .vastaanottotila must_== Vastaanottotila.ehdollisesti_vastaanottanut

      val vastaanotonTulos = vastaanotaVirkailijanaTransaktiossa(
        List(
          VastaanottoEventDto(
            valintatapajonoOid,
            personOid,
    ...
    "estä useampi vastaanotto kun yhden paikan sääntö on voimassa" in {
      ...
      vastaanotonTulos match {
        case Failure(cae: ConflictingAcceptancesException) =>
          cae.conflictingVastaanottos.map(_.hakukohdeOid) must_== Vector(
            alinHakukohdeOid,
            keskimmainenHakukohdeOid
          )
    ...
    "perunut hakija voidaan siirtää hyväksytyksi" in {
      ...
      val results: Iterable[VastaanottoResult] = vastaanotaVirkailijana(
        List(
          VastaanottoEventDto(
  ...
  lazy val ataruHakemusRepository = new AtaruHakemusRepository(appConfig) {
    override def getHakemukset(query: HakemuksetQuery): Either[Throwable, AtaruResponse] =
      Right(AtaruResponse(List.empty, None))
  }
  ...
  private def vastaanotaVirkailijanaTransaktiossa(
