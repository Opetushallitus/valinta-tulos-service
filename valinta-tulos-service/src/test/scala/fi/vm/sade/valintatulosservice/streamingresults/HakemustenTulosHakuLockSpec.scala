...
class HakemustenTulosHakuLockSpec extends Specification {
  ...
  private val operationLagMillis: Int = 10000
  ...
  private val fastLock: HakemustenTulosHakuLock = new HakemustenTulosHakuLock(
    queueLimit = 2,
    lockDuration = fastLockMillis,
    TimeUnit.MILLISECONDS
  )
  private val slowLock: HakemustenTulosHakuLock = new HakemustenTulosHakuLock(
    queueLimit = 2,
    lockDuration = slowLockMillis,
    TimeUnit.MILLISECONDS
  )
  ...
  "HakemustenTulosHakuLock" should {
    "control concurrent access" in {
      ...
      Await.result(fast2, resultTimeout) must beLeft(
        s"Acquiring lock timed out after $fastLockMillis milliseconds: No available capacity for this request, please try again later"
      )
      Await.result(fast3, resultTimeout) must beLeft(
        s"Acquiring lock timed out after $fastLockMillis milliseconds: No available capacity for this request, please try again later"
      )
      Await.result(fast4, resultTimeout) must beLeft(
        s"Results loading queue of size 2 full: No available capacity for this request, please try again later"
      )

      Await.result(slow1, resultTimeout) must beRight("ok")
      Await.result(slow2, resultTimeout) must beRight("ok")
      Await.result(slow3, resultTimeout) must beLeft(
        s"Acquiring lock timed out after $slowLockMillis milliseconds: No available capacity for this request, please try again later"
      )
    }
  }
