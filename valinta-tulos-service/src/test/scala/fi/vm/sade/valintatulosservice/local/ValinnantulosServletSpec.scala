...
class ValinnantulosServletSpec
    extends Specification
    with EmbeddedJettyContainer
    with HttpComponentsClient
    with BeforeAfterAll
    with ForEach[(String, ValinnantulosService, SessionRepository)]
    with ITSetup
    with Mockito {
  ...
  def foreach[R: AsResult](
    f: ((String, ValinnantulosService, SessionRepository)) => R
    ...
    val servlet = new ValinnantulosServlet(
      valinnantulosService,
      valintatulosService,
      sessionRepository,
      appConfig
    )(mock[Swagger])
    ServletTest.withServlet(
      this,
      servlet,
      (uri: String) => AsResult(f((uri, valinnantulosService, sessionRepository)))
    )
  }
  ...
  private val readSession =
    CasSession(ServiceTicket("ST"), "1.2.246.562.24.1", Set(Role.SIJOITTELU_READ))
  private val crudSession =
    CasSession(ServiceTicket("ST"), "1.2.246.562.24.1", Set(Role.SIJOITTELU_CRUD))
  private val now = Instant.now.truncatedTo(ChronoUnit.SECONDS)
  private val ifUnmodifiedSince =
    DateTimeFormatter.RFC_1123_DATE_TIME.format(ZonedDateTime.ofInstant(now, ZoneId.of("GMT")))
  private val userAgent = "ValinnantulosServletSpec-user-agent"
  private def auditInfo(session: Session) =
    AuditInfo((sessionId, session), InetAddress.getByName("127.0.0.1"), userAgent)
  private val defaultHeaders =
    Map("Cookie" -> s"session=${sessionId.toString}", "User-Agent" -> userAgent)
  ...
  private val valinnantulosWithHistoria = ValinnantulosWithTilahistoria(
    valinnantulos,
    List(TilaHistoriaRecord(valintatapajonoOid, hakemusOid, valinnantulos.valinnantila, date))
  )

  "GET /auth/valinnan-tulos" in {
    "palauttaa 401, jos sessiokeksi puuttuu" in {
      t: (String, ValinnantulosService, SessionRepository) =>
        get(
          t._1,
          Iterable("valintatapajonoOid" -> valintatapajonoOid.toString),
    ...
    "palauttaa 401, jos sessio ei ole voimassa" in {
      t: (String, ValinnantulosService, SessionRepository) =>
    ...
    "palauttaa 403, jos käyttäjällä ei ole lukuoikeuksia" in {
      t: (String, ValinnantulosService, SessionRepository) =>
    ...
    "palauttaa 200 ja tyhjän taulukon jos valinnan tuloksia ei löydy" in {
      t: (String, ValinnantulosService, SessionRepository) =>
        t._3.get(sessionId) returns Some(readSession)
        t._2.getValinnantuloksetForValintatapajono(
          ValintatapajonoOid("1"),
          auditInfo(readSession)
        ) returns None
    ...
    "palauttaa 200 ja valintatapajonon valinnan tulokset valintatapajono-oidilla haettaessa" in {
      t: (String, ValinnantulosService, SessionRepository) =>
        t._3.get(sessionId) returns Some(readSession)
        t._2.getValinnantuloksetForValintatapajono(
          valintatapajonoOid,
          auditInfo(readSession)
        ) returns Some((Instant.now, Set(valinnantulos)))
        get(t._1, Iterable("valintatapajonoOid" -> valintatapajonoOid.toString), defaultHeaders) {
          status must_== 200
    ...
    "palauttaa 200 ja hakukohteen valinnan tulokset hakukohdeoidilla haettaessa" in {
      t: (String, ValinnantulosService, SessionRepository) =>
        t._3.get(sessionId) returns Some(readSession)
        t._2.getValinnantuloksetForHakukohde(hakukohdeOid, auditInfo(readSession)) returns Some(
          (Instant.now, Set(valinnantulos))
        )
    ...
    "palauttaa 200 ja valintatapajonon valinnan tulokset hakukohde ja valintatapajono-oidilla haettaessa" in {
      t: (String, ValinnantulosService, SessionRepository) =>
        t._3.get(sessionId) returns Some(readSession)
        t._2.getValinnantuloksetForValintatapajono(
          valintatapajonoOid,
        ...
        get(
          t._1,
          Iterable(
            "hakukohdeOid" -> hakukohdeOid.toString,
            "valintatapajonoOid" -> valintatapajonoOid.toString
          ),
          defaultHeaders
        ) {
          status must_== 200
    ...
    "palauttaa " + appConfig.settings.headerLastModified + " otsakkeen jossa viimeisintä muutoshetkeä seuraava tasasekuntti" in {
      t: (String, ValinnantulosService, SessionRepository) =>
        val lastModified = DateTimeFormatter.RFC_1123_DATE_TIME.format(
          ZonedDateTime.ofInstant(now.plusSeconds(1), ZoneId.of("GMT"))
        )
        t._3.get(sessionId) returns Some(readSession)
        t._2.getValinnantuloksetForValintatapajono(
          valintatapajonoOid,
          auditInfo(readSession)
        ) returns Some((now, Set(valinnantulos)))
        get(t._1, Iterable("valintatapajonoOid" -> valintatapajonoOid.toString), defaultHeaders) {
          status must_== 200
  ...
  "PATCH /auth/valinnan-tulos" in {
    ...
    "palauttaa 400 jos " + appConfig.settings.headerIfUnmodifiedSince + " otsake puuttuu" in {
      t: (String, ValinnantulosService, SessionRepository) =>
    ...
    "palauttaa 409 jos tietoihin on tehty samanaikaisia muutoksia" in {
      t: (String, ValinnantulosService, SessionRepository) =>
        t._3.get(sessionId) returns Some(crudSession)
        t._2.storeValinnantuloksetAndIlmoittautumiset(
          any[ValintatapajonoOid],
          any[List[Valinnantulos]],
          any[Option[Instant]],
          any[AuditInfo]
    ...
    "palauttaa 200 ja tyhjän taulukon jos päivitys onnistui" in {
      t: (String, ValinnantulosService, SessionRepository) =>
        t._3.get(sessionId) returns Some(crudSession)
        t._2.storeValinnantuloksetAndIlmoittautumiset(
          valintatapajonoOid,
    ...
    "palauttaa 200 ja virhetiedon taulukossa jos päivitys epäonnistui" in {
      t: (String, ValinnantulosService, SessionRepository) =>
  ...
  "GET /auth/valinnan-tulos/hakemus/" in {
    ...
    "palauttaa 200 ja tyhjän taulukon jos hakemuksen tuloksia ei löydy" in {
      t: (String, ValinnantulosService, SessionRepository) =>
        t._3.get(sessionId) returns Some(readSession)
    ...
    "palauttaa 200 ja hakemuksen tulokset hakemus-oidilla haettaessa" in {
      t: (String, ValinnantulosService, SessionRepository) =>
        t._3.get(sessionId) returns Some(readSession)
        t._2.getValinnantuloksetForHakemus(hakemusOid, auditInfo(readSession)) returns Some(
          (Instant.now(), Set(valinnantulosWithHistoria))
        )
        get(t._1 + "/hakemus/", Iterable("hakemusOid" -> hakemusOid.toString), defaultHeaders) {
          status must_== 200
          parse(body)
            .extract[List[ValinnantulosWithTilahistoria]]
            .toString()
            .trim()
            .replace("\r", "") must_== List(valinnantulosWithHistoria)
            .toString()
            .trim()
            .replace("\r", "")
        }
