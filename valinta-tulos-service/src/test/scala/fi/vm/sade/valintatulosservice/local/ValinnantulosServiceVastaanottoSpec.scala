...
class ValinnantulosServiceVastaanottoSpec
    extends ITSpecification
    with TimeWarp
    with ThrownMessages
  ...
  "Vastaanota" should {
    "vastaanota sitovasti yksi hakija" in {
      useFixture(
        ...
        hakuFixture = HakuFixtures.korkeakouluYhteishaku
      )
      val valinnantulosForSave = findOne(hakemuksenValinnantulokset, tila(Hyvaksytty))
    ...
    "vastaanota ehdollisesti yksi hakija" in {
      ...
      tallenna(
        List(valinnantulos.copy(vastaanottotila = ValintatuloksenTila.EHDOLLISESTI_VASTAANOTTANUT))
      )
      val valinnantuloksetAfter = hakemuksenValinnantulokset
      valinnantuloksetAfter
        .filter(v => v.valinnantila == Varalla)
        .foreach(_.vastaanottotila must_== ValintatuloksenTila.KESKEN)
      val valinnantulosAfter = findOne(valinnantuloksetAfter, tila(Hyvaksytty))
      valinnantulosAfter.copy(vastaanotonViimeisinMuutos = None) must_== valinnantulos
    ...
    "ylimmän hyväksytyn hakutoiveen voi ottaa ehdollisesti vastaan, jos sen yläpuolella on hakutoive varalla" in {
      useFixture(
        "varalla-hyvaksytty-hyvaksytty.json",
        Nil,
        hakuFixture = HakuFixtures.korkeakouluYhteishaku,
      ...
      val valinnantuloksetBefore = hakemuksenValinnantulokset
      assertTila(
        findOne(valinnantuloksetBefore, hakukohde(HakukohdeOid("1.2.246.562.5.72607738902"))),
        Varalla,
        ValintatuloksenTila.KESKEN
      )
      assertTila(
        findOne(valinnantuloksetBefore, hakukohde(HakukohdeOid("1.2.246.562.5.72607738903"))),
        Hyvaksytty,
        ValintatuloksenTila.KESKEN
      )
      assertTila(
        findOne(valinnantuloksetBefore, hakukohde(HakukohdeOid("1.2.246.562.5.72607738904"))),
        Hyvaksytty,
        ValintatuloksenTila.KESKEN
      )
      tallenna(
      ...
      val valinnantuloksetAfter =
        valintarekisteriDb.runBlocking(valintarekisteriDb.getValinnantuloksetForHakemus(hakemusOid))
      assertTila(
        findOne(valinnantuloksetAfter, hakukohde(HakukohdeOid("1.2.246.562.5.72607738902"))),
        Varalla,
        ValintatuloksenTila.KESKEN
      )
      assertTila(
        findOne(valinnantuloksetAfter, hakukohde(HakukohdeOid("1.2.246.562.5.72607738903"))),
        Hyvaksytty,
        ValintatuloksenTila.EHDOLLISESTI_VASTAANOTTANUT
      )
      assertTila(
        findOne(valinnantuloksetAfter, hakukohde(HakukohdeOid("1.2.246.562.5.72607738904"))),
        Hyvaksytty,
        ValintatuloksenTila.KESKEN
      )
    }
    "varalla olevaa hakutoivetta ei voi ottaa vastaan" in {
      ...
      tallennaVirheella(
        List(valinnantulos.copy(vastaanottotila = ValintatuloksenTila.EHDOLLISESTI_VASTAANOTTANUT)),
        Some("Hakutoivetta ei voi ottaa ehdollisesti vastaan")
      )
      tallennaVirheella(
        List(valinnantulos.copy(vastaanottotila = ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI)),
        Some("Ei voi tallentaa vastaanottotietoa, koska hakijalle näytettävä tila on \"Varalla\"")
      )
      assertTila(
        findOne(hakemuksenValinnantulokset, tila(Varalla)),
        Varalla,
        ValintatuloksenTila.KESKEN
      )
    }
    ...
    "peru yksi hakija" in {
      ...
      val valinnantulosBefore =
        findOne(hakemuksenValinnantulokset, valintatapajono(valintatapajonoOid))
      assertTila(valinnantulosBefore, Hyvaksytty, ValintatuloksenTila.KESKEN)
      val valinnantulosForSave =
        valinnantulosBefore.copy(vastaanottotila = ValintatuloksenTila.PERUNUT)
      tallenna(List(valinnantulosForSave))
      val valinnantulosAfter =
        findOne(hakemuksenValinnantulokset, valintatapajono(valintatapajonoOid))
      valinnantulosAfter.copy(vastaanotonViimeisinMuutos = None) must_== valinnantulosForSave
      valinnantulosAfter.vastaanotonViimeisinMuutos.isDefined must_== true
    ...
    "peruuta yhden hakijan vastaanotto" in {
      ...
      val valinnantulosForSave =
        valinnantulosBefore.copy(vastaanottotila = ValintatuloksenTila.PERUUTETTU)
      tallenna(List(valinnantulosForSave))
      val valinnantulosAfterAfter =
        findOne(hakemuksenValinnantulokset, valintatapajono(valintatapajonoOid))
      valinnantulosAfterAfter.copy(vastaanotonViimeisinMuutos = None) must_== valinnantulosForSave
        .copy(vastaanotonViimeisinMuutos = None)
    ...
    "virkailija voi vastaanottaa peruutetun hakutoiveen" in {
      ...
      tallenna(
        List(
          valinnantulosBefore.copy(vastaanottotila = ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI)
        )
      )
      assertTila(
        findOne(hakemuksenValinnantulokset, valintatapajono(valintatapajonoOid)),
    ...
    "virkailija voi vastaanottaa varasijalta hyväksytyn hakutoiveen" in {
      useFixture(
        "varasijalta_hyvaksytty-kesken-julkaistavissa.json",
        hakuFixture = HakuFixtures.korkeakouluYhteishaku
      )
      val valinnantulosBefore =
        findOne(hakemuksenValinnantulokset, valintatapajono(valintatapajonoOid))
    ...
    "vastaanota sitovasti yksi hakija vaikka toinen vastaanotto ei onnistu" in {
      ...
      val valinnantulosBefore =
        findOne(hakemuksenValinnantulokset, valintatapajono(valintatapajonoOid))
      assertTila(valinnantulosBefore, Hyvaksytty, ValintatuloksenTila.KESKEN)
      val failingValinnantulos = valinnantulosBefore.copy(
        hakukohdeOid = HakukohdeOid("1234"),
        henkiloOid = "1234",
        hakemusOid = HakemusOid("1234"),
        vastaanottotila = ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI
      )
      val statusList = valinnantulosService.storeValinnantuloksetAndIlmoittautumiset(
        valinnantulosBefore.valintatapajonoOid,
        List(
          valinnantulosBefore.copy(vastaanottotila = ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI),
          failingValinnantulos
        ),
        Some(Instant.now),
        auditInfo
      )
    ...
    "kaksi epäonnistunutta vastaanottoa" in {
      ...
      val failingValinnantulos1 = valinnantulos.copy(
        hakukohdeOid = HakukohdeOid("1234"),
        henkiloOid = "1234",
        hakemusOid = HakemusOid("1234"),
        vastaanottotila = ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI
      )
      val failingValinnantulos2 = valinnantulos.copy(
        hakukohdeOid = HakukohdeOid("1234"),
        henkiloOid = "12345",
        hakemusOid = HakemusOid("12345"),
        vastaanottotila = ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI
      )
    ...
    "hyväksytty, ylempi varalla, vastaanotto loppunut, virkailija asettaa ehdollisesti vastaanottanut" in {
      ...
      tallenna(
        List(
          valinnantulosBefore.copy(vastaanottotila =
            ValintatuloksenTila.EHDOLLISESTI_VASTAANOTTANUT
          )
        )
      )
    ...
    "vastaanoton poisto epäonnistuu jos ifUnmodifiedSince on aikaisempi kuin tallennus" in {
      ...
      val valinnantulosBefore =
        findOne(hakemuksenValinnantulokset, valintatapajono(valintatapajonoOid))
      assertTila(valinnantulosBefore, Hyvaksytty, ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI)
      tallennaVirheellaCustomAikaleimalla(
        List(valinnantulosBefore.copy(vastaanottotila = ValintatuloksenTila.KESKEN)),
        Some("Hakemus on muuttunut lukemisen jälkeen"),
        409,
        Some(Instant.now().minusSeconds(TimeUnit.MINUTES.toSeconds(1)))
      )
      val valinnantulosAfter =
        findOne(hakemuksenValinnantulokset, valintatapajono(valintatapajonoOid))
      assertTila(valinnantulosAfter, Hyvaksytty, ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI)
    ...
    "vastaanoton poisto onnistuu jos ifUnmodifiedSince on myöhäisempi kuin tallennus" in {
      ...
      val valinnantulosBefore =
        findOne(hakemuksenValinnantulokset, valintatapajono(valintatapajonoOid))
      assertTila(valinnantulosBefore, Hyvaksytty, ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI)
      tallennaCustomAikaleimalla(
        List(valinnantulosBefore.copy(vastaanottotila = ValintatuloksenTila.KESKEN)),
        Some(Instant.now().plusSeconds(TimeUnit.MINUTES.toSeconds(1)))
      )
      val valinnantulosAfter =
        findOne(hakemuksenValinnantulokset, valintatapajono(valintatapajonoOid))
      assertTila(valinnantulosAfter, Hyvaksytty, ValintatuloksenTila.KESKEN)
    ...
    "vastaanotto epäonnistuu jos ifUnmodifiedSince on aikaisempi kuin tallennus" in {
      ...
      tallennaVirheellaCustomAikaleimalla(
        ...
        Some("Valinnantuloksen tallennus epäonnistui"),
        500,
        Some(Instant.now().minusSeconds(TimeUnit.MINUTES.toSeconds(1)))
      )
      assertTila(
    ...
    "BUG-1794 - vastaanotto ja ilmoittautuminen epäonnistuu ifUnmodifiedSincen ollessa välimaastossa" in {
      ...
      Thread.sleep(5000)
      val valinnantulosMid =
        findOne(hakemuksenValinnantulokset, valintatapajono(valintatapajonoOid))
          .copy(ilmoittautumistila = LasnaKokoLukuvuosi)
      ...
      var valinnantulosAfter = findOne(
        hakemuksenValinnantulokset,
        valintatapajono(valintatapajonoOid)
      ).copy(vastaanottotila = ValintatuloksenTila.KESKEN, ilmoittautumistila = EiTehty)
      tallennaVirheellaCustomAikaleimalla(
        List(valinnantulosAfter),
        Some("Hakemus on muuttunut lukemisen jälkeen"),
        409,
        Some(Instant.now().minusSeconds(3))
      )
  ...
  def hakemuksenValinnantulokset: Set[Valinnantulos] =
    kaikkiTestinHakukohteet
      .map(oid =>
  ...
  def valintatapajono(valintatapajonoOid: ValintatapajonoOid) =
    (v: Valinnantulos) => v.valintatapajonoOid == valintatapajonoOid

  def findOne(
    valinnantulokset: Set[Valinnantulos],
    filter: Valinnantulos => Boolean
  ): Valinnantulos = {
  ...
  def assertTila(
    ...
    vastaanotto: ValintatuloksenTila
  ) = {
  ...
  def tallenna(valinnantulokset: List[Valinnantulos]) = {
    val status = valinnantulosService.storeValinnantuloksetAndIlmoittautumiset(
      valinnantulokset.head.valintatapajonoOid,
      valinnantulokset,
      Some(Instant.now),
      auditInfo
    )
  ...
  def tallennaCustomAikaleimalla(
  ...
  def tallennaVirheellaCustomAikaleimalla(
    valinnantulokset: List[Valinnantulos],
    message: Option[String] = None,
    expectedStatus: Int = 400,
    ifUnmodifiedSince: Option[Instant]
  ...
  private def vastaanotaHakijana(
