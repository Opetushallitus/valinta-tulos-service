...
object YhdenPaikanSaantoBatchAPITester extends App with Logging {
  ...
  val (status, _, result) = new DefaultHttpRequest(
    Http(
      s"http://localhost:${SharedJetty.port}/valinta-tulos-service/virkailija/valintatulos/haku/1.2.246.562.5.2013080813081926341928"
    )
      .method("GET")
  ...
  println(
    s"parsing response of size ${Serialization.read[List[Map[String, JValue]]](result).size} took ${System
