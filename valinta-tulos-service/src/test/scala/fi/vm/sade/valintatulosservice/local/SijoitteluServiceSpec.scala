...
class SijoitteluServiceSpec extends Specification with MockitoMatchers with MockitoStubs {
  ...
  val hakukohde = Hakukohde(hakukohdeOid, hakuOid, Set(tarjoajaOid), null, null, null, null, true, null, 2017, organisaatioRyhmaOids = Set(organisaatioRyhmaOid))
  ...
  trait SijoitteluServiceMocks extends Mockito with Scope with MustThrownExpectations {
    ...
    hakuService.getHakukohde(hakukohdeOid) returns Right(Hakukohde(hakukohdeOid, hakuOid, Set(tarjoajaOid), null, null, null, null, true, null, 2015, Set(organisaatioRyhmaOid)))
