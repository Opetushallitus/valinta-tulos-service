...
class SijoitteluServiceSpec extends Specification with MockitoMatchers with MockitoStubs {
  ...
  "SijoitteluService" in {
    "return correct sijoittelun tulos for hakukohde" in new SijoitteluServiceMocks {
      val hakukohde =
        service.getHakukohdeBySijoitteluajo(hakuOid, "latest", hakukohdeOid, session, auditInfo)
      ...
      there was one(sijoitteluRepository).getHakukohteenValintatapajonot(
        sijoitteluajoId,
        hakukohdeOid
      )
      ...
      there was one(sijoitteluRepository).getSijoitteluajonHakijaryhmistaHyvaksytytHakemukset(
        sijoitteluajoId,
        List("hakijaryhma1")
      )
      there was one(sijoitteluRepository).getSijoitteluajonHakijaryhmienHakemukset(
        sijoitteluajoId,
        List("hakijaryhma1")
      )
      there was one(sijoitteluRepository).getValinnantilanKuvaukset(List(123))

      JsonFormats.javaObjectToJsonString(hakukohde) mustEqual JsonFormats.javaObjectToJsonString(
        createExpected
      )
    }
    "auditlogittaa sijoittelutietojen luvun" in new SijoitteluServiceMocks with Authorized {
      ...
      there was one(audit).log(
        any[User],
        argThat[Operation, Operation](be_==(SijoittelunHakukohteenLuku)),
        any[Target],
        any[Changes]
      )
    }
    ...
    SijoittelunHakukohdeRecord(sijoitteluajoId, hakukohdeOid, true).dto(
      List(
        ValintatapajonoRecord(
          "arvonta",
          ValintatapajonoOid("valintatapajono1"),
          "valintatapajono1",
          1,
          Some(10),
          Some(10),
              ...
              h.dto(
                ...
                List(
                  TilaHistoriaRecord(
                    ...
                    HakemusOid("1234.1"),
                    Hyvaksytty,
                    new Date(now.minus(2, ChronoUnit.DAYS).toEpochMilli)
                  ).dto,
                  TilaHistoriaRecord(
                    ValintatapajonoOid("valintatapajono1"),
                    HakemusOid("1234.1"),
                    Hylatty,
                    new Date(now.minus(4, ChronoUnit.DAYS).toEpochMilli)
                  ).dto
                )
              )
            ).get,
        ...
        ValintatapajonoRecord(
          "arvonta",
          ValintatapajonoOid("valintatapajono2"),
          "valintatapajono2",
          1,
          Some(10),
          Some(10),
      ...
      List(
        HakijaryhmaRecord(
          ...
          None,
          "uri/1/2/3"
        ).dto(List(HakemusOid("1234.1")))
      )
    )
  }

  trait SijoitteluServiceMocks
      extends Mockito
      with Scope
      with MustThrownExpectations
      with RunBlockingMock {
    ...
    authorizer.checkAccess(
      session,
      Set(tarjoajaOid, organisaatioRyhmaOid),
    ...
    sijoitteluRepository.getLatestSijoitteluajoId("latest", hakuOid) returns DBIO.successful(
      sijoitteluajoId
    )
    sijoitteluRepository.getSijoitteluajonHakukohde(sijoitteluajoId, hakukohdeOid) returns Some(
      SijoittelunHakukohdeRecord(sijoitteluajoId, hakukohdeOid, true)
    )
    ...
    sijoitteluRepository.getSijoitteluajonHakijaryhmanHakemukset(
      sijoitteluajoId,
      "hakijaryhma1"
    ) returns List(HakemusOid("1234.1"))
    sijoitteluRepository.getSijoitteluajonHakijaryhmistaHyvaksytytHakemukset(
      sijoitteluajoId,
      List("hakijaryhma1")
    ) returns Map("hakijaryhma1" -> List(HakemusOid("1234.1")))
    sijoitteluRepository.getSijoitteluajonHakijaryhmienHakemukset(
      sijoitteluajoId,
      List("hakijaryhma1")
    ) returns Map("hakijaryhma1" -> List(HakemusOid("1234.1")))
