...
import fi.vm.sade.valintatulosservice.kayttooikeus.{
  GrantedAuthority,
...
class ErillishakuServletSpec
    extends Specification
    with EmbeddedJettyContainer
    with HttpComponentsClient
    with BeforeAfterAll
    with ForEach[
      (String, ValinnantulosService, HyvaksymiskirjeService, KayttooikeusUserDetailsService)
    ]
    with ITSetup
    with Mockito {
  ...
  def foreach[R: AsResult](
    f: ((String, ValinnantulosService, HyvaksymiskirjeService, KayttooikeusUserDetailsService)) => R
    ...
    val servlet = new ErillishakuServlet(
      valinnantulosService,
      hyvaksymiskirjeService,
      userDetailsService,
      appConfig
    ...
    ServletTest.withServlet(
      ...
      servlet,
      (uri: String) =>
        AsResult(f((uri, valinnantulosService, hyvaksymiskirjeService, userDetailsService)))
    )
  }
  ...
  private val auditInfoParameters =
    Iterable("uid" -> uid, "inetAddress" -> inetAddress, "userAgent" -> userAgent)
  ...
  "GET /erillishaku/valinnan-tulos" in {
    "palauttaa 400, jos uid parametri puuttuu" in {
      t: (String, ValinnantulosService, HyvaksymiskirjeService, KayttooikeusUserDetailsService) =>
        get(
    ...
    "palauttaa 400, jos inetAddress parametri puuttuu" in {
      t: (String, ValinnantulosService, HyvaksymiskirjeService, KayttooikeusUserDetailsService) =>
        get(
          s"${t._1}/${valintatapajonoOid.toString}",
    ...
    "palauttaa 400, jos userAgent parametri puuttuu" in {
      t: (String, ValinnantulosService, HyvaksymiskirjeService, KayttooikeusUserDetailsService) =>
        get(
          s"${t._1}/${valintatapajonoOid.toString}",
    ...
    "palauttaa 401, jos käyttäjä ei löydy KO:sta" in {
      t: (String, ValinnantulosService, HyvaksymiskirjeService, KayttooikeusUserDetailsService) =>
        t._4.getUserByUsername(uid) returns Left(
          new AuthenticationFailedException("error for testing")
        )
        get(
          s"${t._1}/${valintatapajonoOid.toString}",
          auditInfoParameters,
    ...
    "palauttaa hyväksymiskirjeiden tiedot pyydettäessä" in {
      t: (String, ValinnantulosService, HyvaksymiskirjeService, KayttooikeusUserDetailsService) =>
        ...
        t._3.getHyvaksymiskirjeet(any[HakukohdeOid], any[AuditInfo]) returns Set(
          Hyvaksymiskirje(
        ...
        get(
          ...
          status must_== 200
          parse(body).extract[List[Valinnantulos]] must_== List(
            valinnantulos.copy(hyvaksymiskirjeLahetetty = Some(hyvaksymiskirjeLahetetty))
          )
        }
    }
  ...
  "POST /erillishaku/valinnan-tulos" in {
    "palauttaa 500, jos audit tietoa ei voitu jäsentää" in {
      t: (String, ValinnantulosService, HyvaksymiskirjeService, KayttooikeusUserDetailsService) =>
        post(
          s"${t._1}/${valintatapajonoOid.toString}",
    ...
    "palauttaa 403, jos käyttäjällä ei ole kirjoitusoikeuksi" in {
      t: (String, ValinnantulosService, HyvaksymiskirjeService, KayttooikeusUserDetailsService) =>
        post(
          s"${t._1}/${valintatapajonoOid.toString}",
          write(
            ValinnantulosRequest(
              List(valinnantulos),
              AuditSessionRequest(kayttajaOid, List(Role.SIJOITTELU_READ.s), userAgent, inetAddress)
            )
          ).getBytes("UTF-8"),
          Map("Content-Type" -> "application/json")
        ) {
      ...
      t: (String, ValinnantulosService, HyvaksymiskirjeService, KayttooikeusUserDetailsService) =>
        ...
        post(
          ...
          write(
            ValinnantulosRequest(
              List(valinnantulos),
              AuditSessionRequest(kayttajaOid, List(Role.SIJOITTELU_CRUD.s), userAgent, inetAddress)
            )
          ).getBytes("UTF-8"),
          Map(
    ...
    "palauttaa 200 jos hyväksymiskirjeiden tietojen päivitys epäonnistui" in {
      t: (String, ValinnantulosService, HyvaksymiskirjeService, KayttooikeusUserDetailsService) =>
        t._2.storeValinnantuloksetAndIlmoittautumiset(
          any[ValintatapajonoOid],
          any[List[Valinnantulos]],
        ...
        t._3.updateHyvaksymiskirjeet(
          any[Set[HyvaksymiskirjePatch]],
          any[AuditInfo]
