...
class KelaHealthCheckServletSpec extends ServletSpecification with Logging {
  "GET /health-check/kela" should {
    "Palauttaa OK responsessa, kun rajapinta toimii" in {
      val session: CasSession = CasSession(ServiceTicket("KelaHealthCheckServletSpecServiceTicket"), "1.2.246.562.24.78412307527", Set(Role("APP_VALINTATULOSSERVICE_KELA_READ")))
