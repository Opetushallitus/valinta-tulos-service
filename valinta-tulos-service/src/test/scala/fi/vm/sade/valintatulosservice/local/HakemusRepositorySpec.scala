  ...
  "HakemusRepository" should {
    "palauttaa yksittäisen Hakemuksen" in {
      useFixture(
        ...
        hakemusFixtures = HakemusFixtures.defaultFixtures
      )

      val hakutoiveet = repo.findHakemus(HakemusOid("1.2.246.562.11.00000878229"))
      hakutoiveet must beRight(
        Hakemus(
          HakemusOid("1.2.246.562.11.00000878229"),
          HakuOid("1.2.246.562.29.92478804245"),
          "1.2.246.562.24.14229104472",
          "FI",
          List(
            Hakutoive(
              HakukohdeOid("1.2.246.562.20.83060182827"),
              "1.2.246.562.10.83122281013",
              "stevari amk hakukohde",
              "Saimaan ammattikorkeakoulu, Skinnarilan kampus, Lappeenranta"
            ),
            Hakutoive(
              HakukohdeOid("1.2.246.562.10.83122281012"),
              "1.2.246.562.10.83122281012",
              "",
              ""
            )
          ),
    ...
    "palauttaa kaikki Hakuun liittyvät Hakemukset" in {
      ...
      hakemukset must_== Seq(
        ...
        Hakemus(
          HakemusOid("1.2.246.562.11.00000441371"),
          HakuOid("1.2.246.562.5.2013080813081926341928"),
          "1.2.246.562.24.14229104472",
          "FI",
          List(
    ...
    "palauttaa yksittäisen Hakemuksen jolla on eri asiointikieli" in {
      ...
      hakutoiveet must beRight(
        Hakemus(
          HakemusOid("1.2.246.562.11.00000878229-SE"),
          HakuOid("1.2.246.562.29.92478804245"),
          "1.2.246.562.24.14229104472",
          "SV",
          List(
