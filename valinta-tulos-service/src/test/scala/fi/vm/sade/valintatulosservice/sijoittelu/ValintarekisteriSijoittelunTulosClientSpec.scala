...
class ValintarekisteriSijoittelunTulosClientSpec
    extends ITSpecification
    with ValintarekisteriTestData {
  step(deleteAll())
  ...
  "fetch latest sijoitteluajo" should {
    ...
    "return sijoitteluajo" in {
      ...
      sijoitteluajo.getHakukohteet.asScala
        .map(_.getOid)
        .diff(List(oidHaku2hakukohde1.toString, "Haku2.1", "Haku2.2")) must_== List()
    }
    ...
    "return sijoitteluajo (erillishaku, ei sijoittelua)" in {
      ...
      sijoitteluajo.getHakukohteet.asScala
        .map(_.getOid)
        .diff(List(oidHaku1hakukohde1.toString, oidHaku1hakukohde2.toString)) must_== List()
    }
  ...
  "fetch hakemuksen tulos" should {
    ...
    step(
      insertValinnantulos(
        ...
        valinnantulos(
          ...
          sijoittelunValintatapajonoOid2,
          sijoittelunHakemusOid1
        )
      )
    )

    "return None if hakija not found" in {
      client.fetchHakemuksenTulos(
        singleConnectionValintarekisteriDb.runBlocking(
          singleConnectionValintarekisteriDb.getLatestSijoitteluajoId(hakuOid2)
        ),
        hakuOid2,
        HakemusOid("fakeHakemusOid")
      ) must beNone
    }

    "return hakijan hakutoiveet" in {
      val hakijaDto = client
        .fetchHakemuksenTulos(
          singleConnectionValintarekisteriDb.runBlocking(
            singleConnectionValintarekisteriDb.getLatestSijoitteluajoId(hakuOid2)
          ),
          hakuOid2,
    ...
    "return hakijan hakutoiveet (erillishaku, ei sijoittelua)" in {
      val hakijaDto = client
        .fetchHakemuksenTulos(
          singleConnectionValintarekisteriDb.runBlocking(
            singleConnectionValintarekisteriDb.getLatestSijoitteluajoId(hakuOid1)
          ),
          hakuOid1,
          hakemusOid2
        )
    ...
    "return hakijan hakutoiveet (both with and without sijoittelu)" in {
      ...
      val hakutoive1 = hakijaDto.getHakutoiveet.asScala
      ...
      val hakutoive2 = hakijaDto.getHakutoiveet.asScala
      ...
      val valintatapajono1 = hakutoive1.getHakutoiveenValintatapajonot.asScala
      ...
      val valintatapajono2 = hakutoive1.getHakutoiveenValintatapajonot.asScala
      ...
      val valintatapajono3 = hakutoive2.getHakutoiveenValintatapajonot.asScala
        .find(_.getValintatapajonoOid.equals(sijoittelunValintatapajonoOid2.toString))
        .get
