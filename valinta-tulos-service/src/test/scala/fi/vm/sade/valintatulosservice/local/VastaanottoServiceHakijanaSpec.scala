...
class VastaanottoServiceHakijanaSpec extends ITSpecification with TimeWarp with ThrownMessages {
  ...
  lazy val valintatulosService = new ValintatulosService(vastaanotettavuusService, sijoittelutulosService, valintarekisteriDb,
    hakuService, valintarekisteriDb, hakukohdeRecordService, sijoitteluContext.valintatulosDao)(appConfig, dynamicAppConfig)
  ...
  private def vastaanota(hakuOid: String, hakemusOid: String, hakukohdeOid: String, tila: Vastaanottotila, muokkaaja: String, selite: String, personOid: String) = {
    vastaanottoService.vastaanotaHakijana(HakijanVastaanotto(personOid, HakemusOid(hakemusOid), HakukohdeOid(hakukohdeOid), HakijanVastaanottoAction.getHakijanVastaanottoAction(tila)))
