...
import fi.vm.sade.valintatulosservice.hakemus.{
  AtaruHakemusEnricher,
  AtaruHakemusRepository,
  HakemusRepository,
  HakuAppRepository
}
...
class ValintatulosServiceSpec extends ITSpecification with TimeWarp {
  ...
  lazy val sijoittelunTulosClient = new ValintarekisteriSijoittelunTulosClientImpl(
    valintarekisteriDb
  )
  lazy val raportointiService =
    new ValintarekisteriRaportointiServiceImpl(valintarekisteriDb, valintatulosDao)
  lazy val sijoittelutulosService = new SijoittelutulosService(
    raportointiService,
    appConfig.ohjausparametritService,
    valintarekisteriDb,
    sijoittelunTulosClient
  )
  lazy val hakukohdeRecordService =
    new HakukohdeRecordService(hakuService, valintarekisteriDb, true)
  lazy val hakijaDtoClient = new ValintarekisteriHakijaDTOClientImpl(
    raportointiService,
    sijoittelunTulosClient,
    valintarekisteriDb
  )
  ...
  lazy val valintatulosService = new ValintatulosService(
    valintarekisteriDb,
  ...
  def testitTyypillisilleHauille(hakuFixture: HakuOid) = {
    "hyväksytty, useimmat haut" in {
      "ylemmat sijoiteltu -> vastaanotettavissa" in {
        ...
        checkHakutoiveState(
          getHakutoive(("1.2.246.562.5.72607738902")),
          Valintatila.hylätty,
        ...
        checkHakutoiveState(
          getHakutoive("1.2.246.562.5.16303028779"),
  ...
  def testitSijoittelunPiirissäOlemattomilleHakutyypeille(hakuFixture: HakuOid) = {
    "hyväksyttyä hakutoivetta alempia ei merkitä tilaan PERUUNTUNUT" in {
      ...
      // HYVÄKSYTTY KESKEN true
      useFixture(
        "hyvaksytty-julkaisematon-hyvaksytty.json",
      ...
      checkHakutoiveState(
        getHakutoive("1.2.246.562.5.72607738903"),
        Valintatila.kesken,
        Vastaanottotila.kesken,
        Vastaanotettavuustila.ei_vastaanotettavissa,
        false
      ...
      checkHakutoiveState(
        getHakutoive("1.2.246.562.5.72607738904"),
        Valintatila.hyväksytty,
        Vastaanottotila.kesken,
        Vastaanotettavuustila.vastaanotettavissa_sitovasti,
        true
  ...
  def testitSijoiteltavilleKorkeakouluHauille(hakuFixture: HakuOid) = {
    ...
    "hyväksytty, sijoittelua käyttävä korkeakouluhaku" in {
      ...
      "ylempi varalla, kun varasijasäännöt ei vielä voimassa -> kesken" in {
        ...
        // HYVÄKSYTTY KESKEN true
        useFixture(
          "hyvaksytty-ylempi-varalla.json",
          hakuFixture = hakuFixture,
          ohjausparametritFixture = OhjausparametritFixtures.varasijasaannotEiVielaVoimassa
        )
        checkHakutoiveState(
          getHakutoive("1.2.246.562.5.72607738902"),
          Valintatila.varalla,
      ...
      "hakutoiveista 1. kesken 2. hyväksytty 3. peruuntunut" in {
        ...
        useFixture(
          "hyvaksytty-ylempi-sijoittelematon-alempi-peruuntunut.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-3")
        )
        checkHakutoiveState(
      ...
      "hakutoiveista 1. hyväksytty, ei julkaistu 2. hyväksytty, odottaa ylempiä toiveita 3. peruuntunut" in {
        ...
        useFixture(
          "hyvaksytty-ylempi-ei-julkaistu-alempi-peruuntunut-historia.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-3")
        )
        checkHakutoiveState(
        ...
        val hakemuksen1tila = ValinnantilanTallennus(
        ...
        val hakemuksen21tila = ValinnantilanTallennus(
          HakemusOid("1.2.246.562.11.00000441369"),
        ...
        val hakemuksen22tila = ValinnantilanTallennus(
          HakemusOid("1.2.246.562.11.00000441369"),
        ...
        val hakemuksen3tila = ValinnantilanTallennus(
          HakemusOid("1.2.246.562.11.00000441369"),
        ...
        val alempiPeruuntunutTulosJokaOnKaannettyHyvaksytyksi =
          getHakutoive("1.2.246.562.5.72607738903")
        checkHakutoiveState(
          alempiPeruuntunutTulosJokaOnKaannettyHyvaksytyksi,
          Valintatila.hyväksytty,
          Vastaanottotila.kesken,
          Vastaanotettavuustila.ei_vastaanotettavissa,
          true
        )
        val jonokohtaisetTulostiedotAlemmaltaHyvaksytyksiKaannetylta: Seq[JonokohtainenTulostieto] =
          alempiPeruuntunutTulosJokaOnKaannettyHyvaksytyksi.jonokohtaisetTulostiedot.sortBy(
            _.valintatapajonoPrioriteetti
          )
        jonokohtaisetTulostiedotAlemmaltaHyvaksytyksiKaannetylta(
          0
        ).valintatila must_== Valintatila.peruuntunut
        jonokohtaisetTulostiedotAlemmaltaHyvaksytyksiKaannetylta(
          1
        ).valintatila must_== Valintatila.hyväksytty
        jonokohtaisetTulostiedotAlemmaltaHyvaksytyksiKaannetylta(
          2
        ).valintatila must_== Valintatila.peruuntunut
        checkHakutoiveState(
      ...
      "hakutoiveista 1. hyväksytty, ei julkaistu 2. hylätty 3. hyväksytty, odottaa ylempiä toiveita" in {
        ...
        ).jonokohtaisetTulostiedot.head.valintatila must_== Valintatila.varalla
        ...
        getHakutoive("1.2.246.562.5.72607738903")
          .jonokohtaisetTulostiedot(1)
          .valintatila must_== Valintatila.hyväksytty
        getHakutoive("1.2.246.562.5.72607738903")
          .jonokohtaisetTulostiedot(2)
          .valintatila must_== Valintatila.peruuntunut
        ...
        val hakemuksen23tila = ValinnantilanTallennus(
          HakemusOid("1.2.246.562.11.00000441369"),
        ...
        val tallennaHakemuksen3Tarkenne = tallennaTilankuvauksenTarkenne(
          HakukohdeOid("1.2.246.562.5.72607738904"),
          ValintatapajonoOid("14090336922663576781797489829888"),
          HakemusOid("1.2.246.562.11.00000441369"),
          PERUUNTUNUT_HYVAKSYTTY_YLEMMALLE_HAKUTOIVEELLE
        ...
        val alimmanPeruuntuneenToiveenTulosKunYlinOnJulkaisematonHyvaksytty =
          getHakutoive("1.2.246.562.5.72607738904")
        checkHakutoiveState(
          alimmanPeruuntuneenToiveenTulosKunYlinOnJulkaisematonHyvaksytty,
          Valintatila.hyväksytty,
          Vastaanottotila.kesken,
          Vastaanotettavuustila.ei_vastaanotettavissa,
          true
        ...
        val alimmanPeruuntuneenToiveenTulosKunYlinHyvaksyttyOnJulkaistu =
          getHakutoive("1.2.246.562.5.72607738904")
        checkHakutoiveState(
          alimmanPeruuntuneenToiveenTulosKunYlinHyvaksyttyOnJulkaistu,
          Valintatila.peruuntunut,
          Vastaanottotila.kesken,
          Vastaanotettavuustila.ei_vastaanotettavissa,
          true
      ...
      "peruuntunutta ei merkitä väliaikaisesti hyväksytyksi jos se ei ole ollut samaan aikaan sekä hyväksytty että julkaistu" in {
        ...
        useFixture(
          "hyvaksytty-ylempi-ei-julkaistu-alempi-peruun-hist-ei-julkaistu.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-2")
        )

        // BUG-2026 reproduction step 2
        // HYVAKSYTTY KESKEN false
        // PERUUNTUNUT KESKEN false
        val hakemuksen1tilaHyvaksytty = ValinnantilanTallennus(
          hakemusOid,
        ...
        val hakemuksen2tilaPeruuntunut = ValinnantilanTallennus(
          hakemusOid,
        ...
        valintarekisteriDb.runBlocking(
          valintarekisteriDb
            .storeValinnantila(hakemuksen1tilaHyvaksytty, None)
            .andThen(
              valintarekisteriDb
                .storeValinnantila(hakemuksen2tilaPeruuntunut, None)
                .andThen(
                  tallennaTilankuvauksenTarkenne(
                    alempiHakukohdeOid,
                    alemmanKohteenJonoOid,
                    hakemusOid,
                    PERUUNTUNUT_HYVAKSYTTY_YLEMMALLE_HAKUTOIVEELLE
        ...
        checkHakutoiveState(
          alemmanPeruuntuneenToiveenTulos,
          Valintatila.kesken,
          Vastaanottotila.kesken,
          Vastaanotettavuustila.ei_vastaanotettavissa,
          false
      ...
      "hakutoiveista 1. hyväksytty, ei julkaistu 2. ei tehty 3. hyväksytty, odottaa ylempiä toiveita" in {
        ...
        // Ajetaan ensin historiadata
        useFixture(
          "hyvaksytty-ylempi-ei-julkaistu-toinen-ei-sijoittelua-alin-peruuntunut.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-3")
        )
        checkHakutoiveState(
        ...
        valintarekisteriDb.runBlocking(
          sqlu"delete from jonosijat where valintatapajono_oid = '14090336922663576781797489829885' and hakukohde_oid = '1.2.246.562.5.72607738903'".transactionally
        )

        valintarekisteriDb.runBlocking(
          sqlu"delete from valintatapajonot where oid = '14090336922663576781797489829885'".transactionally
        )

        valintarekisteriDb.runBlocking(
          sqlu"delete from tilat_kuvaukset where hakukohde_oid = '1.2.246.562.5.72607738903' and hakemus_oid = '1.2.246.562.11.00000441369' and valintatapajono_oid = '14090336922663576781797489829885'".transactionally
        )

        valintarekisteriDb.runBlocking(
          sqlu"delete from tilat_kuvaukset_history where hakukohde_oid = '1.2.246.562.5.72607738903' and hakemus_oid = '1.2.246.562.11.00000441369' and valintatapajono_oid = '14090336922663576781797489829885'".transactionally
        )

        valintarekisteriDb.runBlocking(
          sqlu"delete from valinnantulokset where hakukohde_oid = '1.2.246.562.5.72607738903' and hakemus_oid = '1.2.246.562.11.00000441369'".transactionally
        )

        valintarekisteriDb.runBlocking(
          sqlu"delete from valinnantulokset_history where hakukohde_oid = '1.2.246.562.5.72607738903' and hakemus_oid = '1.2.246.562.11.00000441369'".transactionally
        )

        valintarekisteriDb.runBlocking(
          sqlu"delete from valinnantilat where hakukohde_oid = '1.2.246.562.5.72607738903' and hakemus_oid = '1.2.246.562.11.00000441369'".transactionally
        )

        valintarekisteriDb.runBlocking(
          sqlu"delete from valinnantilat_history where hakukohde_oid = '1.2.246.562.5.72607738903' and hakemus_oid = '1.2.246.562.11.00000441369'".transactionally
        )

        valintarekisteriDb.runBlocking(
          sqlu"delete from sijoitteluajon_hakukohteet where hakukohde_oid = '1.2.246.562.5.72607738903'".transactionally
        )
      ...
      "ylimmällä toiveella julkaistu varalla ja julkaisematon hyväksytty, alempi hyväksytty mutta kaikki jonot ei sijoittelussa -> näytetään alempi kesken" in {
        useFixture(
          "ylin-toive-hyvaksytty-toisesta-jonosta-mutta-julkaisematon-alempi-peruuntunut.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-3"),
      ...
      "ylimmällä toiveella julkaistu varalla ja julkaisematon hyväksytty, alempi julkaistu hyväksytty mutta kaikki jonot ei sijoittelussa -> näytetään alempi hyväksytty, odottaa ylempiä" in {
        ...
        val alemmanToiveenHyvaksytynJononOid =
          ValintatapajonoOid("14090336922663576781797489829888")
      ...
      "ylimmällä toiveella julkaistu varalla ja julkaisematon hyväksytty, alempana kaksi julkaistua hyväksyttyä mutta kaikki jonot ei sijoittelussa -> " +
        ...
        useFixture(
          "ylin-toive-hyvaksytty-toisesta-jonosta-mutta-kaksi-kolmesta-julkaistua-jonoa-alemmmassa-peruuntununeessa.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-3"),
          ohjausparametritFixture = "varasijasaannot-ei-viela-voimassa"
        )

        val alemmanToiveenEnsimmäisenHyväksytynJononOid =
          ValintatapajonoOid("14090336922663576781797489829888")
        val alemmanToiveenToisenHyväksytynJononOid =
        ...
        alemmanToiveenTulos.jonokohtaisetTulostiedot(1).tilanKuvaukset must beSome(
          TilanKuvaukset.peruuntunutHyvaksyttyToisessaJonossa.asScala
        )
    ...
    "ylimmällä toiveella julkaistu varalla ja julkaisematon hyväksytty, alempi julkaistu ehdollisesti hyväksyttynä ja muutettu hylätyksi" +
      ...
      alemmanToiveenTulos.jonokohtaisetTulostiedot.head.tilanKuvaukset
        .get("FI") must_== "Et toimittanut tutkintotodistusta ajoissa"
      alemmanToiveenTulos.jonokohtaisetTulostiedot(1).valintatila must_== Valintatila.kesken
      alemmanToiveenTulos.jonokohtaisetTulostiedot(1).tilanKuvaukset must beNone
      alemmanToiveenTulos.jonokohtaisetTulostiedot(2).valintatila must_== Valintatila.kesken
      alemmanToiveenTulos.jonokohtaisetTulostiedot(2).tilanKuvaukset must beNone
    ...
    "peruuntunut, sijoittelua käyttävä korkeakouluhaku" in {
      "ylempi hyväksytty kesken, koska varasijasäännöt ei vielä voimassa -> näytetään peruuntunut keskeneräisenä" in {
        ...
        // PERUUNTUNUT KESKEN true
        useFixture(
          "varalla-julkaistu-peruuntunut.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-3"),
          ohjausparametritFixture = "varasijasaannot-ei-viela-voimassa"
        )
      ...
      "ylemmät julkaistu, mutta alempi peruuntunut ei, varasijasäännöt ei vielä voimassa -> näytetään peruuntunut keskeneräisenä" in {
        ...
        // PERUUNTUNUT KESKEN false
        useFixture(
          "hyvaksytty-ylempi-peruuntunut-alempi-peruuntunut.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-3"),
          ohjausparametritFixture = "varasijasaannot-ei-viela-voimassa"
        )
    ...
    "peruuntunut toisessa jonossa hyväksymisen takia näytetään kesken, jos hyväksytty-jonoa ei ole julkaistu" in {
      useFixture(
        "samassa-kohteessa-julkaisematon-hyvaksytty-ja-julkaistu-peruuntunut.json",
        hakuFixture = hakuFixture,
        hakemusFixtures = List("00000441369-3")
      )
      ...
      val ylemmanJulkaisemattomanHyvaksytynJononTulos =
        hakutoiveentulos.jonokohtaisetTulostiedot.head
  ...
  def testitKaikilleHakutyypeille(hakuFixture: HakuOid) = {
    ...
    "hyväksytty varasijalta" in {
      "varasijasäännöt ei vielä voimassa -> näytetään hyväksyttynä" in {
        ...
        useFixture(
          "hyvaksytty-varasijalta-julkaistavissa.json",
          hakuFixture = hakuFixture,
          ohjausparametritFixture = OhjausparametritFixtures.varasijasaannotEiVielaVoimassa
        )
      ...
      "varasijasäännöt voimassa -> näytetään varasijalta hyväksyttynä" in {
        ...
        checkHakutoiveState(
          hakutoive,
          Valintatila.varasijalta_hyväksytty,
          Vastaanottotila.kesken,
          Vastaanotettavuustila.vastaanotettavissa_sitovasti,
          true
        )
    ...
    "hyväksytty" in {
      ...
      "Valintatulos julkaistavissa, mutta haun valintatulosten julkaisu paivamaara tulevaisuudessa" in {
        // HYVAKSYTTY, PERUUNTUNUT KESKEN true
        useFixture(
          "hyvaksytty-kesken-julkaistavissa.json",
          hakuFixture = hakuFixture,
          ohjausparametritFixture = OhjausparametritFixtures.tuloksiaEiVielaSaaJulkaista
        )
        val hakutoiveentulos = getHakutoive("1.2.246.562.5.72607738902")
        checkHakutoiveState(
          hakutoiveentulos,
        ...
        val ylemmanJononTulos = hakutoiveentulos.jonokohtaisetTulostiedot.find(
          _.oid == ValintatapajonoOid("14090336922663576781797489829886")
        )
        val alemmanJononTulos = hakutoiveentulos.jonokohtaisetTulostiedot.find(
          _.oid == ValintatapajonoOid("14090336922663576781797489829887")
        )
      ...
      "Valintatulos julkaistavissa ja haun julkaisu paivamaara mennyt" in {
        ...
        useFixture(
          "hyvaksytty-kesken-julkaistavissa.json",
          hakuFixture = hakuFixture,
          ohjausparametritFixture = OhjausparametritFixtures.tuloksetSaaJulkaista
        )
        checkHakutoiveState(
          getHakutoive("1.2.246.562.5.72607738902"),
          Valintatila.hyväksytty,
      ...
      "vastaanoton deadline näytetään" in {
        withFixedDateTime("26.11.2014 12:00") {
          ...
          useFixture("hyvaksytty-kesken-julkaistavissa.json", hakuFixture = hakuFixture)
          getHakutoive("1.2.246.562.5.72607738902").vastaanottoDeadline must_== Some(
            parseDate("10.1.2030 12:00")
          )
        }
      }
      ...
      "ei vastaanottanut määräaikana" in {
        "sijoittelu ei ole ehtinyt muuttamaan tulosta" in {
          ...
          useFixture(
            "hyvaksytty-valintatulos-ei-vastaanottanut-maaraaikana.json",
            hakuFixture = hakuFixture,
            ohjausparametritFixture = "vastaanotto-loppunut"
          )
          val hakutoiveentulos = getHakutoive("1.2.246.562.5.16303028779")
          ...
          jonokohtainenTulostieto.tilanKuvaukset must beSome(
            Map(
        ...
        "sijoittelu on muuttanut tuloksen" in {
          ...
          useFixture(
            "perunut-ei-vastaanottanut-maaraaikana.json",
            hakuFixture = hakuFixture,
            ohjausparametritFixture = "vastaanotto-loppunut"
          )
          ...
          hakutoive.jonokohtaisetTulostiedot.forall { jonokohtainenTulostieto =>
            {
      ...
      "vastaanottanut ehdollisesti" in {
        ...
        useFixture(
          "hyvaksytty-vastaanottanut-ehdollisesti.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-flipped")
        )
        checkHakutoiveState(
          getHakutoive("1.2.246.562.5.16303028779"),
          Valintatila.hyväksytty,
    ...
    "varalla" in {
      "käytetään parasta varasijaa, jos useammassa jonossa varalla" in {
        ...
        hakutoiveentulos.jonokohtaisetTulostiedot.forall(
          _.valintatila == Valintatila.varalla
        ) must beTrue
      }

      "varasijojen määrä näytetään" in {
        useFixture("hyvaksytty-ylempi-varalla.json", hakuFixture = hakuFixture)
        val jonokohtaisetTulostiedot =
          getHakutoive("1.2.246.562.5.72607738902").jonokohtaisetTulostiedot
      ...
      "varasijojen käsittelypäivämäärät näytetään" in {
        ...
        ).varasijojaKaytetaanAlkaen.get.getTime must_== new DateTime(
          "2014-08-01T16:00:00.000Z"
        ).toDate.getTime
        getHakutoive(
          "1.2.246.562.5.16303028779"
        ).varasijojaTaytetaanAsti.get.getTime must_== new DateTime(
          "2014-08-31T16:00:00.000Z"
        ).toDate.getTime
      }
      ...
      "Valintatulos kesken" in {
        // VARALLA KESKEN true
        useFixture(
          "varalla-valintatulos-kesken.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-flipped")
        )
        checkHakutoiveState(
      ...
      "Valintatulos hyvaksytty varasijalta" in {
        ...
        useFixture(
          "varalla-valintatulos-hyvaksytty-varasijalta-flag.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-flipped")
        )
        checkHakutoiveState(
    ...
    "peruuntunut" in {
      ...
      "ylempi julkaisematon -> näytetään keskeneräisenä" in {
        ...
        // PERUUNTUNUT KESKEN true
        useFixture(
          "julkaisematon-peruuntunut.json",
          hakuFixture = hakuFixture,
          hakemusFixtures = List("00000441369-3")
        )
        checkHakutoiveState(
    ...
    "ei vastaanotettu määräaikana" in {
      "virkailija ei merkinnyt myöhästyneeksi" in {
        ...
        val valintatulos2 = getHakutoiveenValintatulos(
          "1.2.246.562.5.2013080813081926341928",
          "1.2.246.562.5.72607738902"
        )
        ...
        val valintatulos3 =
          getHakutoiveenValintatulosByHakemus("1.2.246.562.5.72607738902", hakemusOid.toString)
      ...
      "virkailija merkinnyt myöhästyneeksi" in {
        ...
        valintatulos2.getTilaHakijalle must_== ValintatuloksenTila.EI_VASTAANOTETTU_MAARA_AIKANA
        val valintatulos3 =
          getHakutoiveenValintatulosByHakemus("1.2.246.562.5.16303028779", hakemusOid.toString)
  ...
  def getHakutoive(oid: HakukohdeOid) =
    hakemuksenTulos.hakutoiveet.find { _.hakukohdeOid == oid }.get
  ...
  def getHakutoiveenValintatulos(hakukohdeOid: String): Valintatulos = {
    valintatulosService
      .findValintaTuloksetForVirkailija(hakuOid, HakukohdeOid(hakukohdeOid))
      .find(_.getHakemusOid == hakemusOid.toString)
      .getOrElse(
        throw new NoSuchElementException(
          s"No valintatulos for hakuOid $hakuOid, hakukohdeOid $hakukohdeOid, hakemusOid $hakemusOid"
        )
      )
  }
  ...
  def getHakutoiveenValintatulos(hakuOid: String, hakukohdeOid: String): Valintatulos = {
    valintatulosService
      .findValintaTuloksetForVirkailija(HakuOid(hakuOid))
      .find(_.getHakukohdeOid == hakukohdeOid)
      .get
  }

  def getHakutoiveenValintatulosByHakemus(
    hakukohdeOid: String,
    hakemusOid: String
  ): Valintatulos = {
    valintatulosService
      .findValintaTuloksetForVirkailijaByHakemus(HakemusOid(hakemusOid))
      .find(_.getHakukohdeOid == hakukohdeOid)
      .get
  }

  def checkHakutoiveState(
    hakuToive: Hakutoiveentulos,
    expectedTila: Valintatila,
  ...
  private def tallennaTilankuvauksenTarkenne(
  ...
  private def asetaJulkaistavissa(
    hakukohdeOid: HakukohdeOid,
    jonoOid: ValintatapajonoOid,
    hakemusOid: HakemusOid,
    julkaistavissa: Boolean
  ...
  private def tilankuvauksetKielikoodeittain(
    tarkenne: TilankuvauksenTarkenne
  ): Map[String, String] = {
