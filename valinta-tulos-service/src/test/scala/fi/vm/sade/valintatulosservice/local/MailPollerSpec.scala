...
class MailPollerSpec extends ITSpecification with TimeWarp {
  ...
  lazy val valintatulosService = new ValintatulosService(vastaanotettavuusService, sijoittelutulosService, valintarekisteriDb, hakuService, valintarekisteriDb, hakukohdeRecordService, sijoitteluContext.valintatulosDao)(appConfig, dynamicAppConfig)
  ...
  "Hakujen filtteröinti" in {
    "korkeakouluhaku -> mukaan" in {
      new GeneratedFixture(new SingleHakemusFixture()).apply(sijoitteluContext)
