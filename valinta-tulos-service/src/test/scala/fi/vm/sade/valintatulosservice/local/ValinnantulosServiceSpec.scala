...
class ValinnantulosServiceSpec extends Specification with MockitoMatchers with MockitoStubs {
  "ValinnantulosService" in {
    "exception is thrown, if no authorization" in new Mocks with Korkeakouluhaku {
      authorizer.checkAccess(any[Session], any[Set[String]], any[Set[Role]]) returns Left(
        new AuthorizationFailedException("error")
      )
      val valinnantulokset = List(valinnantulosA)
      service.storeValinnantuloksetAndIlmoittautumiset(
        valintatapajonoOid,
        valinnantulokset,
        Some(Instant.now()),
        auditInfo
      ) must throwA[AuthorizationFailedException]
    }
    "status is 404 if valinnantulos is not found" in new Mocks
      with Authorized
      with Korkeakouluhaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      with TyhjatOhjausparametrit {
      valinnantulosRepository.getValinnantuloksetForValintatapajonoDBIO(
        valintatapajonoOid
      ) returns DBIO.successful(Set())
      yhdenPaikanSaannos.ottanutVastaanToisenPaikanDBIO(
        any[Hakukohde],
        any[Set[Valinnantulos]]
      ) returns DBIO.successful(Set())
      val valinnantulokset = List(valinnantulosA, valinnantulosB)
      service.storeValinnantuloksetAndIlmoittautumiset(
        valintatapajonoOid,
        valinnantulokset,
        Some(ZonedDateTime.now.toInstant),
        auditInfo
      ) mustEqual
        List(
          ValinnantulosUpdateStatus(
            404,
            s"Valinnantulosta ei löydy",
            valintatapajonoOid,
            hakemusOidA
          ),
          ValinnantulosUpdateStatus(
    ...
    "no status for unmodified valinnantulos" in new Mocks
      with Authorized
      with Korkeakouluhaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      ...
      valinnantulosRepository.getValinnantuloksetForValintatapajonoDBIO(
        valintatapajonoOid
      ) returns DBIO.successful(Set(valinnantulosA))
      yhdenPaikanSaannos.ottanutVastaanToisenPaikanDBIO(
        any[Hakukohde],
        any[Set[Valinnantulos]]
      ) returns DBIO.successful(Set(valinnantulosA))
    ...
    "no status for succesfully modified valinnantulos" in new Mocks
      with Authorized
      with Korkeakouluhaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      ...
      there was one(valinnantulosRepository).updateValinnantuloksenOhjaus(
        valinnantulokset(0).getValinnantuloksenOhjaus(session.personOid, "Virkailijan tallennus"),
        Some(lastModified)
      )
    }
    "vastaanotto can be stored for varasijalta hyvaksytty" in new Mocks
      with Authorized
      with Korkeakouluhaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      with TyhjatOhjausparametrit {
      val varasijaltaHyvaksytty =
        valinnantulosA.copy(julkaistavissa = Some(true), valinnantila = VarasijaltaHyvaksytty)
      valinnantulosRepository.getValinnantuloksetForValintatapajonoDBIO(
        valintatapajonoOid
      ) returns DBIO.successful(Set(varasijaltaHyvaksytty))
      yhdenPaikanSaannos.ottanutVastaanToisenPaikanDBIO(
        any[Hakukohde],
        any[Set[Valinnantulos]]
      ) returns DBIO.successful(Set(varasijaltaHyvaksytty))
      ...
      valinnantulosRepository.storeAction(any[VastaanottoEvent], any[Option[Instant]]) returns DBIO
        .successful(())
      service.storeValinnantuloksetAndIlmoittautumiset(
        valintatapajonoOid,
        List(
          varasijaltaHyvaksytty.copy(vastaanottotila = ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI)
        ),
        Some(lastModified),
        auditInfo
      ) mustEqual List()
      there was one(valinnantulosRepository).storeAction(
        VirkailijanVastaanotto(
          hakuOid,
          valintatapajonoOid,
          varasijaltaHyvaksytty.henkiloOid,
          varasijaltaHyvaksytty.hakemusOid,
          hakukohdeOid,
          VastaanotaSitovasti,
          session.personOid,
          "Virkailijan tallennus"
        ),
        Some(lastModified)
      )
    }
    "ehdollinen vastaanotto can be stored for varasijalta hyvaksytty" in new Mocks
      with Authorized
      with Korkeakouluhaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      ...
      valinnantulosRepository.getHakutoiveidenValinnantuloksetForHakemusDBIO(
        hakuOid,
        varasijaltaHyvaksytty.hakemusOid
      ) returns DBIO.successful(
        List(
      ...
      service.storeValinnantuloksetAndIlmoittautumiset(
        valintatapajonoOid,
        List(
          varasijaltaHyvaksytty.copy(vastaanottotila =
    ...
    "different statuses for all failing valinnantulokset" in new Mocks
      with Authorized
      with Korkeakouluhaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      ...
      valinnantulosRepository.getValinnantuloksetForValintatapajonoDBIO(
        valintatapajonoOid
      ) returns DBIO.successful(valinnantulokset1)
      yhdenPaikanSaannos.ottanutVastaanToisenPaikanDBIO(
        any[Hakukohde],
        any[Set[Valinnantulos]]
      ) returns DBIO.successful(valinnantulokset1)
      val valinnantulokset = List(
        ...
        valinnantulosB.copy(
          ...
          ehdollisenHyvaksymisenEhtoKoodi = Some(EhdollisenHyvaksymisenEhtoKoodi.EHTO_MUU)
        ),
        valinnantulosC.copy(
        ...
        valinnantulosG.copy(
          julkaistavissa = Some(false),
          vastaanottotila = ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI,
          ilmoittautumistila = PoissaKokoLukuvuosi
        )
      )
      service.storeValinnantuloksetAndIlmoittautumiset(
        valintatapajonoOid,
        ...
        ValinnantulosUpdateStatus(
          403,
          s"Valinnantilan muutos ei ole sallittu",
          valintatapajonoOid,
          valinnantulokset(0).hakemusOid
        ),
        ValinnantulosUpdateStatus(
          409,
          s"Valinnantuloksen ehdollisen hyväksynnän suomenkielistä ehtoa ei ole annettu.",
          valintatapajonoOid,
          valinnantulokset(1).hakemusOid
        ),
        ValinnantulosUpdateStatus(
          409,
          s"Valinnantulosta ei voida merkitä ei-julkaistavaksi, koska sen vastaanottotila on ${ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI}",
          valintatapajonoOid,
          valinnantulokset(2).hakemusOid
        ),
        ValinnantulosUpdateStatus(
          409,
          s"Ei voida hyväksyä varasijalta",
          valintatapajonoOid,
          valinnantulokset(3).hakemusOid
        ),
        ValinnantulosUpdateStatus(
          409,
          s"Hyväksy peruuntunut -arvoa ei voida muuttaa valinnantulokselle",
          valintatapajonoOid,
          valinnantulokset(4).hakemusOid
        ),
        ValinnantulosUpdateStatus(
          409,
          s"Vastaanottoa ei voi poistaa, koska ilmoittautuminen on tehty",
          valintatapajonoOid,
          valinnantulokset(5).hakemusOid
        ),
        ValinnantulosUpdateStatus(
          409,
          s"Valinnantulosta ei voida merkitä ei-julkaistavaksi, koska sen ilmoittautumistila on ${PoissaKokoLukuvuosi}",
          valintatapajonoOid,
          valinnantulokset(6).hakemusOid
        )
      )
    }
    "no authorization to change hyvaksyPeruuntunut" in new Mocks
      with Korkeakouluhaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      with TyhjatOhjausparametrit {
      ...
      authorizer.checkAccess(
        session,
        Set(tarjoajaOid),
        Set(Role.SIJOITTELU_PERUUNTUNEIDEN_HYVAKSYNTA_OPH)
      ) returns Left(new AuthorizationFailedException("error"))
      valinnantulosRepository.getValinnantuloksetForValintatapajonoDBIO(
        valintatapajonoOid
      ) returns DBIO.successful(Set(valinnantulosA.copy(valinnantila = Peruuntunut)))
      yhdenPaikanSaannos.ottanutVastaanToisenPaikanDBIO(
        any[Hakukohde],
        any[Set[Valinnantulos]]
      ) returns DBIO.successful(Set(valinnantulosA.copy(valinnantila = Peruuntunut)))
      val valinnantulokset =
        List(valinnantulosA.copy(valinnantila = Peruuntunut, hyvaksyPeruuntunut = Some(true)))
      service.storeValinnantuloksetAndIlmoittautumiset(
        valintatapajonoOid,
        valinnantulokset,
        Some(lastModified),
        ...
        ValinnantulosUpdateStatus(
          401,
          s"Käyttäjällä ${session.personOid} ei ole oikeuksia hyväksyä peruuntunutta",
          valintatapajonoOid,
          hakemusOidA
        )
      )
      there was no(valinnantulosRepository).updateValinnantuloksenOhjaus(
        any[ValinnantuloksenOhjaus],
        any[Option[Instant]]
      )
    }
    "authorization to change hyvaksyPeruuntunut" in new Mocks
      with Authorized
      with Korkeakouluhaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
    ...
    "no authorization to change julkaistavissa" in new Mocks
      with ToisenAsteenHaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      with ValintaesitysEiHyvaksyttavissa {
      authorizer.checkAccess(
        session,
        Set(tarjoajaOid),
        Set(Role.SIJOITTELU_READ_UPDATE, Role.SIJOITTELU_CRUD)
      ) returns Right(())
      authorizer.checkAccess(session, rootOrganisaatioOid, Set(Role.SIJOITTELU_CRUD)) returns Left(
        new AuthorizationFailedException("error")
      )
      valinnantulosRepository.getValinnantuloksetForValintatapajonoDBIO(
        valintatapajonoOid
      ...
      service.storeValinnantuloksetAndIlmoittautumiset(
        ...
        ValinnantulosUpdateStatus(
          401,
          s"Käyttäjällä ${session.personOid} ei ole oikeuksia julkaista valinnantulosta",
          valintatapajonoOid,
          hakemusOidA
        )
      )
    ...
    "no authorization to change julkaistavissa but valintaesitys is hyväksyttävissä" in new Mocks
      with ToisenAsteenHaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      with ValintaesitysHyvaksyttavissa {
      authorizer.checkAccess(
        session,
        Set(tarjoajaOid),
        Set(Role.SIJOITTELU_READ_UPDATE, Role.SIJOITTELU_CRUD)
    ...
    "authorization to change julkaistavissa" in new Mocks
      with ToisenAsteenHaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      with TyhjatOhjausparametrit {
      ...
      valinnantulosRepository.getValinnantuloksetForValintatapajonoDBIO(
        valintatapajonoOid
      ) returns DBIO.successful(Set(valinnantulosA.copy(valinnantila = Hyvaksytty)))
      yhdenPaikanSaannos.ottanutVastaanToisenPaikanDBIO(
        any[Hakukohde],
        any[Set[Valinnantulos]]
      ) returns DBIO.successful(Set(valinnantulosA.copy(valinnantila = Hyvaksytty)))
      ...
      val valinnantulokset =
        List(valinnantulosA.copy(valinnantila = Hyvaksytty, julkaistavissa = Some(true)))
      service.storeValinnantuloksetAndIlmoittautumiset(
        valintatapajonoOid,
        valinnantulokset,
        Some(lastModified),
      ...
      there was one(valinnantulosRepository).setHyvaksyttyJaJulkaistavissa(
        valinnantulosA.hakemusOid,
        valinnantulosA.valintatapajonoOid,
        session.personOid,
        "Virkailijan tallennus"
      )
    }
  ...
  "Erillishaku / ValinnantulosService" in {
    ...
    "different statuses for invalid valinnantulokset" in new Mocks
      with Authorized
      with KorkeakouluErillishaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      with TyhjatOhjausparametrit {
      valinnantulosRepository.getValinnantuloksetForValintatapajonoDBIO(
      ...
      val valinnantulokset = List(
        ...
        valinnantulosD
          .copy(valinnantila = Peruutettu, vastaanottotila = ValintatuloksenTila.PERUNUT),
        valinnantulosE.copy(
          valinnantila = Varalla,
          vastaanottotila = ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI
        ),
      ...
      service.storeValinnantuloksetAndIlmoittautumiset(
        ...
        ValinnantulosUpdateStatus(
          409,
          s"Ilmoittautumistieto voi olla ainoastaan hyväksytyillä ja vastaanottaneilla hakijoilla",
          valintatapajonoOid,
          valinnantulokset(0).hakemusOid
        ),
        ValinnantulosUpdateStatus(
        ...
        ValinnantulosUpdateStatus(
          409,
          s"Peruneella vastaanottajalla ei voi olla vastaanottotilaa",
          valintatapajonoOid,
          valinnantulokset(2).hakemusOid
        ),
        ValinnantulosUpdateStatus(
          409,
          s"Vastaanottaneen tai peruneen hakijan tulisi olla hyväksyttynä",
          valintatapajonoOid,
          valinnantulokset(3).hakemusOid
        ),
        ValinnantulosUpdateStatus(
          409,
          s"Hylätty tai varalla oleva hakija ei voi olla ilmoittautunut tai vastaanottanut",
          valintatapajonoOid,
          valinnantulokset(4).hakemusOid
        ),
        ValinnantulosUpdateStatus(
        ...
        ValinnantulosUpdateStatus(
          404,
          s"Valinnantulosta ei voida poistaa, koska sitä ei ole olemassa",
          valintatapajonoOid,
          valinnantulokset(6).hakemusOid
        )
      )
    ...
    "no status for succesfully modified valinnantulos" in new Mocks
      ...
      valinnantulosRepository.storeValinnantuloksenOhjaus(
        any[ValinnantuloksenOhjaus],
        any[Option[Instant]]
      ) returns DBIO.successful(())
      valinnantulosRepository.setHyvaksyttyJaJulkaistavissa(
      ...
      valinnantulosRepository.deleteHyvaksyttyJaJulkaistavissaIfExists(
        any[String],
        any[HakukohdeOid],
        any[Option[Instant]]
      ) returns DBIO.successful(())
      ...
      there was one(valinnantulosRepository).storeValinnantuloksenOhjaus(
        valinnantulokset(0).getValinnantuloksenOhjaus(session.personOid, "Erillishaun tallennus"),
        Some(lastModified)
      )
      there was one(valinnantulosRepository).setHyvaksyttyJaJulkaistavissa(
        valinnantulosA.hakemusOid,
        valintatapajonoOid,
        session.personOid,
        "Erillishaun tallennus"
      )
      there was no(valinnantulosRepository).updateValinnantuloksenOhjaus(
        any[ValinnantuloksenOhjaus],
        any[Option[Instant]]
      )
      there was no(valinnantulosRepository).storeIlmoittautuminen(
        any[String],
        any[Ilmoittautuminen],
        any[Option[Instant]]
      )
      there was no(valinnantulosRepository).storeValinnantila(
        any[ValinnantilanTallennus],
        any[Option[Instant]]
      )
    }
    "no status for succesfully deleted valinnantulos" in new Mocks
      with Authorized
      with KorkeakouluErillishaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      ...
      valinnantulosRepository.getValinnantuloksetForValintatapajonoDBIO(
        valintatapajonoOid
      ) returns DBIO.successful(Set(validiValinnantulos))
      valinnantulosRepository.deleteValinnantulos(
        any[String],
        any[Valinnantulos],
        any[Option[Instant]]
      ) returns DBIO.successful(())
      valinnantulosRepository.deleteIlmoittautuminen(
        any[String],
        any[Ilmoittautuminen],
        any[Option[Instant]]
      ) returns DBIO.successful(())
      ...
      there was one(valinnantulosRepository).deleteValinnantulos(
        session.personOid,
        valinnantulokset(0),
        Some(lastModified)
      )
      there was one(valinnantulosRepository).deleteIlmoittautuminen(
        validiValinnantulos.henkiloOid,
        Ilmoittautuminen(
          validiValinnantulos.hakukohdeOid,
          validiValinnantulos.ilmoittautumistila,
          session.personOid,
          "Erillishaun tallennus"
        ),
        Some(lastModified)
      )
      there was one(valinnantulosRepository).deleteHyvaksyttyJaJulkaistavissaIfExists(
        validiValinnantulos.henkiloOid,
        validiValinnantulos.hakukohdeOid,
        Some(lastModified)
      )
      there was no(valinnantulosRepository).deleteHyvaksynnanEhtoValintatapajonossa(
        any[HakemusOid],
        any[ValintatapajonoOid],
        any[HakukohdeOid],
        any[Instant]
      ...
      there was no(valinnantulosRepository).storeValinnantuloksenOhjaus(
        any[ValinnantuloksenOhjaus],
        any[Option[Instant]]
      )
      there was no(valinnantulosRepository).storeIlmoittautuminen(
    ...
    "no status for succesfully created tila/valinnantulos/ilmoittautuminen" in new Mocks
      with Authorized
      with KorkeakouluErillishaku
      with SuccessfulVastaanotto
      with NoConflictingVastaanotto
      ...
      valinnantulosRepository.storeValinnantila(
        any[ValinnantilanTallennus],
        any[Option[Instant]]
      ) returns DBIO.successful(())
      valinnantulosRepository.setHyvaksyttyJaJulkaistavissa(
      ...
      there was one(valinnantulosRepository).storeValinnantuloksenOhjaus(
        erillishaunValinnantulos
          .getValinnantuloksenOhjaus(session.personOid, "Erillishaun tallennus"),
        Some(lastModified)
      )
      there was one(valinnantulosRepository).storeIlmoittautuminen(
        erillishaunValinnantulos.henkiloOid,
        Ilmoittautuminen(
          erillishaunValinnantulos.hakukohdeOid,
          erillishaunValinnantulos.ilmoittautumistila,
          session.personOid,
          "Erillishaun tallennus"
        ),
        Some(lastModified)
      )
      there was one(valinnantulosRepository).storeValinnantila(
        erillishaunValinnantulos.getValinnantilanTallennus(session.personOid),
        Some(lastModified)
      )
  ...
  trait Mocks extends Mockito with Scope with MustThrownExpectations with RunBlockingMock {
    trait Repository
        extends ValinnantulosRepository
  ...
  trait SuccessfulVastaanotto { this: Mocks =>
    vastaanottoService.vastaanotaVirkailijana(any[List[VastaanottoEventDto]]) answers ((vs: Any) =>
      vs.asInstanceOf[List[VastaanottoEventDto]]
  ...
  trait NoConflictingVastaanotto { this: Mocks =>
    yhdenPaikanSaannos.apply(any[Set[Valinnantulos]]) answers ((vs: Any) =>
      Right(vs.asInstanceOf[Set[Valinnantulos]])
    )
  }
  ...
  trait Korkeakouluhaku { this: Mocks =>
    ...
    hakuService.getHaku(hakuOid) returns Right(
      Haku(
        oid = hakuOid,
        korkeakoulu = true,
        toinenAste = false,
  ...
  trait ValintaesitysEiHyvaksyttavissa { this: Mocks =>
    ohjausparametritService.ohjausparametrit(hakuOid) returns Right(
      Ohjausparametrit(
