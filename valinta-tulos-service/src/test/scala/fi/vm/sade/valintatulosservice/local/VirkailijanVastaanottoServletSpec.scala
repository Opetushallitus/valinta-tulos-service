...
class VirkailijanVastaanottoServletSpec
    ...
    with BeforeAfterAll
    with ForEach[(String, ValintatulosService, VastaanottoService)]
    with Mockito {
  ...
  def foreach[R: AsResult](
    f: ((String, ValintatulosService, VastaanottoService)) => R
    ...
    val servlet = new VirkailijanVastaanottoServlet(valintatulosService, vastaanottoService)(
      mock[Swagger],
      mock[VtsAppConfig]
    )
    ServletTest.withServlet(
      this,
      servlet,
      (uri: String) => AsResult(f((uri, valintatulosService, vastaanottoService)))
    )
  }
  ...
  "GET /virkailija/vastaanotot/haku/:hakuOid" in {
    "palauttaa 200 ja vastaanottorecordeja, jos löytyy YPSn piirissä olevia vastaanottoja" in {
      t: (String, ValintatulosService, VastaanottoService) =>
        ...
        t._2.haunKoulutuksenAlkamiskaudenVastaanototYhdenPaikanSaadoksenPiirissa(
          hakuOid
        ) returns Set(
          VastaanottoRecord(henkiloOid, hakuOid, hakukohdeOid, action, ilmoittaja, timestamp)
        )

        get(t._1 + "/vastaanotot/haku/" + hakuOid.toString) {
          status must_== 200
          (parse(body).extract[List[JObject]].head \ "action")
            .extract[String] must_== ValintatuloksenTila.VASTAANOTTANUT_SITOVASTI.name
        }
    }
  }
