...
class VastaanotettavuusServiceSpec extends Specification with MockitoMatchers with MockitoStubs {
  ...
  trait YhdenPaikanSaantoVoimassa extends Mockito with Scope with MustThrownExpectations { this: VastaanotettavuusServiceWithMocks =>
    ...
    val hakukohde = Hakukohde("1.2.246.562.20.00000000000", haku.oid, Seq(), List(koulutusOid), "KORKEAKOULUTUS", "TUTKINTO",
      Map("kieli_fi" -> "Hakukohteen nimi"), Map("fi" -> "Tarjoajan nimi"), yhdenPaikanSaanto = haku.yhdenPaikanSaanto, true, Some("S"), Some(2016))
  ...
  trait IlmanYhdenPaikanSaantoa extends Mockito with Scope with MustThrownExpectations { this: VastaanotettavuusServiceWithMocks =>
    ...
    val hakukohde = Hakukohde("1.2.246.562.20.00000000001", haku.oid, Seq(), List(koulutusOid), "AMMATILLINEN_PERUSKOULUTUS",
      "TUTKINTO_OHJELMA", Map("kieli_fi" -> "Hakukohteen nimi"), Map("fi" -> "Tarjoajan nimi"), yhdenPaikanSaanto = haku.yhdenPaikanSaanto, false, Some("S"), Some(2016))
    val kausi = Syksy(2015)
    val previousVastaanottoRecord = VastaanottoRecord(
      henkiloOid,
      haku.oid,
