...
trait ServletSpecification extends Specification with ITSetup with TimeWarp {
  sequential
  ...
  def postJSON[T](path: String, body: String, headers: Map[String, String] = Map.empty)(block: => T): T = {
    httpComponentsClient.post(path, body.getBytes("UTF-8"), Map("Content-type" -> "application/json") ++ headers)(block)
  }

  def patchJSON[T](path: String, body: String, headers: Map[String, String] = Map.empty)(block: => T): T = {
    httpComponentsClient.patch(path, body.getBytes("UTF-8"), Map("Content-type" -> "application/json") ++ headers)(block)
  }

  def get[A](uri: String)(f: => A): A = httpComponentsClient.get(uri)(f)
  def get[A](uri: String, params: Tuple2[String, String]*)(f: => A): A = httpComponentsClient.get(uri, params)(f)
  ...
  def post[A](uri: String, params: Tuple2[String, String]*)(f: => A): A = httpComponentsClient.post(uri, params)(f)
  def post[A](uri: String, params: Iterable[(String,String)])(f: => A): A = httpComponentsClient.post(uri, params)(f)
  def post[A](uri: String, params: Iterable[(String,String)], headers: Map[String, String])(f: => A): A = httpComponentsClient.post(uri, params, headers)(f)
  def post[A](uri: String, body: Array[Byte] = Array(), headers: Map[String, String] = Map.empty)(f: => A): A = httpComponentsClient.post(uri, body, headers)(f)
  def post[A](uri: String, params: Iterable[(String, String)], files: Iterable[(String, Any)])(f: => A): A = httpComponentsClient.post(uri, params, files)(f)
  ...
  def put[A](uri: String, body: Array[Byte] = Array(), headers: Map[String, String] = Map.empty)(f: => A): A =
