...
trait ValintarekisteriDbTools extends Specification with json4sCustomFormats {
  ...
  def createTestSession(
    roles: Set[Role] =
      Set(Role.SIJOITTELU_CRUD, Role(s"${Role.SIJOITTELU_CRUD.s}_1.2.246.562.10.39804091914"))
  ) =
    singleConnectionValintarekisteriDb
      .store(CasSession(ServiceTicket("myFakeTicket"), "1.2.246.562.24.1", roles))
      .toString
  ...
  def dateStringToTimestamp(str: String): Date = {
    new java.sql.Timestamp(
      new java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ").parse(str).getTime
    )
  }
  ...
  def compareSijoitteluWrapperToDTO(wrapper: SijoitteluWrapper, dto: SijoitteluajoDTO) = {
    ...
    dto.getHakukohteet.asScala.toList.foreach(dhakukohde => {
      ...
      dhakukohde.getHakijaryhmat.asScala.toList.foreach(dhakijaryhma => {
        val whakijaryhma =
          whakukohde.getHakijaryhmat.asScala.toList.find(_.getOid.equals(dhakijaryhma.getOid)).head
        ...
        dhakijaryhma.getHakemusOid.asScala.toList
          .diff(whakijaryhma.getHakemusOid.asScala.toList) mustEqual List()
      ...
      dhakukohde.getValintatapajonot.asScala.toList.foreach(dvalintatapajono => {
        val wvalintatapajono = whakukohde.getValintatapajonot.asScala.toList
          .find(_.getOid.equals(dvalintatapajono.getOid))
          .head
        ...
        format(dvalintatapajono.getVarasijojaKaytetaanAlkaen) mustEqual format(
          wvalintatapajono.getVarasijojaKaytetaanAlkaen
        )
        format(dvalintatapajono.getVarasijojaTaytetaanAsti) mustEqual format(
          wvalintatapajono.getVarasijojaTaytetaanAsti
        )
        ...
        dvalintatapajono.getHakemukset.asScala.toList.foreach(dhakemus => {
          val whakemus = wvalintatapajono.getHakemukset.asScala.toList
            .find(_.getHakemusOid.equals(dhakemus.getHakemusOid))
            .head
          ...
          dhakemus.getHyvaksyttyHakijaryhmista.asScala.diff(
            whakemus.getHyvaksyttyHakijaryhmista.asScala
          ) mustEqual Set()
  ...
  def compareSijoitteluWrapperToEntity(
    wrapper: SijoitteluWrapper,
  ...
  def loadSijoitteluFromFixture(
    fixture: String,
    path: String = "sijoittelu/",
    tallennaHakukohteet: Boolean = true
  ): SijoitteluWrapper = {
    val json = parse(
      getClass.getClassLoader.getResourceAsStream("fixtures/" + path + fixture + ".json")
    )
    SijoitteluWrapper.fromJson(json) match {
      case Some(wrapper) =>
        if (tallennaHakukohteet) {
          wrapper.hakukohteet.foreach(h =>
            insertHakukohde(HakukohdeOid(h.getOid), HakuOid(wrapper.sijoitteluajo.getHakuOid))
          )
        }
        wrapper
      case None =>
        throw new IllegalArgumentException("Could not get SijoitteluWrapper: no sijoittelus.")
    }
  }
  ...
  def findSijoitteluajo(sijoitteluajoId: Long): Option[SijoitteluAjo] = {
    singleConnectionValintarekisteriDb
      .runBlocking(sql"""select id, haku_oid, "start", "end"
            from sijoitteluajot
            where id = ${sijoitteluajoId}""".as[SijoitteluAjo])
      .headOption
  }
  ...
  private implicit val getSijoitteluajonValintatapajonoResult: GetResult[Valintatapajono] =
    GetResult(r => {
  ...
  def findHakukohteenValintatapajonot(hakukohdeOid: String): Seq[Valintatapajono] = {
    ...
    val sivssnovSijoittelunAlimmatVarallaolijat = singleConnectionValintarekisteriDb.runBlocking(
      sql"""select valintatapajono_oid, hakukohde_oid, jonosija, tasasijajonosija, tila, hakemusoidit
            from sivssnov_sijoittelun_varasijatayton_rajoitus where hakukohde_oid = ${hakukohdeOid}"""
        .as[(String, String, Int, Int, String, String)]
    )
    sivssnovSijoittelunAlimmatVarallaolijat.foreach { t =>
      valintatapajonot.find(_.getOid == t._1).foreach { jono =>
        ...
        jono.setSivssnovSijoittelunVarasijataytonRajoitus(
          java.util.Optional
            .of(new JonosijaTieto(t._3, t._4, Valinnantila(t._5).valinnantila, hakemusOids.asJava))
        )
      }
    }
  ...
  implicit val getHakemuksetForValintatapajonosResult: GetResult[HakemusRecord] = GetResult(r =>
    HakemusRecord(
  ...
  private def findValintatapajononJonosijat(valintatapajonoOid: String): Seq[Hakemus] = {
    val hakemukset = singleConnectionValintarekisteriDb
      .runBlocking(
        sql"""select
                vt.henkilo_oid,
                j.hakemus_oid,
      ...
      .toList
    val hakijaryhmaoidit = singleConnectionValintarekisteriDb
      .runBlocking(
        sql"""select distinct
               j.hakemus_oid,
               hh.hakijaryhma_oid
      ...
      .groupBy(t => HakemusOid(t._1))
      .mapValues(_.map(_._2).toSet)
    val tilankuvaukset = singleConnectionValintarekisteriDb.getValinnantilanKuvaukset(
      hakemukset.map(_.tilankuvausHash)
    )
    hakemukset
      .map(h => {
        SijoitteluajonHakemusWrapper(
          ...
          tila = h.tila,
          tilanKuvaukset =
            Some(tilankuvaukset(h.tilankuvausHash).tilankuvaukset(None).asScala.toMap),
  ...
  case class JonosijanTilankuvauksetResult(
    tila: Valinnantila,
    tilankuvausHash: Long,
    tarkenteenLisatieto: Option[String]
  )

  private implicit val findJonosijanTilaAndtilankuvauksetResult
    : GetResult[JonosijanTilankuvauksetResult] = GetResult(r => {
    JonosijanTilankuvauksetResult(
      tila = Valinnantila(r.nextString),
      tilankuvausHash = r.nextLong,
      tarkenteenLisatieto = r.nextStringOption
    )
  })

  private def findJonosijanTilaAndtilankuvaukset(
    hakemusOid: String,
    sijoitteluajoId: Long,
    valintatapajonoOid: String
  ) = {
    singleConnectionValintarekisteriDb
      .runBlocking(sql"""select tila, tilankuvaus_hash, tarkenteen_lisatieto
            from jonosijat j
         ...
         """.as[JonosijanTilankuvauksetResult])
      .head
  }

  def findTilanViimeisinMuutos(hakemusOid: String): Seq[java.sql.Timestamp] = {
    singleConnectionValintarekisteriDb.runBlocking(
      sql"""select tilan_viimeisin_muutos from valinnantilat where hakemus_oid = ${hakemusOid}"""
  ...
  def assertSijoittelu(wrapper: SijoitteluWrapper): MatchResult[Any] = {
    ...
    val storedHakukohteet: Seq[Hakukohde] = findSijoitteluajonHakukohteet(
      stored.get.getSijoitteluajoId
    )
    wrapper.hakukohteet.foreach(hakukohde => {
      ...
      SijoitteluajonHakukohdeWrapper(hakukohde) mustEqual SijoitteluajonHakukohdeWrapper(
        storedHakukohde.get
      )
      ...
      hakukohde.getValintatapajonot.asScala.toList.foreach(valintatapajono => {
        val storedValintatapajono =
          storedValintatapajonot.find(_.getOid.equals(valintatapajono.getOid))
        ...
        SijoitteluajonValintatapajonoWrapper(
          valintatapajono
        ) mustEqual SijoitteluajonValintatapajonoWrapper(storedValintatapajono.get)
        ...
        valintatapajono.getHakemukset.asScala.toList.foreach(hakemus => {
          ...
          val jonosijanTilankuvaukset = findJonosijanTilaAndtilankuvaukset(
            hakemus.getHakemusOid,
            wrapper.sijoitteluajo.getSijoitteluajoId,
      ...
      hakukohde.getHakijaryhmat.asScala.toList.foreach(hakijaryhma => {
        ...
        storedHakijaryhma.get.getHakemusOid.addAll(
          findHakijaryhmanHakemukset(hakijaryhma.getOid).asJava
        )
        ...
        createSortedHakijaryhmaWrapper(hakijaryhma) mustEqual createSortedHakijaryhmaWrapper(
          storedHakijaryhma.get
        )
      })
  ...
  def createHugeSijoittelu(
    sijoitteluajoId: Long,
    hakuOid: HakuOid,
    size: Int = 50,
    insertHakukohteet: Boolean = true
  ): SijoitteluWrapper = {
    val sijoitteluajo = SijoitteluajoWrapper(
      sijoitteluajoId,
      hakuOid,
    ...
    val hakukohteet = (1 to size par).map(i => {
      ...
      hakukohde.setValintatapajonot(
        ...
        .map(k => {
          ...
          val valintatapajono = SijoitteluajonValintatapajonoWrapper(
            valintatapajonoOid,
            "nimi" + k,
            k,
            Arvonta,
          ...
          valintatapajono.getHakemukset.addAll(
            ...
            .map(j => {
              ...
              val hakemus = SijoitteluajonHakemusWrapper(
                hakemusOid,
                Some(hakemusOid.toString),
                j,
                j,
                None,
                false,
                ...
                false,
                Hylatty,
                Some(Map("FI" -> ("fi" + j), "SV" -> ("sv" + j), "EN" -> ("en" + j))),
                EiTilankuvauksenTarkennetta,
                Set(""),
                List()
              ).hakemus
              valinnantulokset = valinnantulokset ++ IndexedSeq(
                SijoitteluajonValinnantulosWrapper(
                  ...
                  None,
                  None,
                  MailStatusWrapper(None, None, None, None).status
                )
              )
              hakemus
            })
      ...
      hakukohde.getValintatapajonot
        ...
        .getHakemukset()
        .asScala
        .foreach(h => h.setHyvaksyttyHakijaryhmista(hakijaryhmaOids))
      hakukohde
    })
    if (insertHakukohteet)
      hakukohteet.foreach(h => insertHakukohde(HakukohdeOid(h.getOid), hakuOid))
    SijoitteluWrapper(
      sijoitteluajo.sijoitteluajo,
      hakukohteet.seq.asJava,
      valinnantulokset.map(_.valintatulos).asJava
    )
  }
}
