...
class ValintarekisteriForSijoitteluSpec
  ...
  def getSijoittelu(hakuOid: String, sijoitteluajoId: String) = {
    val sijoitteluajo = time("Get sijoitteluajo") {
      valintarekisteri.getSijoitteluajo(hakuOid, sijoitteluajoId)
    }
    val hakukohteet = time("Get hakukohteet") {
      valintarekisteri.getSijoitteluajonHakukohteet(sijoitteluajo.getSijoitteluajoId)
    }
  ...
  "SijoitteluajoDTO with latest sijoitteluajoId should be fetched from database" in {
    val wrapper =
      loadSijoitteluFromFixture("valintatapajono_hakijaryhma_pistetiedot", "sijoittelu/")
    time("Store sijoittelu") { singleConnectionValintarekisteriDb.storeSijoittelu(wrapper) }
  ...
  "Sijoittelu and hakukohteet should be saved in database 1" in {
    val wrapper = loadSijoitteluFromFixture("haku-1.2.246.562.29.75203638285", "QA-import/")
    time("Tallenna sijoittelu") {
      valintarekisteri.tallennaSijoittelu(
        wrapper.sijoitteluajo,
        wrapper.hakukohteet.asJava,
        wrapper.valintatulokset.asJava
      )
    }
  ...
  "Sijoitteluajo should be stored in transaction" in {
    ...
    (valintarekisteri.tallennaSijoittelu(
      ...
      wrapper.hakukohteet.asJava,
      wrapper.valintatulokset.asJava
    ) must throwA[Exception]).message must contain(
      "null value in column \"hakemus_oid\" violates not-null constraint"
    )
  ...
  "Unknown sijoitteluajo cannot be found" in {
    ...
    valintarekisteri.getSijoitteluajo("1.2.246.562.29.75203638285", "1476936450192") must throwA(
      new IllegalArgumentException(
        s"Sijoitteluajoa 1476936450192 ei löytynyt haulle 1.2.246.562.29.75203638285"
      )
    )
  }
  ...
  "Tilahistoria is created correctly" in {
    ...
    updateTila(
      ...
      sijoitteluajo1Ajat._1,
      findHakemus(wrapper, hakukohdeOid, valintatapajonoOid, hakemusOid)
    )
    time("Store sijoittelu") { singleConnectionValintarekisteriDb.storeSijoittelu(wrapper) }
    ...
    val tallennettuTilahistoria =
      findTilahistoria(tallennettuSijoitteluajo, hakukohdeOid, valintatapajonoOid, hakemusOid)
    ...
    val aikaisemminTallennetunTilahistoria =
      findTilahistoria(aikaisempiSijoitteluajo, hakukohdeOid, valintatapajonoOid, hakemusOid)
    ...
    aikaisemminTallennetunTilahistoria
      .get(2)
      .getTila mustEqual HakemuksenTila.VARASIJALTA_HYVAKSYTTY
  ...
  "Tilahistoria is read for hakemukset that have not changed in requested sijoitteluajo" in {
    ...
    updateTila(
      HakemuksenTila.HYVAKSYTTY,
      sijoitteluajo1Ajat._1,
      findHakemus(wrapper, hakukohdeOid, valintatapajonoOid, hakemusOid1)
    )
    updateTila(
      HakemuksenTila.HYVAKSYTTY,
      sijoitteluajo1Ajat._1,
      findHakemus(wrapper, hakukohdeOid, valintatapajonoOid, hakemusOid2)
    )
    time("Store sijoittelu") { singleConnectionValintarekisteriDb.storeSijoittelu(wrapper) }
  ...
  "Poista hakijaryhmät, joiden jonoa ei ole sijoiteltu" in {
    ...
    def createHakukohde(
      oid: String,
      valintatapajonot: java.util.List[Valintatapajono],
      hakijaryhmat: java.util.List[Hakijaryhma]
    ) = {
    ...
    val hakukohteet = List(
      createHakukohde(
        "h1",
        List(
          createValintatapajono("v1"),
          createValintatapajono("v2"),
          createValintatapajono("v3")
        ).asJava,
        List(
          createHakijaryhma("r1", "v1"),
          createHakijaryhma("r2", null),
          createHakijaryhma("r3", "puuttuva")
        ).asJava
      ),
      createHakukohde(
        "h2",
        ...
        List(
          createHakijaryhma("r1", "v1"),
          createHakijaryhma("r2", "v2"),
          createHakijaryhma("r3", "v3")
        ).asJava
      ),
      createHakukohde(
        "h3",
        ...
        List(
          createHakijaryhma("r1", null),
          createHakijaryhma("r2", "puuttuva"),
          createHakijaryhma("r3", "puuttuva")
        ).asJava
      )
    )

    Valintarekisteri.poistaValintatapajonokohtaisetHakijaryhmatJoidenJonoaEiSijoiteltu(
      hakukohteet.asJava
    )
    ...
    hakukohteet
      ...
      .asScala
      .map(_.getOid)
      .diff(List("r1", "r2")) mustEqual List()
    hakukohteet
      .find(_.getOid == "h2")
      .get
      .getHakijaryhmat
      .asScala
      .map(_.getOid)
      .diff(List("r1", "r2", "r3")) mustEqual List()
    hakukohteet
      .find(_.getOid == "h3")
      .get
      .getHakijaryhmat
  ...
  private def findHakemus(
    ...
    wrapper.hakukohteet
      .find(_.getOid.equals(hakukohdeOid))
      .get
      .getValintatapajonot
      .asScala
      .find(_.getOid.equals(valintatapajonoOid))
      .get
      .getHakemukset
      .asScala
      .find(_.getHakemusOid.equals(hakemusOid))
      .get
  }

  private def findTilahistoria(
    sijoitteluajo: SijoitteluajoDTO,
    hakukohdeOid: String,
    valintatapajonoOid: String,
    hakemusOid: String
  ) = {
    sijoitteluajo.getHakukohteet.asScala
      .find(_.getOid.equals(hakukohdeOid))
      .get
      .getValintatapajonot
      .asScala
  ...
  private def updateSijoitteluajo(
