...
class ValintarekisteriDbValinnantuloksetSpec
  ...
  "ValintarekisteriDb" should {
    "store ilmoittautuminen" in {
      singleConnectionValintarekisteriDb.runBlocking(
        singleConnectionValintarekisteriDb.storeIlmoittautuminen(henkiloOid, ilmoittautuminen)
      )
      singleConnectionValintarekisteriDb
        .runBlocking(sql"""select henkilo, selite from ilmoittautumiset""".as[(String, String)])
        .head must_== (henkiloOid, "selite")
    }
    "update ilmoittautuminen" in {
      ...
      singleConnectionValintarekisteriDb.runBlocking(
        singleConnectionValintarekisteriDb.storeIlmoittautuminen(
          henkiloOid,
          ilmoittautuminen.copy(tila = PoissaSyksy, selite = "syksyn poissa")
        )
      )
      singleConnectionValintarekisteriDb
        .runBlocking(sql"""select henkilo, selite from ilmoittautumiset""".as[(String, String)])
        .head must_== (henkiloOid, "syksyn poissa")
      singleConnectionValintarekisteriDb
        .runBlocking(
          sql"""select henkilo from ilmoittautumiset_history where selite = 'selite'""".as[String]
        )
        .head must_== henkiloOid
    }
    "delete ilmoittautuminen" in {
      ...
      singleConnectionValintarekisteriDb
        ...
        .head must_== (henkiloOid, "selite")
      singleConnectionValintarekisteriDb.runBlocking(
        singleConnectionValintarekisteriDb.deleteIlmoittautuminen(henkiloOid, ilmoittautuminen)
      )
      singleConnectionValintarekisteriDb
        .runBlocking(sql"""select henkilo, selite from ilmoittautumiset""".as[(String, String)])
        .size must_== 0
    ...
    "update valinnantuloksen ohjaustiedot" in {
      ...
      singleConnectionValintarekisteriDb
        .getValinnantuloksetForValintatapajono(valintatapajonoOid)
        .head
        .julkaistavissa must beSome(false)
      singleConnectionValintarekisteriDb.runBlocking(
        singleConnectionValintarekisteriDb.updateValinnantuloksenOhjaus(
      ...
      singleConnectionValintarekisteriDb
        .getValinnantuloksetForValintatapajono(valintatapajonoOid)
        .head
        .julkaistavissa must beSome(true)
    }
    "not update valinnantuloksen ohjaustiedot if modified" in {
      ...
      singleConnectionValintarekisteriDb.runBlocking(
        singleConnectionValintarekisteriDb.updateValinnantuloksenOhjaus(
          ...
          Some(notModifiedSince)
        )
      ) must throwA[ConcurrentModificationException]
    }
    "store valinnantila" in {
      singleConnectionValintarekisteriDb.getValinnantuloksetForValintatapajono(
        valintatapajonoOid
      ) mustEqual Set()
      singleConnectionValintarekisteriDb.runBlocking(
    ...
    "update existing valinnantila" in {
      ...
      singleConnectionValintarekisteriDb.runBlocking(
        singleConnectionValintarekisteriDb.storeValinnantila(
          valinnantilanTallennus.copy(valinnantila = VarasijaltaHyvaksytty)
        )
      )
    ...
    "store valinnantuloksen ohjaustiedot" in {
      ...
      (
        result.head.julkaistavissa,
        result.head.ehdollisestiHyvaksyttavissa,
        result.head.hyvaksyPeruuntunut,
        result.head.hyvaksyttyVarasijalta
      ) mustEqual (None, Some(false), None, None)
    ...
    "delete valinnantulos" in {
      ...
      singleConnectionValintarekisteriDb
        ...
        .size mustEqual 1
      singleConnectionValintarekisteriDb.runBlocking(
        singleConnectionValintarekisteriDb.deleteHyvaksynnanEhtoValintatapajonossa(
          hakemusOid,
          valintatapajonoOid,
          hakukohdeOid,
          Instant.now()
      ...
      singleConnectionValintarekisteriDb.runBlocking(
        singleConnectionValintarekisteriDb.deleteValinnantulos(
          muokkaaja,
          valinnantulos.copy(poistettava = Some(true))
        )
      )
      singleConnectionValintarekisteriDb.getValinnantuloksetForValintatapajono(
        valintatapajonoOid
    ...
    "generate muutoshistoria from updates" in {
      ...
      val result = singleConnectionValintarekisteriDb.getMuutoshistoriaForHakemus(
        hakemusOid,
        valintatapajonoOid
      )
      result.size must_== 3
      val julkaistavissaUpdate = result.head
      julkaistavissaUpdate.changes.head must_== KentanMuutos(
      ...
      ehdollisestiHyvaksyttavissaUpdate.changes.head must_== KentanMuutos(
        field = "ehdollisestiHyvaksyttavissa",
        from = Some(true),
        to = false
      )
      ehdollisestiHyvaksyttavissaUpdate.changes.size must_== 1
      val origin = result.tail.tail.head
      origin.changes must contain(
        KentanMuutos(field = "valinnantila", from = None, to = Hyvaksytty)
      )
      origin.changes must contain(
        KentanMuutos(field = "valinnantilanViimeisinMuutos", from = None, to = muutos)
      )
      ...
      origin.changes must contain(
        KentanMuutos(field = "ehdollisenHyvaksymisenEhtoKoodi", from = None, to = "muu")
      )
      origin.changes must contain(
        KentanMuutos(field = "ehdollisenHyvaksymisenEhtoFI", from = None, to = "muu")
      )
      origin.changes must contain(
        KentanMuutos(field = "ehdollisenHyvaksymisenEhtoSV", from = None, to = "andra")
      )
      origin.changes must contain(
        KentanMuutos(field = "ehdollisenHyvaksymisenEhtoEN", from = None, to = "other")
      )
      origin.changes must contain(
        KentanMuutos(field = "hyvaksyttyVarasijalta", from = None, to = false)
      )
      origin.changes must contain(
        KentanMuutos(field = "hyvaksyPeruuntunut", from = None, to = false)
      )
      result.size must_== 3
    }
    ...
    "vastaanotto -> delete -> vastaanotto should be correct in muutoshistoria" in {
      ...
      singleConnectionValintarekisteriDb.runBlocking(
        singleConnectionValintarekisteriDb.storeAction(
          HakijanVastaanotto(henkiloOid, hakemusOid, hakukohdeOid, VastaanotaSitovasti)
        )
      )
      singleConnectionValintarekisteriDb.runBlocking(
        singleConnectionValintarekisteriDb.storeAction(
      ...
      result(0).changes.head must_== KentanMuutos(
        field = "vastaanottotila",
        from = Some("Kesken (poistettu)"),
        to = VastaanotaSitovasti.valintatuloksenTila
      )
      result(1).changes.size must_== 3
      result(1).changes must contain(
        KentanMuutos(
          field = "vastaanottotila",
          from = Some(VastaanotaSitovasti.valintatuloksenTila),
          to = "Kesken (poistettu)"
        )
      )
      result(2).changes.size must_== 1
      result(2).changes.head must_== KentanMuutos(
        field = "vastaanottotila",
        from = None,
        to = VastaanotaSitovasti.valintatuloksenTila
      )
      result(3).changes.size must_== 1
      result(3).changes.head must_== KentanMuutos(
        field = "julkaistavissa",
        from = Some(false),
        to = true
      )
      result(4).changes must contain(
        KentanMuutos(field = "valinnantila", from = None, to = Hyvaksytty)
      )
      result(4).changes must contain(
        KentanMuutos(field = "valinnantilanViimeisinMuutos", from = None, to = muutos)
      ...
      result(4).changes must contain(
        KentanMuutos(field = "julkaistavissa", from = None, to = false)
      )
      result(4).changes must contain(
        KentanMuutos(field = "ehdollisestiHyvaksyttavissa", from = None, to = true)
      )
    ...
    "skip kludged special case of overriding vastaanotto deleting vastaanotto" in {
      ...
      val result = singleConnectionValintarekisteriDb
        .getMuutoshistoriaForHakemus(hakemusOid, valintatapajonoOid)
        .sortBy(_.timestamp)
      result must have size 3
      val ehdollinenVastaanotto = result(1)
      val sitovaVastaanotto = result(2)
      ehdollinenVastaanotto.changes must have size 1
      ehdollinenVastaanotto.changes must contain(
        KentanMuutos(field = "vastaanottotila", from = None, to = EHDOLLISESTI_VASTAANOTTANUT)
      )
      sitovaVastaanotto.changes must have size 1
      sitovaVastaanotto.changes must contain(
        KentanMuutos(
          field = "vastaanottotila",
          from = Some(EHDOLLISESTI_VASTAANOTTANUT),
          to = VASTAANOTTANUT_SITOVASTI
        )
      )
    }
    "update julkaistavissa and hyväksytty/julkaistu dates for valintatapajono" in {
      ...
      singleConnectionValintarekisteriDb.runBlocking(
        DBIO.seq(
          singleConnectionValintarekisteriDb
            .setJulkaistavissa(valintatapajonoOid, "ilmoittaja", "selite"),
          singleConnectionValintarekisteriDb.setHyvaksyttyJaJulkaistavissa(
            valintatapajonoOid,
            "ilmoittaja",
            "selite"
          )
    ...
    "do not update hyväksytty/julkaistu date for hylätty valinnantulos" in {
      storeValinnantilaAndValinnantulos()
      singleConnectionValintarekisteriDb.runBlocking(
        sqlu"update valinnantilat set tila = 'Hylatty'::valinnantila"
      )
      checkJulkaistavissa() must_== false
      checkHyvaksyttyJaJulkaistu() must_== None
      singleConnectionValintarekisteriDb.runBlocking(
    ...
    "update hyväksytty/julkaistu date for valinnantulos" in {
      ...
      singleConnectionValintarekisteriDb.runBlocking(
        sqlu"update valinnantilat set tila = 'VarasijaltaHyvaksytty'::valinnantila"
      )
    ...
    "delete hyväksytty/julkaistu date" in {
      ...
      singleConnectionValintarekisteriDb.runBlocking(
        DBIO.seq(
          singleConnectionValintarekisteriDb.deleteHyvaksyttyJaJulkaistavissa(
            henkiloOid,
            hakukohdeOid
          )
        )
    ...
    "delete hyväksytty/julkaistu date if exists" in {
      ...
      singleConnectionValintarekisteriDb.runBlocking(
        DBIO.seq(
          singleConnectionValintarekisteriDb.deleteHyvaksyttyJaJulkaistavissaIfExists(
            henkiloOid,
            hakukohdeOid
          )
        )
  ...
  private def checkJulkaistavissa(): Boolean = {
    singleConnectionValintarekisteriDb
      .runBlocking(
        sql"select julkaistavissa from valinnantulokset where hakemus_oid = ${hakemusOid}"
          .as[Boolean]
      )
      .head
  }
  ...
  private def checkHyvaksyttyJaJulkaistu(): Option[Timestamp] = {
    singleConnectionValintarekisteriDb
      .runBlocking(
        sql"select hyvaksytty_ja_julkaistu from hyvaksytyt_ja_julkaistut_hakutoiveet where henkilo = ${henkiloOid} and hakukohde = ${hakukohdeOid}"
          .as[Timestamp]
      )
      .headOption
  }
  ...
  private def assertValintaesityksetHistory() = {
    singleConnectionValintarekisteriDb.runBlocking(
      sql"""select 1 from valintaesitykset_history where valintatapajono_oid = ${valintatapajonoOid.toString}"""
        .as[Int]
    ) must beEmpty
  }
  ...
  def assertValinnantuloksenOhjausHistory(
    count: Int,
    valinnantuloksenOhjaus: ValinnantuloksenOhjaus
  ) = {
    val result = singleConnectionValintarekisteriDb.runBlocking(
  ...
  def assertHyvaksymiskirjeet(lahetetty: java.util.Date) = {
    ...
    result.size must_== 1
    java.sql.Timestamp.valueOf(result.head.dropRight(3)) must_== new java.sql.Timestamp(
      lahetetty.getTime
    )
  }
  ...
  def assertHyvaksymiskirjeetHistory(count: Int, lahetetty: java.util.Date) = {
    ...
    result.size must_== count
    java.sql.Timestamp.valueOf(result.last.dropRight(3)) must_== new java.sql.Timestamp(
      lahetetty.getTime
    )
  }
