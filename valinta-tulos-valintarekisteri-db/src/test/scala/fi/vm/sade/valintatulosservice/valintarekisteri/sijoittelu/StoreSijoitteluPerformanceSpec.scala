...
@Ignore
class StoreSijoitteluPerformanceSpec
  ...
  val hakuOid =
    HakuOid(
      "1.2.246.562.29.59856749474"
    ) //HakuOid("1.2.246.562.29.57263577488") // HakuOid("1.2.246.562.29.10108985853")
  ...
  def readSijoitteluFromDb: SijoitteluWrapper =
    Try(
      new ValintarekisteriDb(
        DbConfig(
          dbUrl,
          Some(dbUser),
          Some(dbPasswd),
        ...
        try {
          ...
          MemoryTracker
            .memoryUsage(label) {
              Timer.timed(label, 0) {
                SijoitteluWrapper(
                  valintarekisteriQa.getSijoitteluajo(hakuOid.s, s"$latest"),
  ...
  def storeSijoittelu(
    sijoitteluWrapper: SijoitteluWrapper,
    stopWatch: StopWatch,
    times: Int = 1
  ): Unit = {
  ...
  def testStoreSijoittelu(sijoitteluWrapper: SijoitteluWrapper, times: Int = 1): Unit = {
    sijoitteluWrapper.hakukohteet.foreach(h =>
      singleConnectionValintarekisteriDb.storeHakukohde(
        YPSHakukohde(HakukohdeOid(h.getOid), hakuOid, Kevat(2017))
      )
    )
