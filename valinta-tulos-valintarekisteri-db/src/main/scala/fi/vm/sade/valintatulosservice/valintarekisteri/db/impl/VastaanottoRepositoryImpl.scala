...
trait VastaanottoRepositoryImpl
    extends HakijaVastaanottoRepository
    with VirkailijaVastaanottoRepository
  ...
  private def findkoulutuksenAlkamiskaudenVastaanottaneetYhdenPaikanSaadoksenPiirissaDBIO(
    kausi: Kausi
  ...
  override def findkoulutuksenAlkamiskaudenVastaanottaneetYhdenPaikanSaadoksenPiirissa(
    kausi: Kausi
  ...
  private def findYpsHakemusOiditDBIO(
    kausi: Kausi
  ): DBIO[List[((String, HakukohdeOid), HakemusOid)]] = {
    sql"""select
  ...
  override def findYpsVastaanotot(
    kausi: Kausi,
  ...
  override def findYpsVastaanototDBIO(
    kausi: Kausi,
    henkiloOids: Set[String]
    ...
    findkoulutuksenAlkamiskaudenVastaanottaneetYhdenPaikanSaadoksenPiirissaDBIO(kausi)
      .zip(findYpsHakukohteetDBIO(kausi))
      .zip(findYpsHakemusOiditDBIO(kausi))
      .map {
        case ((x, y), z) => {
          (x.filter(v => henkiloOids.contains(v.henkiloOid)), y.map(h => h.oid -> h).toMap, z.toMap)
        }
      }
      .flatMap {
  ...
  private def assertAllValinnantuloksetExist(
    vastaanotot: Set[VastaanottoRecord],
    hakemusoidit: Map[(String, HakukohdeOid), HakemusOid]
  ): Unit = {
    val missingRecords: Map[HakukohdeOid, Set[VastaanottoRecord]] = vastaanotot
      .filter(v => !hakemusoidit.isDefinedAt(v.henkiloOid, v.hakukohdeOid))
      .groupBy(_.hakukohdeOid)
    if (!missingRecords.isEmpty) {
      throw new IllegalStateException(s"Puuttuvia valinnantuloksia ${missingRecords
        .map(e => s"hakukohteessa ${e._1} henkiloOideille ${e._2.map(_.henkiloOid).mkString(",")}")
        .mkString(", ")}")
    }
  }

  override def aliases(henkiloOid: String): DBIO[Set[String]] = {
    sql"""select linked_oid from henkiloviitteet where person_oid = ${henkiloOid}"""
      .as[String]
      .map(_.toSet)
  }
        ...
        if (retries > 0) {
          ...
          Left(
            new ConcurrentModificationException(
              s"$description failed because of an concurrent action.",
              e
            )
          )
        }
  ...
  override def findHenkilonVastaanototHaussa(
    henkiloOid: String,
    hakuOid: HakuOid
  ): DBIO[Set[VastaanottoRecord]] = {
    sql"""select henkilo, haku_oid, hakukohde, action, ilmoittaja, "timestamp"
  ...
  override def findHenkilonVastaanotot(
    personOid: String,
  ...
  override def findHenkilonVastaanottoHakukohteeseen(
    personOid: String,
    ...
    sql"""select henkilo, haku_oid, hakukohde, action, ilmoittaja, "timestamp"
      ...
      .as[VastaanottoRecord]
      .map(vastaanottoRecords => {
        if (vastaanottoRecords.size > 1) {
          throw ConflictingAcceptancesException(
            personOid,
            vastaanottoRecords,
            "samaan hakukohteeseen"
          )
        } else {
  ...
  override def findYhdenPaikanSaannonPiirissaOlevatVastaanotot(
    personOid: String,
  ...
  override def store(vastaanottoEvent: VastaanottoEvent, vastaanottoDate: Date) = {
    ...
    runBlocking(
      sqlu"""insert into vastaanotot (hakukohde, henkilo, action, ilmoittaja, selite, timestamp)
              values ($hakukohdeOid, $henkiloOid, ${action.toString}::vastaanotto_action, $ilmoittaja, $selite, ${new java.sql.Timestamp(
        vastaanottoDate.getTime
      )})"""
    )
  }
  ...
  override def store(
    ...
    runAsSerialized(
      ...
      s"Storing $vastaanottoEvents",
      DBIO
        .sequence(vastaanottoEvents.map(storeAction))
        .andThen(postCondition)
  ...
  private def tallennaVastaanottoTapahtumaAction(
    vastaanottoEvent: VastaanottoEvent,
    ifUnmodifiedSince: Option[Instant]
  ): DBIO[Unit] = {
    val VastaanottoEvent(henkiloOid, _, hakukohdeOid, action, ilmoittaja, selite) = vastaanottoEvent
    val deleteVastaanotto =
    ...
    deleteVastaanotto.andThen(insertVastaanotto).flatMap {
      case 0 =>
        DBIO.failed(
          new ConcurrentModificationException(
            s"Vastaanottoa $vastaanottoEvent ei voitu päivittää, koska joku oli muokannut sitä samanaikaisesti (${format(ifUnmodifiedSince)})"
          )
        )
      case n =>
        DBIO.successful(())
  ...
  private def kumoaVastaanottotapahtumatAction(
    vastaanottoEvent: VastaanottoEvent,
    ifUnmodifiedSince: Option[Instant]
  ): DBIO[Unit] = {
    ...
    insertDelete.andThen(updateVastaanotto).flatMap {
      case 0 =>
        DBIO.failed(
          new ConcurrentModificationException(
            s"Vastaanottoa $vastaanottoEvent ei voitu päivittää koska sitä ei ole tai joku oli muokannut sitä samanaikaisesti (${ifUnmodifiedSince})"
          )
        )
      case n =>
        DBIO.successful(())
  ...
  override def findHyvaksyttyJulkaistuDatesForHenkilo(
    ...
    sql"""select
          ...
          where henkiloviitteet.person_oid = ${henkiloOid}"""
      .as[(HakukohdeOid, OffsetDateTime)]
      .map(_.toMap)

  override def findHyvaksyttyJulkaistuDatesForHaku(
    hakuOid: HakuOid
  ): Map[HenkiloOid, Map[HakukohdeOid, OffsetDateTime]] =
    timed(s"Hyväksytty ja julkaistu -pvm haulle $hakuOid", 100) {
      runBlocking(sql"""select hjh.henkilo, hjh.hakukohde, hjh.hyvaksytty_ja_julkaistu
        ...
        .groupBy(_._1)
        .map { case (k, v) => (k, v.map(t => (t._2, t._3)).toMap) }
    }

  override def findHyvaksyttyJulkaistuDatesForHakukohde(
    hakukohdeOid: HakukohdeOid
  ): Map[HenkiloOid, OffsetDateTime] =
    timed(s"Hyväksytty ja julkaistu -pvm hakukohteelle $hakukohdeOid", 100) {
      runBlocking(sql"""select henkilo, hyvaksytty_ja_julkaistu
              from hyvaksytyt_ja_julkaistut_hakutoiveet
              where hakukohde = ${hakukohdeOid}""".as[(HenkiloOid, OffsetDateTime)]).toMap
    }

  override def findHyvaksyttyJaJulkaistuDateForHenkiloAndHakukohdeDBIO(
    henkiloOid: HenkiloOid,
    hakukohdeOid: HakukohdeOid
