...
object OrganisaatioService {
  def apply(appConfig: AppConfig): OrganisaatioService =
    new CachedOrganisaatioService(new RealOrganisaatioService(appConfig))
}
class CachedOrganisaatioService(realOrganisaatioService: RealOrganisaatioService)
...
class RealOrganisaatioService(appConfig: AppConfig) extends OrganisaatioService {
  ...
  override def hae(oid: String): Either[Throwable, Organisaatiot] = {
    val url = appConfig.ophUrlProperties.url(
    ...
    fetch(url) { response =>
      ...
      case e: IllegalArgumentException =>
        new IllegalArgumentException(s"No organisaatio $oid found", e)
      case e: IllegalStateException =>
        new IllegalStateException(s"Parsing organisaatio $oid failed", e)
