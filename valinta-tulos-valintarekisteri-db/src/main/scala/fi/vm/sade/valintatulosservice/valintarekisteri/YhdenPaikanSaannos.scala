...
class YhdenPaikanSaannos(
  ...
  def ottanutVastaanToisenPaikanDBIO(
    hakukohde: Hakukohde,
    ...
    if (hakukohde.yhdenPaikanSaanto.voimassa) {
      hakukohde.koulutuksenAlkamiskausi match {
        ...
        case Some(kausi) =>
          virkailijaVastaanottoRepository
            .findYpsVastaanototDBIO(kausi, valinnantulokset.map(_.henkiloOid))
            .flatMap(vastaanotot =>
              DBIO.successful(
                valinnantulokset.map(v =>
                  ottanutVastaanToisenPaikan(v, vastaanotot.find(_._3.henkiloOid == v.henkiloOid))
                )
              )
            )
      }
  ...
  def getYpsTuloksetForManyHakemukses(
    tuloksetJaHakukohteet: Set[(Valinnantulos, Hakukohde)]
        ...
        byKausi.map((kth: (Option[Kausi], Set[(Valinnantulos, Hakukohde)])) =>
          kth._1 match {
            case None =>
              Left(
                new IllegalStateException(
                  s"Yhden paikan säännön piirissä olevissa hakukohteissa ${kth._2.map(hk => hk._2.oid)} ei ole koulutuksen alkamiskautta."
                )
              )
            case Some(kausi) =>
              val vastaanotot = virkailijaVastaanottoRepository
                ...
                .toMap
              Right(
                kth._2.map(v => ottanutVastaanToisenPaikan(v._1, vastaanotot.get(v._1.henkiloOid)))
              )
          }
        )
      )
  ...
  private def ottanutVastaanToisenPaikan(
    ...
    vastaanotto: Option[(HakemusOid, HakukohdeRecord, VastaanottoRecord)]
  ): Valinnantulos = {
    ...
    val ehdollinenVastaanottoToisellaHakemuksella =
      vastaanotto.exists(v =>
        v._1 != valinnantulos.hakemusOid && v._3.action == VastaanotaEhdollisesti
      )
    if (
      valinnantulos.vastaanottotila == ValintatuloksenTila.KESKEN &&
      Set[Valinnantila](Hyvaksytty, VarasijaltaHyvaksytty, Varalla).contains(
        valinnantulos.valinnantila
      ) &&
