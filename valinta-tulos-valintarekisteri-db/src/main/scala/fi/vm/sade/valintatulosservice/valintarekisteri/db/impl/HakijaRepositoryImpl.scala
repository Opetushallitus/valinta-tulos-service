...
trait HakijaRepositoryImpl extends HakijaRepository with ValintarekisteriRepository {
  ...
  override def getHakemuksenHakija(
    hakemusOid: HakemusOid,
    sijoitteluajoId: Option[Long] = None
  ): Option[HakijaRecord] =
  ...
  override def getHakukohteenHakijat(
  ...
  private def replaceHakijaOidsWithLatest(
    hakijatJaAjat: List[(HakijaRecord, OffsetDateTime)]
  ): List[HakijaRecord] = {
    val multipleHakijasForHakemus: Seq[List[(HakijaRecord, OffsetDateTime)]] =
      hakijatJaAjat.groupBy(_._1.hakemusOid).values.filter(_.size > 1).toList
    ...
    val henkiloviitteet: Set[(String, String)] =
      timed(s"Henkilöviitteiden haku ${hakijaOids.size} hakijaOidille", 1000) {
        runBlocking(sql"""
           SELECT * FROM henkiloviitteet WHERE person_oid IN (#${oidsIn})
        """.as[(String, String)]).toSet
      }
        ...
        aliases.foreach { hakija2 =>
          ...
          if (oid1 != oid2 && !henkiloviitteet.contains((oid1, oid2))) {
            throw new RuntimeException(
              s"henkiloviitteet-taulu ei ajan tasalla: ei sisältänyt linkitystä ($oid1, $oid2). Ei voida korjata saman hakemuksen ${hakija1._1.hakemusOid} eri hakijaOideja."
            )
          }
        }
      }
    ...
    hakijatJaAjat
      .map({
        case (hakija, aika) if latestHakemusToHakijaMap.contains(hakija.hakemusOid) =>
          ...
          logger.debug(
            s"Korvataan hakijaOid ${hakija.hakijaOid} viimeisimmällä oidilla ${hakijaOidFromLatest}"
          )
  ...
  override def getHakemuksenHakutoiveetSijoittelussa(
    hakemusOid: HakemusOid,
    sijoitteluajoId: Long
  ): DBIOAction[List[HakutoiveRecord], NoStream, Effect] = {
    sql"""select distinct
                 j.hakemus_oid,
                 j.prioriteetti,
                 j.hakukohde_oid,
  ...
  override def getHakukohteenHakemuksienHakutoiveetSijoittelussa(
    hakukohdeOid: HakukohdeOid,
    sijoitteluajoId: Long
  ): List[HakutoiveRecord] =
    timed(
      s"Sijoitteluajon $sijoitteluajoId hakukohteen $hakukohdeOid hakemuksien hakutoiveiden haku",
      100
    ) {
      runBlocking(sql"""with hakemukset as (
  ...
  override def getHaunHakemuksienHakutoiveetSijoittelussa(
    hakuOid: HakuOid,
    sijoitteluajoId: Long
  ): List[HakutoiveRecord] =
    timed(s"Sijoitteluajon $sijoitteluajoId haun $hakuOid hakemuksien hakutoiveiden haku", 100) {
      runBlocking(
        sql"""select distinct j.hakemus_oid, j.prioriteetti, j.hakukohde_oid, sh.kaikki_jonot_sijoiteltu
              from jonosijat j
              left join sijoitteluajon_hakukohteet sh on sh.hakukohde_oid = j.hakukohde_oid and sh.sijoitteluajo_id = j.sijoitteluajo_id
              where j.sijoitteluajo_id = ${sijoitteluajoId}""".as[HakutoiveRecord]
      ).toList
    }

  override def getHakukohteenHakemuksienHakijaryhmatSijoittelussa(
    hakukohdeOid: HakukohdeOid,
    sijoitteluajoId: Long
  ): Map[HakemusOid, List[HakutoiveenHakijaryhmaRecord]] =
    timed(
      s"Sijoitteluajon $sijoitteluajoId hakukohteen $hakukohdeOid hakemusten hakijaryhmien haku",
      100
    ) {
      runBlocking(
        sql"""with hakemukset as (
              ...
              select hh.hakemus_oid, h.oid, h.nimi, h.hakukohde_oid, h.valintatapajono_oid,
                ...
                where hh.sijoitteluajo_id = ${sijoitteluajoId}"""
          .as[(HakemusOid, HakutoiveenHakijaryhmaRecord)]
      ).groupBy(_._1).map { case (k, v) => (k, v.map(_._2).toList) }
    }

  override def getHaunHakemuksienHakijaryhmatSijoittelussa(
    hakuOid: HakuOid,
    sijoitteluajoId: Long
  ): Map[HakemusOid, List[HakutoiveenHakijaryhmaRecord]] =
  ...
  override def getHakukohteenHakemuksienHakutoiveSijoittelussa(
    hakukohdeOid: HakukohdeOid,
    sijoitteluajoId: Long
  ): List[HakutoiveRecord] =
    timed(
      s"Sijoitteluajon $sijoitteluajoId hakukohteen $hakukohdeOid hakemuksien hakutoiveen haku",
      100
    ) {
      runBlocking(
        sql"""select distinct j.hakemus_oid, j.prioriteetti, j.hakukohde_oid, sh.kaikki_jonot_sijoiteltu
  ...
  override def getHakemuksenHakutoiveidenValintatapajonotSijoittelussa(
    hakemusOid: HakemusOid,
    sijoitteluajoId: Long
  ): DBIOAction[List[HakutoiveenValintatapajonoRecord], NoStream, Effect] = {
  ...
  override def getHakukohteenHakemuksienValintatapajonotSijoittelussa(
    hakukohdeOid: HakukohdeOid,
    sijoitteluajoId: Long
  ): List[HakutoiveenValintatapajonoRecord] =
    timed(
      s"Sijoitteluajon $sijoitteluajoId hakukohteen $hakukohdeOid hakutoiveiden valintatapajonojen haku",
      100
    ) {
      runBlocking(
        sql"""with hakemukset as (
          ...
          .as[HakutoiveenValintatapajonoRecord]
      ).toList
    }

  override def getHaunHakemuksienValintatapajonotSijoittelussa(
    hakuOid: HakuOid,
    sijoitteluajoId: Long
  ): List[HakutoiveenValintatapajonoRecord] =
    timed(
      s"Sijoitteluajon $sijoitteluajoId haun $hakuOid hakemusten hakutoiveiden valintatapajonojen haku",
      100
    ) {
      runBlocking(
        sql"""select j.hakemus_oid,
              ...
              from jonosijat j
                inner join valintatapajonot v on j.valintatapajono_oid = v.oid and j.sijoitteluajo_id = v.sijoitteluajo_id
                inner join tilat_kuvaukset tk on tk.valintatapajono_oid = v.oid and tk.hakemus_oid = j.hakemus_oid and tk.hakukohde_oid = j.hakukohde_oid
              where j.sijoitteluajo_id = ${sijoitteluajoId}""".as[HakutoiveenValintatapajonoRecord]
      ).toList
    }

  override def getHakukohteenHakemuksienHakutoiveenValintatapajonotSijoittelussa(
    hakukohdeOid: HakukohdeOid,
    sijoitteluajoId: Long
  ): List[HakutoiveenValintatapajonoRecord] =
    timed(
      s"Sijoitteluajon $sijoitteluajoId hakukohteen $hakukohdeOid hakutoiveen valintatapajonojen haku",
      100
    ) {
      runBlocking(
        sql"""select j.hakemus_oid,
  ...
  override def getHakemuksenHakutoiveidenHakijaryhmatSijoittelussa(
    hakemusOid: HakemusOid,
    sijoitteluajoId: Long
  ): List[HakutoiveenHakijaryhmaRecord] =
    timed(
      s"Sijoitteluajon $sijoitteluajoId hakemuksen $hakemusOid hakutoiveiden hakijaryhmien haku",
      100
    ) {
      runBlocking(
  ...
  override def getHakijanHakutoiveidenHakijatSijoittelussa(
    hakemusOid: HakemusOid,
    sijoitteluajoId: Long
  ): Map[(HakukohdeOid, ValintatapajonoOid), Int] =
    timed(
      s"Hakemuksen $hakemusOid hakutoiveiden sijojen hakeminen sijoitteluajossa $sijoitteluajoId",
      100
    ) {
      runBlocking(
        sql"""
         ...
         select j.hakukohde_oid, j.valintatapajono_oid, count(j.hakemus_oid) from jonosijat j
           ...
           where j.sijoitteluajo_id = ${sijoitteluajoId}
           group by j.hakukohde_oid, j.valintatapajono_oid"""
          .as[(HakukohdeOid, ValintatapajonoOid, Int)]
      ).map(r => (r._1, r._2) -> r._3).toMap
    }

  override def getHakijanHakutoiveidenHakijatValinnantuloksista(
    hakemusOid: HakemusOid
  ): Map[(HakukohdeOid, ValintatapajonoOid), Int] =
  ...
  override def getHakijanHakutoiveidenHyvaksytytValinnantuloksista(
    hakemusOid: HakemusOid
  ): Map[(HakukohdeOid, ValintatapajonoOid), Int] =
