...
object Role {
  ...
  val VALINTAKAYTTAJA_MUSIIKKIALA = Role(
    "APP_VALINTOJENTOTEUTTAMINEN_TOISEN_ASTEEN_MUSIIKKIALAN_VALINTAKAYTTAJA"
  )
  ...
  val SIJOITTELU_PERUUNTUNEIDEN_HYVAKSYNTA_OPH = Role(
    "APP_SIJOITTELU_PERUUNTUNEIDEN_HYVAKSYNTA_1.2.246.562.10.00000000001"
  )
...
case class CasSession(casTicket: ServiceTicket, personOid: String, roles: Set[Role])
    extends Session {
