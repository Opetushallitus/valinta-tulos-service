...
import fi.vm.sade.valintatulosservice.security.{
  AuditSession,
  CasSession,
  Role,
  ServiceTicket,
...
trait SessionRepositoryImpl extends SessionRepository with ValintarekisteriRepository {
  ...
  override def store(session: Session): UUID =
    session match {
      case AuditSession(_, _) =>
        throw new IllegalArgumentException("Audit-sessiota ei voi tallentaa")
      case CasSession(ServiceTicket(ticket), personOid, roles) =>
        ...
        runBlocking(
          DBIO.seq(
            ...
            DBIO.sequence(
              roles
                .map(role => sqlu"""insert into roolit (sessio, rooli) values ($id, ${role.s})""")
                .toSeq
            )
          ),
  ...
  override def delete(id: UUID): Unit = {
    runBlocking(
      sqlu"""delete from sessiot where id = $id""",
  ...
  override def delete(ticket: ServiceTicket): Unit = {
    runBlocking(
      sqlu"""delete from sessiot where cas_tiketti = ${ticket.s}""",
      timeout = Duration(10, TimeUnit.SECONDS)
    )
  }
