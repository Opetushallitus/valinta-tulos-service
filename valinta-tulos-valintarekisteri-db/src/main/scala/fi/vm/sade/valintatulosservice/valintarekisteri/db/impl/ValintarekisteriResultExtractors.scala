...
trait ValintarekisteriResultExtractors {
  ...
  protected implicit val getValinnantulosResult: GetResult[Valinnantulos] = GetResult(r => Valinnantulos(
    ...
    ehdollisenHyvaksymisenEhtoTekstiFI = r.nextStringOption(),
    ehdollisenHyvaksymisenEhtoTekstiSV = r.nextStringOption(),
    ehdollisenHyvaksymisenEhtoTekstiEN = r.nextStringOption(),
  ...
  implicit object SetValinnanTilanKuvausHashCode extends SetParameter[ValinnanTilanKuvausHashCode] {
    override def apply(valinnanTilanKuvausHashCode: ValinnanTilanKuvausHashCode, pp: PositionedParameters): Unit = {
      pp.setInt(valinnanTilanKuvausHashCode.valinnanTilanKuvausHashCode)
    }
  }
