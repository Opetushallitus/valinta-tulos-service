...
trait ValintarekisteriResultExtractors {
  ...
  protected implicit val getVastaanottoResult = GetResult(r =>
    VastaanottoRecord(
  ...
  protected implicit val getHakukohdeResult = GetResult(r => {
    ...
    (yhdenPaikanSaantoVoimassa, kktutkintoonJohtava) match {
      ...
      case (false, false) =>
        EiKktutkintoonJohtavaHakukohde(oid, hakuOid, r.nextStringOption().map(Kausi(_)))
    }
  })

  protected implicit val getHakijaResult =
    GetResult(r => HakijaRecord(hakemusOid = HakemusOid(r.nextString), hakijaOid = r.nextString))

  protected implicit val getHakutoiveResult = GetResult(r =>
    HakutoiveRecord(
  ...
  protected implicit val getHakutoiveenValintatapajonoResult = GetResult(r =>
  ...
  protected implicit val getHakutoiveenHakijaryhmaResult = GetResult(r =>
  ...
  protected implicit val getSijoitteluajoResult = GetResult(r =>
    SijoitteluajoRecord(
  ...
  protected implicit val getSijoitteluajoHakukohteetResult = GetResult(r =>
  ...
  protected implicit val getValintatapajonotResult = GetResult(r =>
    ValintatapajonoRecord(
      ...
      sivssnovSijoittelunVarasijataytonRajoitus = r.nextIntOption().map { jonoSija =>
        new JonosijaTieto(
          jonoSija,
          r.nextInt(),
          Valinnantila(r.nextString()).valinnantila,
          JsonMethods.parse(r.nextString()).extract[Seq[String]].asJava
        )
      }
    )
  )

  protected implicit val getHakemuksetForValintatapajonosResult = GetResult(r =>
    HakemusRecord(
  ...
  protected implicit val getHakemuksenTilahistoriaResult = GetResult(r =>
    TilaHistoriaRecord(
      valintatapajonoOid = ValintatapajonoOid(r.nextString),
      hakemusOid = HakemusOid(r.nextString),
      tila = Valinnantila(r.nextString),
  ...
  protected implicit val getHakijaryhmatResult = GetResult(r =>
    HakijaryhmaRecord(
      prioriteetti = r.nextInt,
      oid = r.nextString,
      nimi = r.nextString,
  ...
  protected implicit val getTilankuvauksetResult = GetResult(r =>
    TilankuvausRecord(
      hash = r.nextInt,
  ...
  protected implicit val getValinnantulosResult: GetResult[Valinnantulos] = GetResult(r =>
    Valinnantulos(
      hakukohdeOid = HakukohdeOid(r.nextString),
      valintatapajonoOid = ValintatapajonoOid(r.nextString),
      hakemusOid = HakemusOid(r.nextString),
      ...
      vastaanottotila = r.nextStringOption
        .map(VastaanottoAction(_).valintatuloksenTila)
        .getOrElse(ValintatuloksenTila.KESKEN),
      ilmoittautumistila =
        r.nextStringOption.map(SijoitteluajonIlmoittautumistila(_)).getOrElse(EiTehty),
      valinnantilanViimeisinMuutos = parseOffsetDateTime(r),
      vastaanotonViimeisinMuutos = parseOffsetDateTime(r)
    )
  )

  protected implicit val getVastaanottoMuutosResult: GetResult[
    (
      ValintatuloksenTila,
      OffsetDateTime,
      Option[Long],
  ...
  protected implicit val getHakutoiveenValinnantulosResult: GetResult[HakutoiveenValinnantulos] =
    GetResult(r =>
  ...
  protected implicit val getMailReasonOptionResult: GetResult[Option[MailReason]] =
    GetResult(r => r.nextStringOption().map(MailReason.apply))

  protected implicit val getInstantOptionResult: GetResult[Option[Instant]] =
    GetResult(r => r.nextTimestampOption().map(_.toInstant))

  protected implicit val getInstantResult: GetResult[Instant] =
    GetResult(r => r.nextTimestamp().toInstant)

  protected implicit val getValinnantila: GetResult[Valinnantila] =
    GetResult(r => Valinnantila(r.nextString))

  protected implicit val getHaunValinnantilat
    : GetResult[(HakukohdeOid, ValintatapajonoOid, HakemusOid, Valinnantila)] = GetResult(r =>
    (
  ...
  protected implicit val getValintatuloksenTila: GetResult[ValintatuloksenTila] =
    GetResult(r => VastaanottoAction(r.nextString).valintatuloksenTila)

  protected implicit val getSijoitteluajonIlmoittautumistila
    : GetResult[SijoitteluajonIlmoittautumistila] =
    GetResult(r => SijoitteluajonIlmoittautumistila(r.nextString))
  ...
  protected implicit val getOffsetDateTimeOption: GetResult[Option[OffsetDateTime]] =
    GetResult(r => {
      parseOffsetDateTime(r)
    })
  ...
  implicit object SetOptionValintatapajonoOid extends SetParameter[Option[ValintatapajonoOid]] {
    def apply(o: Option[ValintatapajonoOid], pp: PositionedParameters): Unit =
      o match {
  ...
  implicit object SetOptionZonedDateTime extends SetParameter[Option[ZonedDateTime]] {
    def apply(v: Option[ZonedDateTime], pp: PositionedParameters): Unit =
      v match {
