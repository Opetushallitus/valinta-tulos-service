...
trait ValinnantulosRepository extends ValintarekisteriRepository {
  ...
  def getIlmoittautumisenAikaleimat(
    hakuOid: HakuOid
  ...
  def storeValinnantuloksenOhjaus(
    ohjaus: ValinnantuloksenOhjaus,
    ifUnmodifiedSince: Option[Instant] = None
  ): DBIO[Unit]

  def storeValinnantila(
    tila: ValinnantilanTallennus,
    ifUnmodifiedSince: Option[Instant] = None
  ): DBIO[Unit]

  def storeValinnantilaOverridingTimestamp(
    tila: ValinnantilanTallennus,
    ifUnmodifiedSince: Option[Instant] = None,
    tilanViimeisinMuutos: TilanViimeisinMuutos
  ): DBIO[Unit]

  def setJulkaistavissa(
    valintatapajonoOid: ValintatapajonoOid,
    ilmoittaja: String,
    selite: String
  ): DBIO[Unit]

  def setHyvaksyttyJaJulkaistavissa(
    valintatapajonoOid: ValintatapajonoOid,
    ilmoittaja: String,
    selite: String
  ): DBIO[Unit]
  ...
  def updateValinnantuloksenOhjaus(
    ohjaus: ValinnantuloksenOhjaus,
    ifUnmodifiedSince: Option[Instant] = None
  ): DBIO[Unit]
  ...
  def getViimeisinValinnantilaMuutosHyvaksyttyJaJulkaistuJonoOidHistoriasta(
    hakemusOid: HakemusOid,
  ...
  def getValinnantuloksetAndLastModifiedDateForValintatapajono(
    valintatapajonoOid: ValintatapajonoOid,
  ...
  def getValinnantuloksetAndLastModifiedDateForHakemus(
    hakemusOid: HakemusOid
  ): Option[(Instant, Set[Valinnantulos])]
  ...
  def getLastModifiedForValintatapajono(
    valintatapajonoOid: ValintatapajonoOid
  ): DBIO[Option[Instant]]

  def getHakuForHakukohde(hakukohdeOid: HakukohdeOid): HakuOid

  def getHakutoiveidenValinnantuloksetForHakemusDBIO(
    hakuOid: HakuOid,
    hakemusOid: HakemusOid
  ): DBIO[List[HakutoiveenValinnantulos]]

  def deleteValinnantulos(
    muokkaaja: String,
    valinnantulos: Valinnantulos,
    ifUnmodifiedSince: Option[Instant] = None
  ): DBIO[Unit]

  def deleteIlmoittautuminen(
    henkiloOid: String,
    ilmoittautuminen: Ilmoittautuminen,
    ifUnmodifiedSince: Option[Instant] = None
  ): DBIO[Unit]

  def deleteHyvaksyttyJaJulkaistavissa(
    henkiloOid: String,
    hakukohdeOid: HakukohdeOid,
    ifUnmodifiedSince: Option[Instant] = None
  ): DBIO[Unit]

  def deleteHyvaksyttyJaJulkaistavissaIfExists(
    henkiloOid: String,
    hakukohdeOid: HakukohdeOid,
    ifUnmodifiedSince: Option[Instant] = None
  ): DBIO[Unit]

  def getValinnantuloksetAndLastModifiedDateForHakukohde(
    hakukohdeOid: HakukohdeOid,
    timeout: Duration = Duration(10, TimeUnit.SECONDS)
  ): Option[(Instant, Set[Valinnantulos])] =
      ...
      getLastModifiedForHakukohde(hakukohdeOid)
        .flatMap {
          case Some(lastModified) =>
            getValinnantuloksetForHakukohde(hakukohdeOid).map(vs => Some((lastModified, vs)))
  ...
  def getValinnantuloksetAndReadTimeForHaku(
    hakuOid: HakuOid,
    timeout: Duration = Duration(5, TimeUnit.MINUTES)
  ): (Instant, Set[Valinnantulos]) = {
    runBlockingTransactionally(
      now().zip(getValinnantuloksetForHaku(hakuOid)),
      timeout = timeout
    ) match {
      case Right(result) => result
      case Left(error)   => throw error
