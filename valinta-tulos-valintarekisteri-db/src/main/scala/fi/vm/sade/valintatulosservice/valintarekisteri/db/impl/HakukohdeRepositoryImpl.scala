...
trait HakukohdeRepositoryImpl extends HakukohdeRepository with ValintarekisteriRepository {
  ...
  override def all: Set[HakukohdeRecord] = {
    runBlocking(
      sql"""select hakukohde_oid, haku_oid, yhden_paikan_saanto_voimassa, kk_tutkintoon_johtava, koulutuksen_alkamiskausi
            from hakukohteet""".as[HakukohdeRecord]
    ).toSet
  }

  override def findHakukohteet(hakukohdeOids: Set[HakukohdeOid]): Set[HakukohdeRecord] =
    hakukohdeOids match {
      ...
      case _ => {
        ...
        if (invalidOids.nonEmpty) {
          throw new IllegalArgumentException(
            s"${invalidOids.size} huonoa oidia syötteessä: $invalidOids"
          )
        }
        val inParameter = hakukohdeOids.map(oid => s"'$oid'").mkString(",")
        runBlocking(
          sql"""select hakukohde_oid, haku_oid, yhden_paikan_saanto_voimassa, kk_tutkintoon_johtava, koulutuksen_alkamiskausi
            from hakukohteet where hakukohde_oid in (#$inParameter)""".as[HakukohdeRecord]
        ).toSet
      }
    }
    ...
    try {
      runBlocking(
        DBIO.seq(
          ...
          DBIO.sequenceOption(koulutuksenAlkamiskausi.map(alkamiskausi => sqlu"""insert into koulutuksen_alkamiskausi (hakukohde_oid, koulutuksen_alkamiskausi)
  ...
  override def updateHakukohde(hakukohdeRecord: HakukohdeRecord): Boolean = {
    ...
    runBlocking(
      DBIO
        ...
        .andThen(sqlu"""update hakukohteet set (yhden_paikan_saanto_voimassa, kk_tutkintoon_johtava, koulutuksen_alkamiskausi)
                   = (${hakukohdeRecord.yhdenPaikanSaantoVoimassa},
                      ${hakukohdeRecord.kktutkintoonJohtava},
                      ${koulutuksenAlkamiskausi.map(_.toKausiSpec)})
                 where hakukohde_oid = ${hakukohdeRecord.oid} and
                       (yhden_paikan_saanto_voimassa <> ${hakukohdeRecord.yhdenPaikanSaantoVoimassa} or
                        ...
                        koulutuksen_alkamiskausi is distinct from ${koulutuksenAlkamiskausi.map(
          _.toKausiSpec
        )})""")
    ) == 1
  }
  ...
  override def hakukohteessaVastaanottoja(oid: HakukohdeOid): Boolean = {
    runBlocking(
      sql"""select count(*) from newest_vastaanotot where hakukohde = ${oid}""".as[Int]
    ).head > 0
  }
}
