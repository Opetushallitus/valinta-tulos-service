...
object SijoitteluajonHakija {
  def dto(
    repository: HakijaRepository with SijoitteluRepository with ValinnantulosRepository,
      ...
      case Some(x) =>
        (
          timed(s"Getting valinnantulokset for hakemus $hakemusOid", 100) {
            repository
              .runBlocking(repository.getValinnantuloksetForHakemus(hakemusOid))
              .groupBy(_.hakukohdeOid)
          },
          sijoitteluajoId
            .map(id =>
              repository
                .runBlocking(repository.getHakemuksenHakutoiveetSijoittelussa(hakemusOid, id))
                .map(h => h.hakukohdeOid -> h)
                .toMap
            )
            .getOrElse(Map())
        )
    ...
    def getVastaanotto(hakukohdeOid: HakukohdeOid): ValintatuloksenTila = {
      val vastaanotto =
        hakemuksenValinnantulokset.getOrElse(hakukohdeOid, Set()).map(_.vastaanottotila)
      if (1 < vastaanotto.size) {
        val vastaanottoString = vastaanotto.mkString(",")
        throw new RuntimeException(
          s"Hakemukselle ${hakemusOid} löytyy monta vastaanottoa hakukohteelle ${hakukohdeOid}: ${vastaanottoString}"
        )
      } else {
    ...
    val hakutoiveidenHakeneet =
      repository.getHakijanHakutoiveidenHakijatValinnantuloksista(hakemusOid)
    //TODO: Ei tarvitse hakea, jos kaikki hakutoiveet sijoittelussa
    val hakutoiveidenHyvaksytyt =
      repository.getHakijanHakutoiveidenHyvaksytytValinnantuloksista(hakemusOid)
    val hakutoiveidenHakeneetSijoittelussa = sijoitteluajoId
      .map(repository.getHakijanHakutoiveidenHakijatSijoittelussa(hakemusOid, _))
      .getOrElse(Map())

    def hakukohdeDtoSijoittelu(
    ...
    if (hakutoiveetSijoittelussa.isEmpty) {
      ...
      val valintatapajonotSijoittelussa = sijoitteluajoId
        .map(id =>
          repository
            .runBlocking(
              repository.getHakemuksenHakutoiveidenValintatapajonotSijoittelussa(hakemusOid, id)
            )
            .groupBy(_.hakukohdeOid)
        )
        .getOrElse(Map())
      val hakijaryhmatSijoittelussa = sijoitteluajoId
        .map(
          repository
            .getHakemuksenHakutoiveidenHakijaryhmatSijoittelussa(hakemusOid, _)
            .groupBy(_.hakukohdeOid)
        )
        .getOrElse(Map())
      ...
      hakija.map(_.dto(hakukohdeOidit.map { hakukohdeOid =>
        if (hakutoiveetSijoittelussa.contains(hakukohdeOid)) {
          hakukohdeDtoSijoittelu(
            hakukohdeOid,
            valintatapajonotSijoittelussa,
            hakijaryhmatSijoittelussa,
            tilankuvauksetSijoittelussa
          )
        } else {
  ...
  def dto(
    ...
    dto(
      repository,
      Some(repository.runBlocking(repository.getLatestSijoitteluajoId(sijoitteluajoId, hakuOid))),
      hakuOid,
      hakemusOid
    )
  }
...
object SijoitteluajonHakijat {
  ...
  def dto(
    ...
    val hakutoiveet = HakutoiveetGrouped.apply(
      sijoitteluajoId
        .map(repository.getHaunHakemuksienHakutoiveetSijoittelussa(hakuOid, _))
        .getOrElse(List())
    )
    val valinnantulokset = timed("Valinnantulokset haulle", 100) {
      ValinnantuloksetGrouped.apply(
        repository.runBlocking(repository.getValinnantuloksetForHaku(hakuOid))
      )
    }
    val valintatapajonot = ValintatapajonotGrouped.apply(
      sijoitteluajoId
        .map(repository.getHaunHakemuksienValintatapajonotSijoittelussa(hakuOid, _))
        .getOrElse(List())
    )
    val tilankuvauksetSijoittelussa =
      repository.getValinnantilanKuvaukset(valintatapajonot.tilankuvausHashit)
    val hakijaryhmatSijoittelussa = sijoitteluajoId
      .map(repository.getHaunHakemuksienHakijaryhmatSijoittelussa(hakuOid, _))
      .getOrElse(Map())
    dto(
      hakijat,
      hakutoiveet,
  ...
  def dto(
    ...
    dto(
      repository,
      SyntheticSijoitteluAjoForHakusWithoutSijoittelu.getSijoitteluajoId(sijoitteluajo),
  ...
  def dto(
    ...
    hakuOid: HakuOid,
    hakukohdeOid: HakukohdeOid,
    haunValinnantulokset: Option[ValinnantuloksetGrouped] = None
  ): List[HakijaDTO] = {
    val hakijat = repository.getHakukohteenHakijat(hakukohdeOid, sijoitteluajoId)
    val hakutoiveet = HakutoiveetGrouped.apply(
      sijoitteluajoId
        .map(repository.getHakukohteenHakemuksienHakutoiveetSijoittelussa(hakukohdeOid, _))
        .getOrElse(List())
    )
    val valinnantulokset: ValinnantuloksetGrouped = haunValinnantulokset.getOrElse {
      timed("Valinnantulokset haulle", 100) {
        ValinnantuloksetGrouped.apply(
          repository.runBlocking(repository.getValinnantuloksetForHaku(hakuOid))
        )
      }
    ...
    val valintatapajonot = ValintatapajonotGrouped.apply(
      sijoitteluajoId
        .map(repository.getHakukohteenHakemuksienValintatapajonotSijoittelussa(hakukohdeOid, _))
        .getOrElse(List())
    )
    val tilankuvauksetSijoittelussa =
      repository.getValinnantilanKuvaukset(valintatapajonot.tilankuvausHashit)
    val hakijaryhmatSijoittelussa = sijoitteluajoId
      .map(repository.getHakukohteenHakemuksienHakijaryhmatSijoittelussa(hakukohdeOid, _))
      .getOrElse(Map())
    dto(
      hakijat,
      hakutoiveet,
  ...
  def kevytDtoVainHakukohde(
    repository: HakijaRepository with SijoitteluRepository with ValinnantulosRepository,
    sijoitteluajo: SijoitteluAjo,
    hakukohdeOid: HakukohdeOid
  ): List[KevytHakijaDTO] = {
    kevytDtoVainHakukohde(
      repository,
      SyntheticSijoitteluAjoForHakusWithoutSijoittelu.getSijoitteluajoId(sijoitteluajo),
      HakuOid(sijoitteluajo.getHakuOid),
      hakukohdeOid
  ...
  def kevytDtoVainHakukohde(
    ...
    val hakijat = repository.getHakukohteenHakijat(hakukohdeOid, sijoitteluajoId)
    val hakutoive = sijoitteluajoId
      .map(repository.getHakukohteenHakemuksienHakutoiveSijoittelussa(hakukohdeOid, _))
      .getOrElse(List())
    ...
    val valintatapajonot = ValintatapajonotGrouped.apply(
      sijoitteluajoId
        .map(
          repository
            .getHakukohteenHakemuksienHakutoiveenValintatapajonotSijoittelussa(hakukohdeOid, _)
        )
        .getOrElse(List())
    )
    val hakutoiveenTilankuvauksetSijoittelussa =
      repository.getValinnantilanKuvaukset(valintatapajonot.tilankuvausHashit)
    val hakukohteenValinnantulokset = timed(s"Getting hakukohteen $hakukohdeOid valinnantulokset") {
      repository
        .runBlocking(repository.getValinnantuloksetForHakukohde(hakukohdeOid))
        .groupBy(_.hakemusOid)
        .map(t => t._1 -> t._2.groupBy(_.hakukohdeOid))
    }

    hakijat.map(hakija =>
      hakija.kevytDto(hakutoive.get(hakija.hakemusOid) match {
        case Some(x) =>
  ...
  private def kevytDto(
    hakijat: Seq[HakijaRecord],
    ...
    hakijat
      .map(hakija => {
        ...
        val hakijanHakutoiveetEiSijoittelua: Set[HakukohdeOid] =
          filterHakijanHakutoiveetEiSijoittelua(hakija.hakemusOid, valinnantulokset, hakutoiveet)
  ...
  private def dto(
    ...
    valintatapajonot: ValintatapajonotGrouped,
    tilankuvaukset: Map[Int, TilankuvausRecord],
    hakijaryhmat: Map[HakemusOid, List[HakutoiveenHakijaryhmaRecord]]
  ): List[HakijaDTO] =
    timed("HakijaDTOiden luonti " + hakijat.size + " hakijalle", 10) {
      hakijat.par.map(hakija => {
        val hakijanHakutoiveetEiSijoittelua: Set[HakukohdeOid] =
          filterHakijanHakutoiveetEiSijoittelua(hakija.hakemusOid, valinnantulokset, hakutoiveet)
        timed("HakijaDTOn luonti", 100)(
          hakija.dto(
  ...
  private def getVastaanotto(
    hakemusOid: HakemusOid,
    hakukohdeOid: HakukohdeOid,
    hakemuksenValinnantuloksetHakutoiveella: Set[Valinnantulos]
  ): ValintatuloksenTila = {
    val vastaanotto = hakemuksenValinnantuloksetHakutoiveella.map(_.vastaanottotila)
    if (1 < vastaanotto.size) {
      throw new RuntimeException(
        s"Hakemukselle ${hakemusOid} löytyy monta vastaanottoa hakukohteelle ${hakukohdeOid}"
      )
    } else {
      vastaanotto.headOption.getOrElse(ValintatuloksenTila.KESKEN)
    }
  ...
  private def hakutoiveDTOtSijoittelu(
    hakemusOid: HakemusOid,
    ...
    hakemuksenHakutoiveet.map(hakukohde => {
      val hakemuksenValinnantuloksetHakutoiveella =
        hakemuksenValinnantulokset.getOrElse(hakukohde.hakukohdeOid, Set())
      val hakutoiveenHyvaksytytJaHakeneet =
        hyvaksytytJaHakeneet.getOrElse(hakukohde.hakukohdeOid, Map())

      hakukohde.dto(
        getVastaanotto(hakemusOid, hakukohde.hakukohdeOid, hakemuksenValinnantuloksetHakutoiveella),
        hakemuksenValintatapajonot
          .getOrElse(hakukohde.hakukohdeOid, List())
          .map(jono => {
            val jononHyvaksytytJaHakeneet = hakutoiveenHyvaksytytJaHakeneet
              .getOrElse(jono.valintatapajonoOid, HyvaksytytJaHakeneet(0, 0))
            jono.dto(
              hakemuksenValinnantuloksetHakutoiveella
                .find(_.valintatapajonoOid.equals(jono.valintatapajonoOid)),
  ...
  private def hakutoiveDTOtEiSijoittelua(
    hakemusOid: HakemusOid,
    hakukohdeOids: Set[HakukohdeOid],
    hakemuksenValinnantulokset: Map[HakukohdeOid, Set[Valinnantulos]],
    hyvaksytytJaHakeneet: Map[HakukohdeOid, Map[ValintatapajonoOid, HyvaksytytJaHakeneet]]
  ): List[HakutoiveDTO] =
    timed("HakukohdeDTOt ei sijoittelua", 10) {
      hakukohdeOids
        .map(hakukohdeOid => {
          val hakemuksenValinnantuloksetHakutoiveella =
            hakemuksenValinnantulokset.getOrElse(hakukohdeOid, Set())
          ...
          HakutoiveRecord(hakemusOid, Some(1), hakukohdeOid, None).dto(
            ...
            hakemuksenValinnantulokset
              .getOrElse(hakukohdeOid, Set())
              .map(valinnantulos =>
                HakutoiveenValintatapajonoRecord.dto(
                  valinnantulos,
                  hakutoiveenHyvaksytytJaHakeneet
                    .getOrElse(valinnantulos.valintatapajonoOid, HyvaksytytJaHakeneet(0, 0))
                    .hakeneet,
                  hakutoiveenHyvaksytytJaHakeneet
                    .getOrElse(valinnantulos.valintatapajonoOid, HyvaksytytJaHakeneet(0, 0))
                    .hyvaksytyt
  ...
  private def kevytHakukohdeDTOtEiSijoittelua(
    hakemusOid: HakemusOid,
    hakukohdeOids: Set[HakukohdeOid],
    hakemuksenValinnantulokset: Map[HakukohdeOid, Set[Valinnantulos]]
  ): List[KevytHakutoiveDTO] = {
    hakukohdeOids
      .map(hakukohdeOid => {
        val hakemuksenValinnantuloksetHakutoiveella =
          hakemuksenValinnantulokset.getOrElse(hakukohdeOid, Set())
        HakutoiveRecord(hakemusOid, Some(1), hakukohdeOid, None).kevytDto(
          hakemuksenValinnantuloksetHakutoiveella
            .map(valinnantulos => HakutoiveenValintatapajonoRecord.kevytDto(valinnantulos))
            .toList
        )
      })
      .toList
  ...
  private def kevytHakukohdeDTOtSijoittelu(
    hakemusOid: HakemusOid,
    hakemuksenHakutoiveet: List[HakutoiveRecord],
    hakemuksenValintatapajonot: Map[HakukohdeOid, List[HakutoiveenValintatapajonoRecord]],
    hakemuksenValinnantulokset: Map[HakukohdeOid, Set[Valinnantulos]],
  ...
  private def filterHakijanHakutoiveetEiSijoittelua(
    hakemusOid: HakemusOid,
    valinnantulokset: ValinnantuloksetGrouped,
    hakutoiveet: HakutoiveetGrouped
  ) = {
    val hakijanHakutoiveetSijoittelussa: Set[HakukohdeOid] =
      hakutoiveet.hakutoiveOiditHakemuksittain.getOrElse(hakemusOid, Set())
    val hakijanHakutoiveetValinnantuloksissa: Set[HakukohdeOid] =
      valinnantulokset.hakutoiveOiditHakemuksittain.getOrElse(hakemusOid, Set())
    hakijanHakutoiveetValinnantuloksissa.filterNot(hakijanHakutoiveetSijoittelussa)
  }

  case class ValinnantuloksetGrouped(
    valinnantuloksetHakemuksittain: Map[HakemusOid, Map[HakukohdeOid, Set[Valinnantulos]]],
    hakutoiveOiditHakemuksittain: Map[HakemusOid, Set[HakukohdeOid]],
    hyvaksytytJaHakeneetHakukohteittain: Map[
      HakukohdeOid,
      Map[ValintatapajonoOid, HyvaksytytJaHakeneet]
    ]
  )
  ...
  object ValinnantuloksetGrouped {
    def apply(
      ...
      countHyvaksytytJaHakeneet: Boolean = true
      ...
      ValinnantuloksetGrouped(
        ...
        Option(countHyvaksytytJaHakeneet)
          .collect {
            case true =>
              valinnantulokset
                .groupBy(_.hakukohdeOid)
                .map(t =>
                  t._1 -> t._2
                    ...
                    .map(t => t._1 -> HyvaksytytJaHakeneet(t._2))
                )
          }
          .getOrElse(Map())
      )
  ...
  case class HakutoiveetGrouped(
    hakutoiveetSijoittelussa: Map[HakemusOid, List[HakutoiveRecord]],
    hakutoiveOiditHakemuksittain: Map[HakemusOid, Set[HakukohdeOid]]
  )
  ...
  case class ValintatapajonotGrouped(
    valintatapajonotSijoittelussa: Map[HakemusOid, Map[HakukohdeOid, List[
      HakutoiveenValintatapajonoRecord
    ]]],
    tilankuvausHashit: List[Int]
  )
  ...
  object HyvaksytytJaHakeneet {
    def apply(jononTulokset: Set[Valinnantulos]): HyvaksytytJaHakeneet = {
      HyvaksytytJaHakeneet(
        jononTulokset.count(vt =>
          vt.valinnantila == Hyvaksytty || vt.valinnantila == VarasijaltaHyvaksytty
        ),
        jononTulokset.size
      )
    }
