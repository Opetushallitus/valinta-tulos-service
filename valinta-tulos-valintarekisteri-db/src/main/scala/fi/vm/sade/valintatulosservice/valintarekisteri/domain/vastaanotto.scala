...
case class HakijanVastaanottoDto(
  hakemusOid: HakemusOid,
  hakukohdeOid: HakukohdeOid,
  action: HakijanVastaanottoAction
)

case class HakijanVastaanotto(
  henkiloOid: String,
  hakemusOid: HakemusOid,
  hakukohdeOid: HakukohdeOid,
  action: HakijanVastaanottoAction
) extends VastaanottoEvent {
  val ilmoittaja = henkiloOid
  val selite = "Hakijan oma vastaanotto"
}

case class VirkailijanVastaanotto(
  hakuOid: HakuOid,
  valintatapajonoOid: ValintatapajonoOid,
  henkiloOid: String,
  hakemusOid: HakemusOid,
  hakukohdeOid: HakukohdeOid,
  action: VirkailijanVastaanottoAction,
  ilmoittaja: String,
  selite: String
) extends VastaanottoEvent

object VirkailijanVastaanotto {
  def tallennusJarjestys: (VirkailijanVastaanotto, VirkailijanVastaanotto) => Boolean =
    (first, second) => {
  ...
  def apply(dto: VastaanottoEventDto): VirkailijanVastaanotto = {
    VirkailijanVastaanotto(
      ...
      dto.hakemusOid,
      dto.hakukohdeOid,
      VirkailijanVastaanottoAction.getVirkailijanVastaanottoAction(dto.tila),
      dto.ilmoittaja,
      dto.selite
    )
  }
...
case object VastaanotaEhdollisesti
    extends VirkailijanVastaanottoAction
    with HakijanVastaanottoAction {
...
object HakijanVastaanottoAction {
  ...
  def apply(value: String): HakijanVastaanottoAction =
  ...
  def getHakijanVastaanottoAction(vastaanottotila: Vastaanottotila): HakijanVastaanottoAction =
...
object VirkailijanVastaanottoAction {
  ...
  val values: List[String] = valueMapping.keysIterator.toList
  def apply(value: String): VirkailijanVastaanottoAction =
    valueMapping.getOrElse(
      value, {
        throw new IllegalArgumentException(s"Unknown action '$value', expected one of $values")
      }
  ...
  def getVirkailijanVastaanottoAction(
    vastaanottotila: Vastaanottotila
  ): VirkailijanVastaanottoAction =
    vastaanottotila.toString match {
