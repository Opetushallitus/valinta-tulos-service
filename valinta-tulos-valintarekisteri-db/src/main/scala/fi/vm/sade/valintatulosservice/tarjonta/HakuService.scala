...
import fi.vm.sade.valintatulosservice.organisaatio.{
  Organisaatio,
  OrganisaatioService,
  Organisaatiot
}
...
object HakuService {
  def apply(
    ...
    if (config.stubbedExternalDeps) {
      ...
      new CachedHakuService(
        new TarjontaHakuService(config),
        new KoutaHakuService(appConfig, casClient, organisaatioService, koodistoService),
        appConfig
      )
    }
  }
...
case class Hakukohde(
  ...
  def koulutuksenAlkamiskausi: Option[Kausi] =
    (koulutuksenAlkamiskausiUri, koulutuksenAlkamisvuosi) match {
      case (Some(uri), Some(alkamisvuosi)) if uri.matches("""kausi_k#\d+""") =>
        Some(Kevat(alkamisvuosi))
      case (Some(uri), Some(alkamisvuosi)) if uri.matches("""kausi_s#\d+""") =>
        Some(Syksy(alkamisvuosi))
      case _ => None
    }
...
class HakukohdeKelaSerializer
    extends CustomSerializer[HakukohdeKela]((formats: Formats) => {
      ...
      (
        {
          case o: JObject =>
            ...
            val children =
              (o \ "koulutusLaajuusarvos").extractOpt[Seq[KoulutusLaajuusarvo]].getOrElse(Seq())
            HakukohdeKela(
...
protected trait JsonHakuService {
  ...
  protected def toHaku(haku: HakuTarjonnassa): Haku = {
    ...
    val amkopeTarkenteet = Set(
      "haunkohdejoukontarkenne_2#",
      "haunkohdejoukontarkenne_4#",
      "haunkohdejoukontarkenne_5#",
      "haunkohdejoukontarkenne_6#"
    )
    val sallittuKohdejoukkoKelaLinkille: Boolean =
      !haku.kohdejoukonTarkenne.exists(tarkenne => amkopeTarkenteet.exists(tarkenne.startsWith))
    val toinenAste: Boolean = Option(haku.kohdejoukkoUri).exists(k =>
      k.contains("_11") || k.contains("_17") || k.contains("_20")
    )
    val koulutuksenAlkamisvuosi = haku.koulutuksenAlkamisVuosi
    val kausi =
      if (haku.koulutuksenAlkamiskausiUri.isDefined && haku.koulutuksenAlkamisVuosi.isDefined) {
        if (haku.koulutuksenAlkamiskausiUri.get.startsWith("kausi_k")) {
          ...
          throw new MappingException(
            s"Haku ${haku.oid} has unrecognized kausi URI '${haku.koulutuksenAlkamiskausiUri.get}' . Full data of haku: $haku"
          )
      } else None
...
class CachedHakuService(tarjonta: TarjontaHakuService, kouta: KoutaHakuService, config: AppConfig)
    extends HakuService {
  ...
  private val kaikkiJulkaistutHautCache = TTLOptionalMemoize.memoize[Unit, List[Haku]](
    f = _ =>
      tarjonta.kaikkiJulkaistutHaut.right.flatMap(tarjontaHaut =>
        kouta.kaikkiJulkaistutHaut.right.map(_ ++ tarjontaHaut)
      ),
    lifetimeSeconds = Duration(4, HOURS).toSeconds,
    maxSize = 1
  )
...
private case class HakuTarjonnassa(
  oid: HakuOid,
  hakutapaUri: String,
...
class TarjontaHakuService(config: HakuServiceConfig)
    extends HakuService
    with JsonHakuService
    with Logging {
  ...
  def getHakukohdeKela(hakukohdeOid: HakukohdeOid): Either[Throwable, Option[HakukohdeKela]] = {
    for {
      ...
      kelaHakukohde <-
        (if (haku.sallittuKohdejoukkoKelaLinkille) {
           val hakukohdeUrl =
             config.ophProperties.url("tarjonta-service.hakukohdekela", hakukohdeOid)
           fetch(hakukohdeUrl) { response =>
             Some(parse(response).extract[HakukohdeKela])
           }.left.map {
             case e: IllegalArgumentException =>
               new IllegalArgumentException(s"No hakukohde $hakukohdeOid ($hakukohdeUrl) found", e)
             case e: IllegalStateException =>
               new IllegalStateException(
                 s"Parsing hakukohde $hakukohdeOid ($hakukohdeUrl) failed",
                 e
               )
             case e: Exception =>
               new RuntimeException(s"Failed to get hakukohde $hakukohdeOid ($hakukohdeUrl)", e)
           }
         } else {
           Right(None)
         }).right
  ...
  def getHakukohde(hakukohdeOid: HakukohdeOid): Either[Throwable, Hakukohde] = {
    val hakukohdeUrl = config.ophProperties.url(
      ...
      hakukohdeOid,
      Map("populateAdditionalKomotoFields" -> true).asJava
    )
    fetch(hakukohdeUrl) { response =>
      ...
      case e: IllegalArgumentException =>
        new IllegalArgumentException(s"No hakukohde $hakukohdeOid ($hakukohdeUrl) found", e)
      case e: IllegalStateException =>
        new IllegalStateException(s"Parsing hakukohde $hakukohdeOid ($hakukohdeUrl) failed", e)
      case e: Exception =>
        new RuntimeException(s"Failed to get hakukohde $hakukohdeOid ($hakukohdeUrl)", e)
    }
  }
  ...
  def kaikkiJulkaistutHaut: Either[Throwable, List[Haku]] = {
    val url =
      config.ophProperties.url("tarjonta-service.find", Map("addHakuKohdes" -> false).asJava)
    fetch(url) { response =>
...
case class KoutaHakuaika(alkaa: String, paattyy: String) {
  ...
  def toHaku: Either[Throwable, Haku] = {
    for {
      alkamiskausi <- ((alkamiskausiKoodiUri, alkamisvuosi.map(s => (s, Try(s.toInt)))) match {
          case (Some(uri), Some((_, Success(vuosi)))) if uri.startsWith("kausi_k#") =>
            Right(Some(Kevat(vuosi)))
          case (Some(uri), Some((_, Success(vuosi)))) if uri.startsWith("kausi_s#") =>
            Right(Some(Syksy(vuosi)))
          case (Some(uri), Some((_, Success(_)))) =>
            Left(new IllegalStateException(s"Unrecognized koulutuksen alkamiskausi URI $uri"))
          case (Some(_), Some((s, Failure(t)))) =>
            Left(new IllegalStateException(s"Unrecognized koulutuksen alkamisvuosi $s", t))
          case _ => Right(None)
        }).right
    } yield Haku(
      oid = HakuOid(oid),
      korkeakoulu = kohdejoukkoKoodiUri.startsWith("haunkohdejoukko_12#"),
      toinenAste = List("haunkohdejoukko_11#", "haunkohdejoukko_17#", "haunkohdejoukko_20#").exists(
        kohdejoukkoKoodiUri.startsWith
      ),
      sallittuKohdejoukkoKelaLinkille = !List(
        "haunkohdejoukontarkenne_2#",
        "haunkohdejoukontarkenne_4#",
        "haunkohdejoukontarkenne_5#",
        "haunkohdejoukontarkenne_6#"
      ).exists(kiellettyTarkenne =>
        kohdejoukonTarkenneKoodiUri.exists(_.startsWith(kiellettyTarkenne))
      ),
      ...
      yhdenPaikanSaanto = YhdenPaikanSaanto(
        voimassa = false,
        syy = "Yhden paikan sääntö Kouta:ssa aina hakukohdekohtainen"
      ),
...
case class KoutaToteutus(oid: String, koulutusOid: String, tarjoajat: List[String])
...
case class KoutaHakukohde(
  ...
  def toHakukohde(
    haku: Option[KoutaHaku],
    toteutus: KoutaToteutus,
    koulutus: KoutaKoulutus,
    tarjoajaorganisaatiot: List[Organisaatio]
  ): Either[Throwable, Hakukohde] = {
    for {
      tarjoaja <-
        tarjoajaorganisaatiot.headOption
          .toRight(new IllegalStateException(s"Could not find tarjoaja for hakukohde $oid"))
          .right
      koulutuksenAlkamisvuosi <- ((if (kaytetaanHaunAlkamiskautta) { haku.get.alkamisvuosi }
                                   else { alkamisvuosi }).map(s => (s, Try(s.toInt))) match {
          case Some((_, Success(vuosi))) => Right(Some(vuosi))
          case Some((s, Failure(t))) =>
            Left(new IllegalStateException(s"Unrecognized koulutuksen alkamisvuosi $s", t))
      ...
      koulutusAsteTyyppi = if (List("yo", "amk").exists(koulutus.koulutustyyppi.contains(_))) {
        "KORKEAKOULUTUS"
      } else { "" },
      ...
      koulutuksenAlkamiskausiUri = if (kaytetaanHaunAlkamiskautta) { haku.get.alkamiskausiKoodiUri }
  ...
  def toHakukohdeKela(
    ...
    tarjoajaorganisaatiohierarkiat: List[Organisaatiot]
  ): Either[Throwable, HakukohdeKela] = {
    val koulutuksenAlkamiskausiUri = if (kaytetaanHaunAlkamiskautta) {
      haku.get.alkamiskausiKoodiUri
    } else { alkamiskausiKoodiUri }
    val koulutuksenAlkamisvuosi = if (kaytetaanHaunAlkamiskautta) { haku.get.alkamisvuosi }
    else { alkamisvuosi }
    for {
      oppilaitos <-
        tarjoajaorganisaatiohierarkiat.toStream
          .map(_.find(_.organisaatiotyypit.contains("OPPILAITOS")))
          .collectFirst {
            case Some(oppilaitos) => oppilaitos
          }
          .toRight(new IllegalStateException(s"Could not find oppilaitos for hakukohde $oid"))
          .right
      oppilaitoskoodi <-
        oppilaitos.oppilaitosKoodi
          .toRight(
            new IllegalStateException(
              s"Could not find oppilaitoskoodi for oppilaitos ${oppilaitos.oid}"
            )
          )
          .right
      koulutuksenAlkamiskausi <-
        ((koulutuksenAlkamiskausiUri, koulutuksenAlkamisvuosi.map(s => (s, Try(s.toInt)))) match {
          case (Some(uri), Some((_, Success(vuosi)))) if uri.startsWith("kausi_k#") =>
            Right(Some(Kevat(vuosi)))
          case (Some(uri), Some((_, Success(vuosi)))) if uri.startsWith("kausi_s#") =>
            Right(Some(Syksy(vuosi)))
      ...
      koulutuslaajuusarvot = Seq(
        KoulutusLaajuusarvo(
          ...
          koulutustyyppi =
            koulutuskoodi.flatMap(_.findSisaltyvaKoodi("koulutustyyppi")).map(_.arvo),
...
class KoutaHakuService(
  ...
  def getHakukohdeKela(oid: HakukohdeOid): Either[Throwable, Option[HakukohdeKela]] = {
    if (false) {
      for {
        ...
        koutaHaku <-
          (if (koutaHakukohde.kaytetaanHaunAlkamiskautta) {
        ...
        koulutuskoodi <-
          koutaKoulutus.koulutusKoodiUri
            .fold[Either[Throwable, Option[Koodi]]](Right(None))(
              koodistoService.getKoodi(_).right.map(Some(_))
            )
            .right
        opintojenlaajuuskoodi <-
          koutaKoulutus.metadata
            .flatMap(_.opintojenLaajuusKoodiUri)
            .fold[Either[Throwable, Option[Koodi]]](Right(None))(
              koodistoService.getKoodi(_).right.map(Some(_))
            )
            .right
        tarjoajaorganisaatiohierarkiat <-
          MonadHelper
            .sequence(koutaHakukohde.tarjoajat(koutaToteutus).map(organisaatioService.hae))
            .right
        hakukohde <-
          koutaHakukohde
            .toHakukohdeKela(
  ...
  def getHakukohde(oid: HakukohdeOid): Either[Throwable, Hakukohde] = {
    for {
      ...
      koutaToteutus <- getKoutaToteutus(koutaHakukohde.toteutusOid).right
      koutaKoulutus <- getKoutaKoulutus(koutaToteutus.koulutusOid).right
      tarjoajaorganisaatiot <-
        MonadHelper.sequence(koutaHakukohde.tarjoajat(koutaToteutus).map(getOrganisaatio)).right
      hakukohde <-
        koutaHakukohde
          .toHakukohde(koutaHaku, koutaToteutus, koutaKoulutus, tarjoajaorganisaatiot)
          .right
    } yield hakukohde
  }
  ...
  private def getKoutaHaunHakukohteet(oid: HakuOid): Either[Throwable, Seq[KoutaHakukohde]] = {
    ...
    fetch[List[KoutaHakukohde]](
      config.ophUrlProperties.url("kouta-internal.hakukohde.search", query)
    )
  }
  ...
  private def getOrganisaatio(oid: String): Either[Throwable, Organisaatio] = {
    organisaatioService
      ...
      .flatMap(
        _.find(_.oid == oid)
          .toRight(new IllegalArgumentException(s"Could not find organisation $oid"))
      )
  }
  ...
  private def fetch[T](url: String)(implicit manifest: Manifest[T]): Either[Throwable, T] = {
    Uri
      ...
      .flatMap(uri =>
        client
          .fetch(Request(method = GET, uri = uri)) {
                ...
                .handleWith {
                  case t =>
                    r.bodyAsText.runLog
                      ...
                      .flatMap(body =>
                        Task.fail(
                          new IllegalStateException(s"Parsing result $body of GET $url failed", t)
                        )
                      )
                }
            case r if r.status.code == 404 =>
              r.bodyAsText.runLog
                .map(_.mkString)
                .flatMap(body =>
                  Task.fail(new IllegalArgumentException(s"GET $url failed with status 404: $body"))
                )
            case r =>
              r.bodyAsText.runLog
                .map(_.mkString)
                .flatMap(body =>
                  Task.fail(
                    new RuntimeException(s"GET $url failed with status ${r.status.code}: $body")
                  )
                )
          }
          .unsafePerformSyncAttemptFor(Duration(1, TimeUnit.MINUTES))
      )
      .toEither
  }
}
