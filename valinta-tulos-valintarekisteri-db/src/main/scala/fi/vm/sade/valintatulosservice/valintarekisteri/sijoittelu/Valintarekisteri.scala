...
import fi.vm.sade.valintatulosservice.valintarekisteri.db.impl.{
  ValintarekisteriDb,
  ValintarekisteriRepository
}
import fi.vm.sade.valintatulosservice.valintarekisteri.db.{
  SijoitteluRepository,
  StoreSijoitteluRepository,
...
class ValintarekisteriForSijoittelu(
  valintarekisteriDb: SijoitteluRepository
    with StoreSijoitteluRepository
    with ValinnantulosRepository,
  hakukohdeRecordService: HakukohdeRecordService
) extends Valintarekisteri(valintarekisteriDb, hakukohdeRecordService) {

  private def this(
    appConfig: ValintarekisteriAppConfig.ValintarekisteriAppConfig,
  ...
  def this(properties: java.util.Properties) =
    this(ValintarekisteriAppConfig.getDefault(properties))
}

class ValintarekisteriService(
  valintarekisteriDb: SijoitteluRepository
    with StoreSijoitteluRepository
    with ValinnantulosRepository,
  hakukohdeRecordService: HakukohdeRecordService
...
abstract class Valintarekisteri(
  valintarekisteriDb: SijoitteluRepository
    with StoreSijoitteluRepository
  ...
  def tallennaSijoittelu(
    ...
    logger.info(
      s"Poistetaan haun ${sijoitteluajo.getHakuOid} tuloksen hakukohteista hakijaryhmät, joiden jonoja ei ole sijoiteltu"
    )
    ...
    try {
      ...
      sijoittelu.hakukohteet
        .map(h => HakukohdeOid(h.getOid))
        .foreach(hakukohdeRecordService.getHakukohdeRecord)
      ...
      case sqle: java.sql.SQLException =>
        ...
        logger.error(
          s"Sijoitteluajon tallennus haulle ${sijoitteluajo.getHakuOid} epäonnistui tietokantavirheeseen:\n$message"
        )
        throw new Exception(message)
      case e: Exception =>
        logger.error(
          s"Sijoitteluajon tallennus haulle ${sijoitteluajo.getHakuOid} epäonnistui: ${e.getMessage}"
        )
        throw new Exception(e)
    }
  }

  def cleanRedundantSijoitteluTuloksesForHakemusInHakukohde(
    hakemusOid: String,
    hakukohdeOid: String
  ): Unit = {
    valintarekisteriDb.deleteSijoitteluResultsForHakemusInHakukohde(
  ...
  def getSijoitteluajo(hakuOid: String, sijoitteluajoId: String): SijoitteluAjo = {
    val latestId = valintarekisteriDb.runBlocking(
      valintarekisteriDb.getLatestSijoitteluajoId(sijoitteluajoId, HakuOid(hakuOid))
    )
    valintarekisteriDb
  ...
  def getValintatulokset(hakuOid: String): java.util.List[Valintatulos] = {
    val (read, valinnantulokset) =
      valintarekisteriDb.getValinnantuloksetAndReadTimeForHaku(HakuOid(hakuOid))
  ...
  def getHakukohdeForSijoitteluajo(sijoitteluajoId: Long, hakukohdeOid: String) = {
    new SijoitteluajonHakukohde(valintarekisteriDb, sijoitteluajoId, HakukohdeOid(hakukohdeOid))
      .entity()
  }
}
...
object Valintarekisteri extends Logging {
  ...
  /**
    ...
    */
  def poistaValintatapajonokohtaisetHakijaryhmatJoidenJonoaEiSijoiteltu(
    hakukohteet: util.List[Hakukohde]
  ) {
    hakukohteet.asScala.foreach(h => {
      val original = h.getHakijaryhmat.size

      h.setHakijaryhmat(
        h.getHakijaryhmat.asScala
          .filter(r =>
            null == r.getValintatapajonoOid || h.getValintatapajonot.asScala
              .map(_.getOid)
              .contains(r.getValintatapajonoOid)
          )
          .asJava
      )
      ...
      (original - h.getHakijaryhmat.size) match {
        case x: Int if x > 0 =>
          logger.info(
            s"Poistettiin $x hakijaryhmää, jotka viittasivat jonoihin, joita ei ollut " +
