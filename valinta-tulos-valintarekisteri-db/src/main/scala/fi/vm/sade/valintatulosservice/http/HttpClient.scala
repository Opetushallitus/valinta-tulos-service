...
object DefaultHttpClient extends HttpClient {
  ...
  def httpPost(url: String, data: Option[String], options: HttpOptions.HttpOption*)
              (clientCallerId: String): HttpRequest = {
    val postRequest = Http(url).method("POST").options(options)
    data match {
      case None => new DefaultHttpRequest(postRequest.postForm
        .header("Caller-Id", clientCallerId)
        .header("CSRF", clientCallerId)
        .cookie("CSRF", clientCallerId))
